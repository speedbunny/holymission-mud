...
/*
 * Initial version used for expansion by Bonzo.
 * Expansion would include:
 * set_alt_name(), add_name() (Which implies a different method
 * of storing the id...) and set_smell().
 */

string name, alias_name, short_desc, long_desc, read_message, read_file;
status can_get,alt_name;
int weight, value;
...
read(str) {
	...
	say(this_player()->query_name() + " reads the " + name + ".\n");
	    ...
	    cat(read_file);
            ...
            write(process_string(read_message));
	return 1;
    }
} /* read */
