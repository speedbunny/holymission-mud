...
string function,match,type,parser,dir;
int tt;
int tried;
string walk;
int gd,place;
...
object voorwerp;
  ...
  if(!arg) {
    ...
    for(i=0;i<15;i++)
     tried[i] = 0;
    dir = ({"pub2","church","vill_shore4","vill_shore3","vill_shore2",
            "vill_shore","vill_green","vill_track","vill_road1",
            "vill_road1b","vill_road2","vill_road3","main_shop",
            "station","jetty","jetty2","broker","party_booth",
            "adv_guild",
           });  /* All these rooms are located in the main village
                   that bard Taliesin knows */
    load_chat(1,({
      ...
      "Dancing and singing brings out the wonderous varieties in nature.\n",
      "Taliesin says: I love the birds, flying high upon a wing and a prayer.\n",
      "Taliesin says: If you want a tag, just ask me to give one.\n"}));
    set_heart_beat(1);
    walk = ({"north","southeast","northwest","down","up","out","exit","west",
    ...
    if(!present("harp",this_object())) {
      ...
      move_object(voorwerp,this_object());
      transfer(clone_object("players/kbl/instruments/piano"),this_object());
    }
  }
}
...
move_vill()
{
  ...
  for(i=0;i<19;i++)
  {
    if(environment(this_object())->short() == "/room/"+dir[i]->short())
    {
      pla = i;
      loc = environment(this_object())->short();
    }
  }
  if(!loc)
   return;
  switch(pla)
  {
    case 0: ran = random(16);
            if(ran > 10)
             command("play piano",this_object());
            break;
    case 1: ran = random(16);
            if(ran > 10)
              say("I invite you to join the Bards guild. Music sooths the\n"+
                  "savage beast. Best off to play as a adventure for a time\n"+
                  "then come join me for real.\n");
             break;
    case 2: ran = random(16);
            switch(ran)
            {
              case 0: say("Taliesin waxes his violin. He smiles happily\n"+
                          "as he hums a tune while waxing.\n");
                      break;
              case 1:
              case 2:
              case 3:
              ...
              case 5:
              case 6:
              case 7: command("sing Just like heaven",this_object());
                      break;
             }
    case 3:
    case 4:
    ...
    case 10:
    case 11:
    case 12: say("Taliesin hums a ancient tune to himself.\n");
             ran = random(16);
             switch(ran)
             {
               case 0:
               ...
               case 5:
               case 6:
               case 7:  command("east",this_object());
                        break;
               case 8:
               case 9:
               case 10:
               case 11:
               case 12:  command("west",this_object());
                         break;
    case 13: ran = random(10);
             if(ran > 4)
    ...
    default: transfer(this_object(),"/room/pub2");
             break;
  }
}
...
move_kbl()
{
  ...
  if((environment(this_object())->short() == "/players/kbl/kblroom"->short())
      && tt < 5)
  {
    ranl = random(10);
    switch(ranl)
    {
      case 0: command("look at floor",this_object());
              say("I love the way the clovers intertwine.\n");
              command("smile",this_object());
              break;
      case 1: command("look at ceiling",this_object());
              say("I could sing for you! KBL!\n");
              command("sing Just like heaven",this_object());
              break;
      case 2: command("look at table",this_object());
              say("Nice piece of work, very smooth surface. Wonder if its\n"+
                  "hiding something under that smooth finish.\n");
              break;
      case 3: command("read sign",this_object());
              break;
      case 4: command("look at plaque",this_object());
              break;
      case 5: command("look at portal",this_object());
              command("west",this_object());
              break;
      case 6: command("look at floor",this_object());
              say("Beautiful clover patterns, so interwoven, so precious.\n");
              break;
      case 7: command("look at decanter",this_object());
              command("smile",this_object());
              break;
      case 8: command("look at paintings",this_object());
              say("The one jumping into the light! Oh, you must do these\n"+ 
                  "type of things to solve Kbls' quest. Many bad creatures\n"+
                  "there. Chaotic. Still, some are aggressive and some are\n"+
                  "not. You must be careful. Retrieve the Any Slayer from\n"+
                  "Zesronatae and return it to the Light Elemental. It can\n"+
                  "be found in the upper most level of Kbl's library.\n");
      case 9: command("look",this_object());
              say("Water Elementals hide in places you would never jump\n"+
                  "into. *sigh* I've seen many drown. Are you good enough?\n"+
                  "Ha! Kbl has a devious mind, but I figured him out! Yes.\n"+
                  "He has a way to get to his library, buyt it takes gutts!\n"+
                  "Let go. Just let go. *sigh* I don't know where tho. I'm\n"+
                  "working on that maybe being a place or a state of mind.\n");
              break;
    }
    tt++;
    say("I'm in the right place!\n");
    return 1;
  }
  else
    if(tt > 4)
...
string who,phrase;
  ...
  if(environment(this_player())!=environment()) {
    tell_object(this_player(),
      "Taliesin tells you: Please, come here if it's important.\n"
    + "My surroundings are "+lower_case(environment()->short())+".\n");
    return;
  }
