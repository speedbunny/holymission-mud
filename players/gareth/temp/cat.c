...
long()
{
  if (petname && friend)
     ...
     write("You see a cute green-eyed, black panther.\n");
...
short()
{
   if (objectp(friend) && environment() == friend)
      return "Panther ["+friend->query_name()+"'s pet]";
   return "Panther";
}
...
int get()
{
  ...
  else if (!objectp(friend))
  {
     ...
     tell_object(tp,"Your panther purrs softly.\n");
     set_heart_beat(0);
     return 1;
  } else 
...
void heart_beat()
{
  ...
  if (objectp(friend) && env()!=friend && !stay)
  {
      ...
      if (env()!=env(friend))
      {
          tell_room(env(friend),"Panther pads in.\n");
          SAY("Panther pads out.\n");
...
void init()
{
  ...
  if (this_player() && environment()!=this_player())
      add_action("fcn_command","C");  /* commanding the panther */
...
int fcn_task(string arg)
{
  string where, what;
    ...
    {
        say(apply(call,tp,"query_name")+" gives Panther an order.\n"); 
        SAY("Panther shrugs helplessly.\n");
        return 1;
    }
    ...
    if (find_object(places[where]))
    {
       SAY("Panther pads out.\n");
       stay = 1;
       tell_room(places[where],"Panther pads in.\n");   
  ...
  if (!arg || arg=="all")
  {
      ...
      for(sz = sizeof(inv),i=0;i<sz;i++)
      {
          if (stringp(apply(call,inv[i],"query_name")))
          {
              ...
              "Panther drops "+apply(call,inv[i],"query_name")+".\n");
          }
       }
      return 1;
    }
...
int fcn_resc(string arg)
{
            ...
            && vic!=this_object() && !stay)
   {
     if (env(friend) == env())
     {
       ...
       write("Your Panther jumps into the fight to rescue you !\n");
       say(apply(call,friend,"query_name")+
           "' panther jumps into the fight !\n");
       attack_object(vic);
...
int fcn_eat()
{
  ...
  {
   SAY("Panther devours the corpse.\n");
   destruct(corpse);
...
int fcn_stay()
{
   ...
   if (!stay)
   {
     SAY("Panther sits down and rests.\n");    
     stay = 1;
	 return 1;
   }
   SAY("Panther stands up.\n");    
   stay = 0;
   return 1;
}
...
int fcn_kill(string whom)
{
  ...
  mob = present(whom,env());
    ...
    write("Your panther looks confused.\n");
    return 1;
  }
 ...
 if (mob && living(mob) && this_player()==friend)
 {
	...
	{
	   tell_object(friend,"Your panther dislikes your attemt.\n");
	   return 1; 
    }
	SAY("A panther bites "+mob->query_name()+".\n");
	attack_object(mob);
	change_attacker(mob);
 }
return 1;
...
void catch_tell(string arg)
{
   if (friend && interactive(friend) && stay)
	   tell_object(friend,"panther#"+arg);
}
...
{
 string h;
 if (sscanf(arg,"panther %s",h)!=1)
     return 0;
 else if (tp!=friend) 
 {
   notify_fail("You are not the owner of this panther.\n");
   return 0;
 }  
   petname = h;
   set_alt_name(petname);
  write("Your panther is now called "+petname+".\n");
  return 1;
}
...
int no_follower(object player)
{
  ...
  if (ofriend = present("panther",player) && 
...
attack()
{
  ...
  {
   tell_object(friend,"Your cat flees head over heals.\n");
	destruct(to);
  }
  if (random(100) < 80)
      ::attack();
...
{
   if ((arg!="panther" && arg!=petname) || tp!=friend)
      return 0;
   write("Your panther leaves you.\n");
   destruct(this_object());
   return 1;
}
