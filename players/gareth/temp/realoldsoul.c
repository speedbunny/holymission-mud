...
time_guild_soul() { 
    if(!no_but) return;
    if(xatt && living(xatt) && environment(xatt)==environment(TP)) {
     write(xmsg);
...
compare(str) {
  ...
  if(w1-w2>0)
      ...
      write(wp2 + " glows in a strange red light.\n");
  else
...
berzerk() {
  ...
  is_berzerk=1;
  berz_where=environment(this_player());
  ...
  if(arg && (whom=present(arg,environment(this_player())))) {
      if(!whom->query_attack()) {
          ...
          if(random(100)<30+(2*this_player()->query_level())) {
              ...
              write("You couldn't irritate the monster.\n");
          }
          return 1;
      }
  }
