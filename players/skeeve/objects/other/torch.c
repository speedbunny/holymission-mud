 ...
 * 280393 Modified by Skeeve.
 *       error: player also hears, when torch goes out!
 *       version 6: The torch flickers before goes out.
 */
...
string name;
status is_lighted;
int amount_of_fuel,weight,auto_light,greek_fire;
...
light(str) {
  ...
  if(environment(this_player())->query_env_name()=="water") {
    if((!greek_fire)&&(environment(this_player())->query_danger())) {
  ...
  is_lighted = 1;
  if (amount_of_fuel > WEAK_TIME) {
    call_out("out_of_fuel", (amount_of_fuel - WEAK_TIME)*2);
    amount_of_fuel = WEAK_TIME;
  } else {
...
out_of_fuel() {
  ...
  if (amount_of_fuel==WEAK_TIME) {
    tell_room (environment(this_player()),
      "The " + name + " flickers momentarily.  It's flame seems weak.\n");
    long_desc = "A weak torch.\n";
    call_out("out_of_fuel",amount_of_fuel*2);
    amount_of_fuel = 0;
    return;
  }
  if (set_light(-1) == 0) {
    tell_room (environment(this_player()),
      "There is darkness as a " + name + " goes dark.\n");
  } else {
    tell_room (environment(this_player()),
      "The " + name + " goes dark.\n");
  }
