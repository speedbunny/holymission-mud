...
#include <lw.h>
#define PATH "/players/kawai/areas/quest/"
#define OPATH "/players/kawai/obj/"
#define NUM_COINS 12
#define MAX_MOVES 3
#define VISION "A vision of Kazak appears and says: "
#define DEBUG 0
...
int verify_move(string str, int x)
 {if(!str)
    {if(!x) 
      {notify_fail(lw(VISION+
      "Come again, fool?  The proper way to utilize the equipment "+
      ...
      {notify_fail(lw(VISION+
      "Say again, pathetic creature...what coin do you choose?\n"));
      return 0;}}
  if(this_player()!=player)
    {if(player)
      {if(present(player,this_object()))
        ...
        "different place.\n");
        this_player()->move_player("in a burst of light","room/church");
        return 1;}
      writelw(VISION+
  ...
  if((num<1)||(num>NUM_COINS))
    ...
    {if(coins[num-1]==2)
      ...
      "pay the price!\n\n"+
       ...
       reset_game();
       this_player()->move_player("in a puff of smoke","room/church");
       return 1;}
    else
      {writelw(VISION+
      "Such surprise from my little plaything.  Proceed...\n");
      reset_game();
      this_player()->move_player("in a puff of smoke",PATH+"entrance");
      return 1;}}
  return 0;}
...
status read_note(string str) 
  ...
  if(str=="note")
    {writelw(
    "Greetings prospective Cultist!\n\n"+
    "   As you are aware, only those of great wisdom may make our "+
    "company.  In order to join us, you must pass a simple test!  "+
    "On the table "+
    ...
    "for example, 'balance 1 3 4 against 2 7 9'.  The scale will then "+
    ...
    "price for failure.\n"+
   "                                           -Lupo The Wizard\n"+
   "                                            Grand Wizard, Follower of Kazak\n");
    return 1;}
  else
    {return 0;}}
...
void reset(int arg)
  {if(!arg)
    ...
    reset_game();
    set_light(1);
    short_desc="The Gateway";
    long_desc="You have somehow managed to make you way "+
    "into a small chamber of solid rock.  There are no entrances, no exits, "+
    "only a small "+
