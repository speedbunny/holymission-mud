...
void reset(int arg)
{
	...
	if(thief)  // changed by Llort .. caused Bugs in the startup of the mud
	{
	    dex       = thief->query_dex();
...
void init() {
    ...
    if (thief && !immortal)
	thief->set_vis();
    if(thief->query_real_name()=="blade"){
      add_action("test","test");
      add_action("thief_wear","thief_wear");
      add_action("xtra_look","l");
    }
    ...
    if(!immortal||thief->query_real_name()=="blade"){
      get_guild_params();
...
void guild_changes() {
    ...
    if (thief->thief_shadow()) {
	if (immortal)
	    write("Destructing the thief_shadow.\n");
...
int xtra_look(string arg){
  object looked_at;
  string poison_level;
  if(sscanf(arg,"at %s",arg)==1){
    if(looked_at=present(arg, environment(thief))){
	...
	write("You think "+looked_at->query_name()+" is "+poison_level+
	      " poisoned.\n");
	return 0;
      }
    }
...
int put_guild_params(){
  ...
  rm("guild/thieves/"+thief->query_real_name());
  write_file("guild/thieves/"+thief->query_real_name(),
...
int get_guild_params(){
  string file_string;
  if(!read_file("/players/blade/guild/thieves/"+thief->query_real_name())){
    return 1;
  }
  sscanf(read_file("/players/blade/guild/thieves/"+thief->query_real_name()),
	 "%s",file_string);
...
int appraise(string arg){
  ...
  if(appraised_ob = present(arg, thief)) ;
    ...
    if(appraised_ob = present(arg, environment(thief))) ;
    else{
  ...
  final=(int)(appraised_ob->query_value()*mistake);
  final=random(final) - (final / 2);
  final=appraised_ob->query_value()+final;
...
int throw(string arg){
  ...
  if (!(victim = present(v_name, environment(thief))) || !living(victim)) {
    write("Throw at whom?\n");
    return 1;
  }
  ...
  if(thrown_weapon->query_weight()>2){
    write(w_name+" is too unwieldly to throw.\n");
    return 1;
  }
  if(victim==thief){
  ...
  if(thrown_weapon->query_weapon()){
    tell_object(thief,"You throw "+thrown_weapon->query_name()+" at "+
		victim->query_name()+".\n");
    no_throw=1;
  }
  else{
    write(w_name+" is not a weapon.\n");
    return 1;
  }
...
int mudball(string arg){
  ...
  if(random(hit)>25){
    ...
    tell_object(victim,thief->query_name()+" throws a ball of mud at you!\n");
    say(thief->query_name()+" throws a ball of mud at "+victim->query_name()+
	"!\n");
    return 1;
  }
  else{
    write("You throw a ball of mud at "+victim->query_name()+", but miss!\n");
...
/*    if (!victim->query_is_pk() && victim->query_player() ) {
	write("that player is protected by the gods!\n");
	return 1;
    }
*/
	...
	write(victim->query_name()+" is obviously much too wary for that attack.\n");
	if (thief->query_invis())
	    return 1;
	else
	...
	if (check->query_stab_it() >= (thief->query_level()+dex)) {
	    write(capitalize(victim->query_name())+" would notice your attempt.\n");
	    return 1;
	}
    }
...
int backstab_it(object victim) {
	...
	if(dam > victim->query_hp()){
	    ...
	    say(environment()->query_name()+" plunges "+
	      environment()->query_possessive()+
	      " weapon deep into the innards of "+victim->query_name()+
	      ", instantly killing "+victim->query_objective()+".\n");
	//    thief->add_exp(victim->hit_player(dam,7,13));
// Used heal_self so that monster doesnt get a hit before it dies.
  //        thief->add_exp(dam);
/* Monster is dead in one backstab, so 9999999999 should cover any
   monster killable with a backstab.  misc_hit is the only funct
   that allows instakills, but it is somewhat random allowing the
   possibility of an instakill surviving, thus the large number */
/* Extra computations because misc_hit() doesnt award XP for kills */
	    experience=victim->query_exp();
	    experience=(experience/5)*4;
	    victim->misc_hit(9999999999);
      	    levs=(thief->query_level()+thief->query_legend_level());
	    divide=40*levs;
	    thief->add_exp(experience*levs/divide);
      	    return 1;
	}
...
int ambush(string arg) {
    ...
    else if (!(victim = present(arg, environment(thief))) || !living(victim)) {
	write("Ambush whom?\n");
	return 1;
    }
    ...
    /* Don't allow ambushing in no_fight areas: Colossus */
    if(ENV(thief)->query_property("no_fight")){
	write("You are not allowed to do that in this sanctuary.\n");
	return 1;
    }
...
int contest(){
  ...
  if(contest){
  (find_object("/players/blade/guild/contest/contest_room"))->contest_status();
  return 1;
}
...
/*    if ( !(objp->allowed_attack(this_player())) ) {
	write( "You are not allowed to steal from "+capitalize(whom)+" !\n");
	return( 1 );
    } */
...
void time_guild_soul()
{
    ...
    if(no_throw&&(xdam!=0)){
      ...
      write("You hit "+xatt->query_name()+" with the "+
	    thrown_weapon->query_name()+"!\n");
      say(thief->query_name()+" hit "+xatt->query_name()+" with a "+
	  thrown_weapon->query_name()+"\n",thief);
      xatt->hit_player(xdam,6);
      thief->add_exp(xdam);
      no_throw=0;
    ...
    if(no_throw&&(xdam==0)){
      move_object(thrown_weapon,ENV(thief));
