...
int _buy(string str) {
    ...
    if(str=="broadsword" && TP->query_money() > 2499) {
        obj=clone_object("/players/kelly/obj/broadsword");
        MOVE(obj,TP);
        TP->add_money(-2500);
   ...
   if(str=="dagger" && TP->query_money()>149) {
       obj=clone_object("/players/kelly/obj/dagger");
       MOVE(obj,TP);
       TP->add_money(-150);
       write("The smith says: Here you are, hope you'll be satisfied!\n");
       return 1;
  ...
  if(str=="knife" && TP->query_money()>20) {
      obj=clone_object("/players/kelly/obj/knife");
      MOVE(obj,TP);
      TP->add_money(-19);
      write("The smith says: Here you are, hope you'll be satisfied!\n");
      return 1;
...
void reset(int arg) {
    ...
    dest_dir=({
               "players/kelly/rooms/mythwood/faun_mark2","southwest",
               "players/kelly/rooms/mythwood/faun_shop","west",
             });
    items=({
            "sign","It's written in Faunen letters, but you can ask for a list",
          });
    MOVE(clone_object("players/kelly/monster/faunsmith"),TO);
}
