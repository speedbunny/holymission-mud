...
void reset(int arg) {
  ...
  dest_dir = ({
    "/players/kelly/rooms/vill/wardrobe", "north",
  });

  hidden_dir = ({
    "/players/kelly/rooms/vill/police_office", "down",
  });
}
...
status _slide(string str) {
  switch(str) {
    ...
    case "on pole": {
      ...
      tell_room("players/kelly/rooms/vill/police_office",
                TP->query_name() + " slides down the pole.\n");
      move_object(TP, "players/kelly/rooms/vill/police_office");
      TP->look();
      return 1;
    }
    default: {
...
status _enter(string str) {
  ...
  TP->move_player("into the wardrobe#players/kelly/rooms/vill/wardrobe");
  return 1;
}
