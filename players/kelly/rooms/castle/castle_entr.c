...
reset(arg) {
  ...
  dest_dir=({
    "players/kelly/rooms/mythwood/track2","south",
   });
   items=({
    "gate","It's an iron gate, but you can see no hole for a key a something like that",
   });
   move_object(clone_object("players/kelly/monster/eye"),this_object());
}
...
open() {
   ...
   this_player()->move_player("north#players/kelly/rooms/castle/castle_hall");	
   return 1;
}
