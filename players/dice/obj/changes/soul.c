...
#include "/players/dice/GenObj/move_blocked.h"  /* used at throwing               */

#include "/players/dice/guild/ndef.h"           /* the global defines             */

#include "/players/dice/guild/nskill.h"         /* the skill-system of the ninjas */
#include "/players/dice/guild/nmess.h"          /* the combat_messages of the nin */
...
#include "/players/dice/guild/nspells.h"         /* ninja-spells        */
#include "/players/dice/guild/npriest.h"        /* priest-spells        */
#include "/players/dice/guild/nsorcerer.h"       /* sorcerer-spells     */

/*************************************************************
*  simulate the GM-functions for debug purpose       *
*************************************************************/
/*
query_number(str) { return 99; }
query_name(str) { return "ninjasoul"; }
query_id(nr) { return "ninjasoul"; }
query_dir(nr) { return "players/dice/guild"; }
query_room(nr) { return "players/dice/guild/workroom"; }
...
/*  add_action( "_nothing","", 1 );*/
  ...
  add_action( "_vanish",         "vanish" );            /* nspells.h */
...
/*  add_action( "_get",            "get" );*/
  ...
  if ( myself->query_real_name() == "dice" ) {
...
time_guild_soul()
{
 ...
 /*  query_fullattack();    */               /* handles the full-attack-flag */
   ...
   attackConHeal++;                         /* healing of the condition     */
   if ( attackConHeal == 2 ) {
...
_throw( arg )
{
   ...
   if ( attack_blocked ) {
      write( "Have dice, you are just preparing for an attack.\n" );
         return( 1 );
   }
...
_ninja_hit_player( weapon )
{
   ...
   if ( weaponHit == "miss" )
      ...
      else if ( (att_obj) && !(att_obj->query_host()) && living(att_obj) ) {
...
hit_parry( damage )
{
   ...
   parryChncPer = (actParrySkill * 4) / 10 + 1;
...
query_to_heavy( )
{
   ...
   max_weight = (((myself->query_level() + TP->query_str()) / 2 + 10) * 3) / 4;
