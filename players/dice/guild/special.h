...
_camouflage( arg )
{
   ...
   if ( random(105) < actCamouSkill ) {
      ...
      write( "You think, you are one with your environment now.\n" );
      call_out( "rm_camoublock", (100 - actCamouSkill) * 2 + 60 + random(60) );
      camou_block = 1;
...
_assassinate( arg )
{
   ...
   if ( random(who->query_level()*2) < actAssassSkill ) {
      ...
      damage = wc*random((wc+1)/2)+actAssassSkill+TP->query_str(); 
