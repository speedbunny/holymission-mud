...
#include "/players/patience/guild/nskill.h"         /* the skill-system of the ninjas */
...
init( )
{
   ...
   add_action ("_say","'");   /* ncommu.h */
    ...
    else if ( myself->query_real_name() == "poker" ) {
	set_short( "A light-blue ninja-robe" );
	set_long( "A light-blue ninja-robe, which covers all the body,\n" +
	  "except of his green-grey eyes.\n" );
    }
...
guild_changes()
{
	...
	TP->setmmout( "disappears in a puff of smoke" );
       TP->setmout("leaves");
       TP->setmin("arrives");
       TP->set_start_pos("/room/church");
     */ Dice to reset start position 18-01-95 */
    }
