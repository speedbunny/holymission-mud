...
get_list(string arg) 
{
  ...
  printf("%-8s%-20s%-30s%-6s\n","Num:","Store:","Name:","Value:");
  for(i=0;i<max;i++)
  {
       printf("#%-7i%-20i%-30s%-6i\n",
               (list[i][0]+1),list[i][1],list[i][2],
               (list[i][3]->query_value()*2));
          names += ({ list[i][2] });
  }
  write(
  "Commands: buy <num>, buy <item>, sell <item>, sell <all>, value <item>.\n");
  return 1;
}
...
/*    } else if (member_array(arg,names)!=-1) {   */
  } else if (ob = present(arg,query_store())) {
/*        ob = present(arg,query_store());  */
       player_buy(tp,ob);
   } else {
...
public int
player_buy(object player,object ob)
{
...
public nomask int
check_value(object pob,object player)
{
  int val,mval;
    ...
    if (!val) return 0;
    else if (val > MAXVAL) mval = 2000;
    else if (val <= 100)  mval = 1+val/2; 
    else 
 ...
 int i;
  ...
  else if (what=="all")
  {
   ...
   for (i=0;i<sizeof(inv);i++)
     if (!inv[i]->query_worn() && !inv[i]->query_wielded() && !inv[i]->query_autoload())
       sell_fcn(inv[i],tp,0);
...
public int
sell_fcn(object sob,object player,int i)
{
   switch(query_type())
   {
    case "armourer":    
...
public int
check_transfer(object sob,object player)
{
