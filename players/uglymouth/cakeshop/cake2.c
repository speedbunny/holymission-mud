...
reset(arg) {
   ...
   if(!arg) {
      ...
      set_alias("cake");
      set_short("A three storey high wedding cake");
...
string *elist;
   ...
   if(parts > 8) {
      ...
      this_player()->restore_hit_points(25);
      this_player()->eat_food(5);
      parts--;
      return 1;
      }
...
set_groom(str) {
   groom=str;
   set_long("A three storey high wedding cake from Candy's confectioner's shop. \n"
          + "Specially created for the most happy marriage of " + cap(bride) + " and " + cap(groom) + ".\n"
          + "It's three storeys high and so detailed that you have to look \n"
