...
reset(arg) {
   ...
   if(!arg) {
      ...
      set_alias("cake");
      set_short("A five storey high wedding cake");
...
int storeynr;
   ...
   if(storeynr==4) {
      if(parts < 25) {
         write(slist[clist[7]*2-1]);
         return 1;
         }
      write(slist[clist[7]*2]);
      return 1;
      }
   if(storeynr==3) {
      if(parts < 19) {
         write(slist[clist[5]*2-1]);
         return 1;
         }
      write(slist[clist[5]*2]);
      return 1;
      }
   if(storeynr==2) {
      if(parts < 11) {
         write(slist[clist[3]*2-1]);
         return 1;
         }
      write(slist[clist[3]*2]);
      return 1;
      }
   if(storeynr==1) {
...
string *elist;
   ...
   elist = ({ "dummy",
      ...
      "A dragon says: Happy marriage, " +cap(bride)+ " and " +cap(groom)+ ", and flies away. \n",
      "One of the wheels turns into a tulip, turning around the centre. \n",
      "The images of " +cap(bride)+ " and " +cap(groom)+ " appear in the ship as it sails to heaven. \n",
      });

   if(parts > 24) {
      write(elist[clist[7]] + elist[clist[8]+4]);
      say(cap(TPQRN)+" makes a happy wish for the newly wedded couple \n"
         + "and eats a part of the fourth storey of the cake. \n",TP);
   ...
   if(parts > 18) {
      write(elist[clist[5]] + elist[clist[6]+4]);
      say(cap(TPQRN)+" says: HURRAY! and eats a part of the third storey of the cake. \n",TP);
      this_player()->heal_self(5);
      this_player()->restore_spell_points(25);
      this_player()->eat_food(5);
      parts--;
   ...
   if(parts > 10) {
      write(elist[clist[3]] + elist[clist[4]+4]);
      say(cap(TPQRN) + " congratulates the happy couple as " + TP->query_pronoun() + " eats a part of the second storey. \n",TP);
      this_player()->heal_self(5);
      this_player()->restore_spell_points(25);
      this_player()->eat_food(5);
      parts--;
...
string *dlist,*mlist;
   ...
   if(storeynr==1) storeyparts = 10;
   if(storeynr==2) storeyparts = 8;
   if(storeynr==3) storeyparts = 6;
   if(storeynr==4) storeyparts = 4;
...
set_groom(str) {
   groom=str;
   set_long("A five storey high wedding cake from Candy's confectioner's shop. \n"
          + "Specially created for the most happy marriage of " + cap(bride) + " and " + cap(groom) + ".\n"
          + "It's five storeys high and so detailed that you have to look \n"
