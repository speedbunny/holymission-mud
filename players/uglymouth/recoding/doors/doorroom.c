 ...
 * 151097: Sauron: Slightly altered the format of how the door descs are
 *                 added to the long_desc.
 * 081197: Sauron: Altered 'other_side' check for reset msgs (lines 43-47).
 * 980109, Uglymouth: fixed move fail msgs to print only for players, not monsters.
 */
#include <doorroom.h>
...
// 081197: Sauron: Need to make sure that the room object on the other side
//                 of the door is loaded as well as set before using
//                 tell_room().
        tell_room( this_object(), door_data[ doors[i] ][ "reset_msg" ] );
        if( door_data[ doors[i] ][ "other_side" ] &&
            find_object( door_data[ doors[i] ][ "other_side" ] ) )
...
void set_door( string door_id ) {
  ...
  if( !original_long_desc ) {
    original_long_desc = long_desc;
    if( original_long_desc[<1..<1] == "\n" )
      original_long_desc = original_long_desc[0..<2];
    original_long_desc += " ";
  }
}
