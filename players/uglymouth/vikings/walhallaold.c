...
object odin,thor,loki,mjol,ob;
   ...
   if(!odin->query_hero()) {
      ...
      while(ob) {
         if(ob->query_name() == "ruby" || ob->query_name() == "emerald"
            || ob->query_name() == "sapphire") destruct ob;
   ...
   items=({"gods","The dreaded gods of the vikings: Odin, Thor and Loki",
           ...
           "stew","It reminds you of your youth. Didn't you just love to do other things \n"
                + "with it than eat it?",
           ...
           "knives","Although they're huge, they're sharp as can be", });
...
object target,odin;
   ...
   if(!str) return 0;
   if(str=="food" || str=="stew") {
      write("Throw food at whom? \n");
      return 1;
      }
   if(sscanf(str,"food at %s",godname)!=1 &&
      sscanf(str,"stew at %s",godname)!=1) {
      return 0;
      }
   if(godname!="odin" && godname!="thor" && godname!="loki") {
   ...
   busy=1;
   write("You throw some divine food at " + capitalize(godname) + ". The God looks very displeased. \n");
   target=present(godname);
   target->irritated(level);
   if(godname=="loki") {
      level++;
