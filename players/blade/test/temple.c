...
"A black altar lies in the center of the room.\n";
        
    dest_dir=({
      "/players/blade/portal_room.c","west", 
      "/players/blade/test/contest_room.c","east"
    });
    items=({
...
int sacrifice(string str_arg){
  ...
  if(!str_arg){
    ...
    if(sac_item=present(str_arg,this_player())){
      say(this_player()->query_name()+" sacrifices "+str_arg+" upon the altar.\n",this_player());
...
int kneel_altar(){
  ...
  tell_object(this_player(),"You kneel before the altar in reverence of Mask.\n");
}
  return 1;
}
