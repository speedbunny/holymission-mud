...
int throw(string arg){
  ...
  if(thrown_weapon->query_weight()>2){
    write("This is too unwieldly to throw.\n");
    return 1;
  }
  if(victim==thief){
  ...
  if(random(hit)>random(40)){
    ...
    xdam=(thief->query_str()+dex+(2*thrown_weapon->weapon_class())+
...
int steal(string arg)
{
    ...
    // ----- ONLY ALLOW STEALING FROM PLAYERS WHO ARE ABLE TO ATTACK YO!
    // ----- (pat, 220295)
