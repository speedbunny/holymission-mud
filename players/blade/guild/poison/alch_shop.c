...
reset(arg)
{
    ...
    if (!present("alchemist"))
      move_object(clone_object("/players/blade/guild/poison/alchemist"),
		  this_object());
    if (arg)
	return;
    set_light(1);
    ...
    dest_dir = ({
      "room/eastroad3", "west",
      "/players/blade/guild/poison/lab","up"
    });
    Store_room = "/players/blade/guild/poison/store";
}
...
do_sell(ob)
{
	...
	return notify_fail("This store only deals in items of alchemic value.\n"); 
