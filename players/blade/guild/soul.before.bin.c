...
int palm(string arg){
  ...
  if(item=present(arg,environment(thief))){
    if(!living(item)){
      success=transfer(item,this_player());
      if(!success&&random(dex)<random(10)){
        success=-1;
      }
      switch(success){
      case -1:{
...
int query_thief_banish(string arg){
  object player;
  if(player=present(arg,thief)&&player->query_guild()==1){
...
int test(string arg) {
  ...
  if (!(victim = present(arg, environment(thief)))
      || !living(victim)) {
    write("Kill who?\n");
    return 1;
  }
  
  victim->misc_hit((victim->query_hp()),7);
  victim->misc_hit(1,7);
  return 1;
}
/*============================== Appraise ==================================*/
...
/*      thief->add_exp(victim->query_exp() / 
                     (180-((140*thief->query_quest_points())
                           /"/room/quest_room"->query_total_weight())));
*/
      if(victim->query_player())
        log_file("KILLS.PLAYER",thief->query_real_name()+" ("+
                 thief->query_level()+") killed "+
                 victim->query_real_name()+" ("+
                 victim->query_level()+") with "+
                 (thief->query_wielded())->query_name()+".\n");
      thief->add_exp(victim->query_level()*victim->query_hp());
