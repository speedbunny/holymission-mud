...
int my_tattoo(string str) {
  string who,what;
  object ob;

  if(!str || sscanf(str,"%s %s",who,what)!=2) {
    notify_fail("Tattoo what on whom?\n");
    return 0;
  }
  else
    ob=find_player(who);
  if(!ob) {
    notify_fail(CAP(str)+" is not playing at this time.\n");
    return 0;
  }
  else {
    what=CAP(what);
    who=CAP(who);
    WRITE("You reach across the mud and tattoo '"+what+"' on "+who
         +"'"+(who[<1..<1]=="s"?"":"s")+" butt!\n");
    TELLO(ob,NAME(TP)+" reaches across the mud and tattoos '"+what
                    +"' on your butt!\n");
    return 1;
  }
}
...
void extra_look() {
  WRITE("\nBefore you stands a man, or one in man's shape, but greater than"
...
void init() {
  ...
  if(RNAME(TP)=="sauron") {
    ...
    add_action("my_tattoo","tattoo");
    add_action("my_tattoo","tt");
