...
reset(arg){
    ...
    dest_dir=({GPATH+"open14","north",
      GPATH+"open12","east",
      GPATH+"open18","west",
    });
    items=({"crag","A pillar of rock jetting out of the plain",
...
int leave_religion(){
    ...
    TP->set_con(TP->query_con()-(TP->query_con()-2));
    return 1;
}
_sacrifice(arg){
    if(!arg) return "Sacrifice what?";
    if(arg=="iron" || arg=="iron bar"){
        if(present("iron bar",TP)){
	    write("You sense Crom is pleased.  Perhaps you should pray.\n");
	    MO(CO(GPATH+"sac_ok"),TP);
        destruct(present("iron bar",TP));
	    return 1;
	}
	else {
