...
{ if (arg) return;
  set_name("malaria");
  set_short("malaria");
  set_long("malaria, please trompf it :)\n");
  set_alias("disease");
}
...
{ object *env;
  ...
  { env = all_inventory(environment(environment()));
    ...
    { if(living(env[i]) && !present("mal_27",env[i])) 
      { move_object(clone_object("/players/beardy/disease/malaria.c"),env[i]);
        destruct(this_object());
        return 1;
      }
    }
...
query_auto_load()
{
  return "/players/beardy/obj/mal:";
}

trompf(str)
{ if(str=="malaria")
  { shout(this_player()->query_name()+" junked the deadly disease !\n");
   destruct(this_object());
   return 1;
  }
  return 1;
