// 3.13.95 Kasgaroth: strict_types and closures 
...
void show_stats() {
  ...
  write("  This is Padrone's Walking Castle, version " +
           WALKER_VERSION + ".\n");
...
int indoors_room(object the_room) {
  string room_desc, junk1, junk2;

//    room_desc = apply(call, the_room, "short");
  the_room->xxx();
  room_desc = (string)the_room->short();
  if (!room_desc)
 ...
 */
int move_castle(string direction, int auto_moving) {
 ...
 */
  apply(call,CASTLE_MOVER, "come_here");
 ...
 */
  apply(call,CASTLE_MOVER, "init_command", direction);

  new_room_object = apply(call,CASTLE_MOVER, "query_environment");
...
/* The castle is auto-walking, i. e. the pilot is alive on the bridge */
    if (   (new_room_object != latest_room_object)
        && (   (!room_room(new_room_object)
        && room_room(old_room_object))
        || (file_name(new_room_object)
                 == "players/padrone/outside/finish_quest"))) {
/* Auto-walking to a place we cannot auto-walk to (and not the latest room) */
      on_the_move = 0;
/* Because of her catch_tell()! */
      apply(call,CASTLE_MOVER, "come_here");
      return 1;
    }
...
/* Still in the same room. We couldn't move that way. */
    tell_room(old_room_object, "The castle tries to move " +
              direction + " but fails.\n");
    tell_room(BRIDGE, "The castle tries to move " + direction +
                      " but fails.\n");
    tell_room(ROPE_LADDER, "The castle tries to move " + direction +
                           " but fails.\n");
    if (drawbridge_down)
...
/* And just to be on the very safe side: */
  ...
  if (airborne)
    ...
    tell_room(old_room_object, "The castle walks " + direction + ".\n");
/*
    tell_room(old_room_object, "The castle takes a giant step and leaves "+
                               direction + ".\n");
*/
  ...
  if (airborne) {    
    ...
    tell_room(BRIDGE, "The castle takes a giant step " + direction +
              " and arrives in " + apply(call,new_room_object,"short") +
              ".\n");
  ...
  if (drawbridge_down) {
    if (airborne)
      ...
      tell_room(GATE_ROOM, "The castle takes a giant step " +
                           direction + "!\n");
  }
 ...
 */
  if (!airborne) {
    ...
    while (obj && !players_in_room) {
      if (living(obj) && !apply(call,obj,"query_npc"))
        players_in_room = 1;
      obj = next_inventory(obj);
    }
    if (!players_in_room)
      return 1;        /* Noone to see it */
    if (!apply(call,BRIDGE, "pilot_is_alive"))
      return 1;        /* Castle driven by players */
    ...
    while (obj) {
      ...
      this_obj = obj;
      obj = next_inventory(obj);
      if (   !random(30) && living(this_obj) 
           && apply(call,this_obj,"query_npc") 
           && file_name(this_obj) != CASTLE_MOVER) {
        monstername = apply(call,this_obj,"query_name");
/* So the messages are not repeated to the bridge. */
        on_the_move = 1;
        ...
        tell_room(new_room_object, "It steps on the poor " + monstername +
                                   "! How horrible!\n");
        on_the_move = 0;
        apply(call,this_obj,"hit_player",1000);
        return 1;
      }
    }
  }
...
string query_name() { return "castle"; }
status query_drawbridge_down() { return drawbridge_down; }
status query_airborne() { return airborne; }
object query_environment() { return environment(this_object()); }

object who_drove_it_here() { return latest_driver; }

/* set_drawbridge_down */
void set_drawbridge_down(status f) {
/* So the messages are not repeated to the bridge. */
  on_the_move = 1;
...
void set_airborne(status f) {
/* So the messages are not repeated to the bridge. */

  on_the_move = 1;
  if (f && !airborne) {
    tell_room(environment(this_object()),
              "Suddenly, the walking castle takes off and soars " +
              "up into the sky!\n" +
    ...
    tell_room(environment(this_object()),
              "Suddenly, a flying castle lands! You are amazed!\n" +
              "This is the famous walking castle, that can also fly " +
              "through the skies.\n");
    tell_room(BRIDGE,
              "The eagle has landed. Now you can see the ground " +
              "outside the window.\n");
    tell_room(ROPE_LADDER, "Now you can see the ground below you.\n");
    if (drawbridge_down)
      tell_room(GATE_ROOM, "Now you can see the ground outside " +
                           "the open gates.\n");
  }
  airborne = f;
  on_the_move = 0;
...
void error_check() {
  ...
  apply(call,CASTLE_RESTARTER, "restart_castle");
}

void callable_castle_reset() 
{   /* callable_castle_reset */
    move_object(this_object(), DEST);
    apply(call,CASTLE_MOVER, "come_here");
    set_drawbridge_down(0);
    set_airborne(0);
    apply(call,BRIDGE, "reset", 1);
    error_check();
} 
...
int enter(string str) {
  ...
  apply(call,this_player(),"move_player",({ "into castle", GATE_ROOM }));
  return 1;
}

int climb(string str) {

  if (!id(str) && str != "ladder"
               && str != "rope ladder" 
               && str != "rope"
               && str != "up")
    return 0;
  ...
  apply(call, this_player(), "move_player",
              ({"up on the rope ladder",ROPE_LADDER}));
  return 1;
}

int tell_bridge(string str) {
...
int query_show_short() {  return 1; }

string short() {
  if (airborne)
    return "A castle flies high up in the sky";
  else
    return "A castle stands here on its giant legs";
}
...
int long() {
  ...
  write("This looks like an ordinary castle with walls, " +
        "towers and a drawbridge,\n" +
        "except for the fact that it's standing on a pair of giant legs.\n" +
        "You reason that this must be the famous walking castle.\n");
  if (drawbridge_down)
  ...
  if (this_player() && apply(call,this_player(),"query_wizard")) {
    write("---\n");
    show_stats();
  }
}
