...
/* Changed by Ethereal Cashimor to use room/room.c, 230293 */
/* 170694: Nokey: Strict typed it */
#pragma strict_types
inherit "room/room";
#define COST 1000
...
object weapon, armour, advertisement;
  ...
  ::reset(arg);
  if(!arg) {
    dest_dir=({"players/padrone/inside/courtyard","east"});
    short_desc="Antonio's Personal Protection Service";
    long_desc="You are in the office of Antonio's Personal Protection\n"
            + "Service. In this room you can hire a bodyguard (use 'hire').\n";
    set_light(1);
    smell="The place is cool and pleasant. Almost too pleasant.";
    items=({"office","Everything is made of oak"});
  }
  if(!antonio) {
...
object make_bodyguard(object client) {
...
int hire() {
    ...
    else if ((string)this_player()->query_name() == "Someone")
