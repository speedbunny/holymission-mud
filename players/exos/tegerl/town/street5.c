

/* Coded by Tegerl, 8.3.1995 */
/* rooms/town/street5 */


inherit "room/room";
void reset(int arg) 
{
::reset(arg);

if (arg) return;

set_light(1);
short_desc="A road in town";

long_desc=
    "The road leads further north to the tournament place. " +
    "To the northeast you can see a castle and to the east " +
    "you can hear noise of people working.\n";
  
dest_dir=({
    "players/tegerl/jabberwocky/rooms/town/street6", "north",
    "players/tegerl/jabberwocky/rooms/town/manu", "east",
    "players/tegerl/jabberwocky/rooms/town/street4", "south",
  });

items=({
    "entrance","The entrance is heavily guarded. There is one tower left and right of the door",
    "door","The big door is made of wood, but it has metal fittings",
    "people","There are people, and they all seem to be very busy",
    "houses","Along the roads there are many houses, some are shops",
});

smell = "The air could be fresh and clean, if that town wouldn't be there. So you can only smell waste and dump.";
  
  }

