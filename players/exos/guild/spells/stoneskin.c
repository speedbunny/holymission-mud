#define STONESKIN_COST 150
#define STONESKIN_FAIL 95
#define STONESKIN_FAIL_COST 75

int main( string str ) {
    object target, skiner;
    int level, time;
    ...
    if( this_player()->query_spell_points() < STONESKIN_COST ) {
	...
	write( "Cast Stoneskin at whom?\n" );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return ( 1 );
    }

    /* Is our target already effected? */
    if( target->stoneskin_shad() ) {
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < STONESKIN_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - STONESKIN_FAIL_COST );
	return ( 1 );
    }
    ...
    skiner = clone_object( "/players/exos/mage/shadows/stoneskin_shad" );
    skiner->start_shadow( target, time );

    /* Are we a wizard?  If so, keep our SP and tell us the duration. */
    if( !this_player()->query_immortal() ) 
	this_player()->restore_spell_points( - STONESKIN_COST );
    else
