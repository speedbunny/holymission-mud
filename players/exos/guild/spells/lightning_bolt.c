...
int main( string str ) {
    ...
    if( this_player()->query_spell_points() < BOLT_COST ) {
	...
	write( "Cast Lightning Bolt at whom?\n" );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return ( 1 );
    }
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < BOLT_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - BOLT_FAIL_COST );
	return ( 1 );
    }
    ...
    for( i = 0; i < level; i++ ) 
	damage += random( 30 ) + 1;
    printf( "You point at %s.  A bolt of lightning arcs from your finger towards %s.\n",
      target->query_name(), target->query_objective() );
    tell_object( target, this_player()->query_name() + " points at you.\n"+
      "Suddenly a bolt of lightning arcs at you!\n" );
    say( this_player()->query_name() + " points at " + target->query_name() +
      ".\nSuddenly a bolt of lightning arcs at " + target->query_objective() + "!\n" );

    /* Do we kill our target? */
    if( damage >= target->query_hp() ) {
	printf( "Your lightning bolt burns a gaping hole through %s!\n", target->query_name() );
	say( "The lightning bolt burns a gaping hole through " + target->query_name() + "!\n", target );
	tell_object( target, "You are electrified as everything begins to grow dark!\n" );
    }
	...
	this_player()->restore_spell_points( - BOLT_COST );
    else
