#define SCRY_COST 100
#define SCRY_FAIL 130
#define SCRY_FAIL_COST 50

int main( string str ) {
    object target, scryer;
    int level, time;
    ...
    if( this_player()->query_spell_points() < SCRY_COST ) {
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < SCRY_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - SCRY_FAIL_COST );
	return ( 1 );
    }
    ...
    scryer = clone_object( "/players/exos/mage/shadows/scry_shad" );
    scryer->start_shadow( target, time );

    /* Are we a wizard?  If so, keep our SP and tell us the duration. */
    if( !this_player()->query_immortal() ) 
	this_player()->restore_spell_points( - SCRY_COST );
    else
