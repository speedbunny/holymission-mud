#define HASTE_COST 120
#define HASTE_FAIL 60
#define HASTE_FAIL_COST 60

int main( string str ) {
    object target, haster;
    int level, time;
    ...
    if( this_player()->query_spell_points() < HASTE_COST ) {
	...
	write( "Cast Haste at whom?\n" );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return ( 1 );
    }

    /* Is our target already effected? */
    if( target->haste_shad() ) {
	printf( "%s is already hasted!\n", target->query_name() );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return( 1 );
    }
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < HASTE_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - HASTE_FAIL_COST );
	return ( 1 );
    }
    ...
    haster = clone_object( "/players/exos/mage/shadows/haste_shad" );
    haster->start_shadow( target, time );

    /* Are we a wizard?  If so, keep our SP and tell us the duration. */
    if( !this_player()->query_immortal() ) 
	this_player()->restore_spell_points( - HASTE_COST );
    else
