#define DETECT_COST 30
#define DETECT_FAIL 15
#define DETECT_FAIL_COST 15

int main( string str ) {
    object target, detecter;
    int level, time;
    ...
    if( this_player()->query_spell_points() < DETECT_COST ) {
	...
	write( "Cast Detect Invisible at whom?\n" );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return ( 1 );
    }

    /* Is our target already effected? */
    if( target->detect_shad() ) {
	printf( "%s is already enchanted!\n", target->query_name() );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return( 1 );
    }
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < DETECT_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - DETECT_FAIL_COST );
	return ( 1 );
    }
    ...
    if( level > 30 )
	level = 30;
    time = 60 + level * this_player()->query_wis() * 2;
    ...
    detecter = clone_object( "/players/exos/mage/shadows/detect_shad" );
    detecter->start_shadow( target, time );

    /* Are we a wizard?  If so, keep our SP and tell us the duration. */
    if( !this_player()->query_immortal() ) 
	this_player()->restore_spell_points( - DETECT_COST );
    else
