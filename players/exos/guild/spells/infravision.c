#define INFRAVISION_COST 80
#define INFRAVISION_FAIL 80
#define INFRAVISION_FAIL_COST 40

int main( string str ) {
    object target, infravisioner;
    int level, time;
    ...
    if( this_player()->query_spell_points() < INFRAVISION_COST ) {
	...
	write( "Cast Infravision at whom?\n" );
	say( this_player()->query_name() + " stops casting.\n", this_player() );
	return ( 1 );
    }

    /* Is our target already effected? */
    if( target->infravision_shad() ) {
    ...
    if( this_player()->query_level() * 2 + this_player()->query_int() * 2 +
      this_player()->query_wis() + random( 40 ) < INFRAVISION_FAIL ) {
	write( "Your spell fizzles!\n" );
	say( this_player()->query_name() + "'s spell fizzles!\n", this_player() );
	this_player()->restore_spell_points( - INFRAVISION_FAIL_COST );
	return ( 1 );
    }
    ...
    infravisioner = clone_object( "/players/exos/mage/shadows/infravision_shad" );
    infravisioner->start_shadow( target, time );

    /* Are we a wizard?  If so, keep our SP and tell us the duration. */
    if( !this_player()->query_immortal() ) 
	this_player()->restore_spell_points( - INFRAVISION_COST );
    else
