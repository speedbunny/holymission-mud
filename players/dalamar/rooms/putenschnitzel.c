...
void reset(int arg) {
  if(arg)
    return;
  set_id(COLOURS[random(5)] + " Putenschnitzel");
  set_short("A " + name);
  set_alias("Putenschnitzel");
  set_value(10);
  set_weight(0);
...
int throw(string str) {
  ...
  write("You throw the Putenschnitzel to " + capitalize(str) + ".\n");
  transfer(this_object(), environment(ob));
  shout((string)this_player()->query_name()
	+ " sends a Putenschnitzel across the sky to "
	+ (string)ob->query_name() + "!\n");
  tell_object(ob, "You leap up and catch the Putenschnitzel with your hands.\n");
  say((string)ob->query_name() + " leaps up and catches the Putenschnitzel with "
