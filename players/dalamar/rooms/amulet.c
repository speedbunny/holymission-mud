...
id(str) {
  if(TEST(desc_array[DID]))
    return str == "amulet" || str == desc_array[DID];
  else
    return str == "amulet";
}
...
reset(arg) {
  ...
  desc_array[DSHORT] = "A very old amulet";
  desc_array[DLONG]  = "An aura of magic surrounds it.\n";
  desc_array[DID]    = "amulet";
}
...
init() {
  if(this_player()->query_real_name() != "muzmuz")
  if(!environment() ||
     this_player() != environment(this_object()) ||
     LEVEL(this_player()) < 35 || this_player()->query_real_name() != "muzmuz")
    return;
  ...
  tell_object(this_player(),"Master ! You cloned me ! Thank you so much !\n");
  finit();
}
...
static toggle_info(str) {
    if(str) {
      notify_fail("The amulet's _info_ command, must be used with no parameters.\n");
      return 0;
    }
    info = info;
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
amulet_help() {
  command("more /players/silas/tools/staff_help.c", fob);
  return 1;
}
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
...
/* not be destructed by the amulet. It puzzled me for some */
...
clean2(ob) {
  ...
  while(ob) {
    ...
    else if(CALL(ob,"id","amulet")) check = 1;
...
reset_room(str) {
    ...
    say(capitalize(fgender)+" amulet begins to glow brightly.\n");
    write("You reset :\n   "); write_ob_and_desc(ob);
    return 1;
}
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
/*            Make the amulet into a light source                */
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
set_lys(str) {
  int n;
  string what;
  ...
  if(sscanf(str, "%d", n) == 1) {
     set_light(n);
     if(n > 0) {
       say(NAME(fob) + "'s amulet glows brightly !!\n");
       write("Your amulet lights up this dark place > :-)\n");
     }
     else
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
/*                      Search routines                         */
/*           These functions are the core of the amulet         */
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
search_next(str, obj) {
  int nr;
