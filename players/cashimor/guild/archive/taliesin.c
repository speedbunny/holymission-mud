...
object voorwerp;
  ...
  if(!arg) {
    ...
    if(!present("harp",this_object())) {
      ...
      voorwerp=clone_object("players/cashimor/instruments/harp");
      voorwerp->set_creator("taliesin");
      move_object(voorwerp,this_object());
      voorwerp=clone_object("players/cashimor/guild/tinyharp");
      move_object(voorwerp,this_object());
      notebook=clone_object("players/cashimor/guild/notebook");
      move_object(notebook,this_object());
      init_command("wear harp");
      voorwerp=clone_object("players/cashimor/objects/xp");
      move_object(voorwerp,this_object());
    }
    dirs=({"smile","order beer","drink beer","w","s","e","e","n","sell bottle","s","e",
    "n","n","n","n","n","w","buy 3","wink","e","s","s","s","s","s","e","n",
    "climb fence","n","e","s","buy skiwater","giggle",
    "n","w","s","climb fence","s","w","w","w","w","n","e"});
  }
}
...
int i,nowalk;
  ...
  call_out("movement",30);
}

ok_move() {
  place=place+1;
  if(place==sizeof(dirs)) {
    place=0;
  }
  init_command(dirs[place]);
}
...
string lphrase,bits,split,verb;
  ...
  for(i=0; i<sizeof(split); i=i+1) {
    ...
    if(!number) number="players/cashimor/guild/room"->numberer(split[i]);
  }
...
do_it(commando,argument) {
  ...
  call_other("players/cashimor/guild/room",commando,argument);
  return;
}
