...
/* 270194 Galadriel: it didn't load, fixed a comment */
...
#define KBL "/players/cashimor/guild/"
#define KBLI "/players/cashimor/instruments/"
#define GNR	6
...
int i;
  ...
  filenaam=KBL+"songroom"->query_song(str);
  if(!filenaam) {
...
string recept;
  ...
  if(!str) return 0;
  ok=KBL+"room"->query_instruments(str);
  if(ok==0) return 0;
  recept=KBL+"room"->query_needed(ok);
