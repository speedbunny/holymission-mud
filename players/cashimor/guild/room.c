...
workroom() {
  ...
  {
    notify_fail("You are not high enough level to enter this way!\n");
    return 1;
  }
  this_player()->move_player("workroom#players/kbl/workroom");
  return 1;
}
...
do_join() {
  ...
  log_file("kbl.guild","*** "+this_player()->query_real_name()
         + " joined! ***\n");
  return;
}
