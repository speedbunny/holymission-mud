...
/* 061094: Made is possible to easily remove the skill-system again */
...
int tmp;
  ...
  switch(tmp) {
    ...
    case SS_ABSOLUTE_FAILURE:
      write("You hurt yourself, but the wheel doesn't move.\n");
      say(lw(this_player()->query_name()
           + " fails painfully at turning the wheel.\n"));
      this_player()->hit_player(1);
      return 1;
    case SS_FAILURE:
...
check_elevator(verbose) {
  ...
  if("players/cashimor/rooms/elevator"->query_level()==level) {
    if(verbose) tell_room(environment(this_object()),lw("A platform arrives "
                        + "on this floor in the elevator shaft.\n"));
    return 1;
  }
  return 0;
}
