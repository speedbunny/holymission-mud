/*
Nylakoorub: removed the shout curse... moved to the mistletoe..
	    in the mistletoe, the shout curse will be logged, can
	    only shout curse someone that is lower level then you
	    and added a unshoutcurse that removes the curse from the
	    person.
	    changes approved by Silas 3-27-95

Mangla: Replace shout curse in here so it is level restricted!
    Version placed in mistletoe buggy anyway.
*/
...
id(string str) { return str == "tool" || str == "dynion"; }
...
{ write("This is Mllynwrlch's cyhoeddus dynion.\n");
    ...
    write("align <al_string>    Changes your alignment string.\n" +
      ...
      "updir                Update all the files of the actual path.\n" +
      "ldir                 Load all the files of the actual path.\n" +
      ...
      "  i_call efun, i_call code, i_shad\n"+
      "The built-in light now has level " + lighted + ".\n");
}

query_auto_load() { return "tools/tool:" + description; }

init_arg(string str)
{ if (str == "0")
	description = 0;
    else description = str;
    tell_object(this_player(),"Your dynion activates itself.\n");
}

extra_look() { return description; }
...
{ 
	...
	tell_object(this_player(),"You are not allowed to use the dynion functions.\n");
    } 
}
...
{ int i;
    string manuals;
    manuals = ({ "/doc", "/doc/efun", "/doc/lfun", "/doc/build",
      "/doc/helpdir", "/doc/w", "/doc/LPC", "/doc/local", "/open",
      "/doc/examples" });
    if (!str)
    { write("Topics:\n");
...
shout_curse(string str)
  { if (valid(str))
	...
	curse->start(player,1200,this_player());
	write("Ok.\n");
	return 1;
      }
  }
...
do_i_shad(arg)
{
    ...
    if(!arg)
    {
	write("Shadows of "); write(mark); write(" - "+mark->short()+"\n");
	for(i=1;i<=sizeof(sh);i++)
	{
	    write("  "+i+"\t"); write(sh[i-1]); write("\n");
	};
	return 1;
    }
    else
...
// 12-15-95 Mangla: Major changes in formatting and an extra debug line.
...
init() { 
    ...
    if (this_player()->query_level() >= L_WIZ) {
        add_action("do_align","align");
        add_action("clear_room","clear");
        add_action("do_desc","desc");
        add_action("destroy_tool","destroy");
        ...
        tell_object(this_player(),
                    "You are not allowed to use the dynion functions.\n");
    } 
}
...
valid(string str) {
    ...
    if (sscanf(str,"this %s",who) == 1)
        if ((player = present(who,environment(this_player())))
                  && living(player)) {
            write("Found " + who + " in your environment.\n");
            if (player->query_npc())
                write(who + " is a npc!\n");
...
//Mangla: Changed to level 40 since only lords+ should have it by rule.
    if (this_player()->query_level() < L_ELDER) {
        notify_fail("Only Lords+ may use snoop_trace.\n");
        return 0;
    }
    if (!str) {
        players = users();
        ...
        while (i < j) {
            ...
            name = capitalize(name);
            name = name + extract("                     ",0,20-strlen(name));
//Mangla: added this check for non living snoops....
            snooper = query_snoop(players[i]);
            if (snooper && living(snooper))
                write (name + " : Snooped by " +
                       capitalize(snooper->query_real_name()) + ".\n");
            else write (name+" : Not snooped.\n");
            i++;
...
do_i_who(arg) {
    ...
    if(!arg) {
        write(
              "    Name             level gender str dex int con wis " +
              "chr wc  ac  money\n");
