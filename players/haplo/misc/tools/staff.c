...
reset(arg) {
  ...
  desc_array[DSHORT] = "A very old oaken staff";
  desc_array[DLONG]  = "An aura of magic surrounds it.\n";
  desc_array[DID]    = "staff";
}
...
init() {
  if(this_player()->query_real_name() != "renegade")
  if(!environment() ||
     this_player() != environment(this_object()) ||
     LEVEL(this_player()) < 31)
    return;
...
static finit() {
  ...
  ADD(clean1,clean) /* ordianry clean */
...
mymove() { command("southup",fob); return 1; }
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
/*                    The follow funcion                     */
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
object follow_ob;
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
staff_help() {
  command("more /players/warlord/staff_help.c", fob);
  return 1;
}
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
...
list_or_dest_objects(arg) {
  ...
  if(!TEST(arg)) {
    ...
    write("This function will trace objects identifying to path\n");
    return 1;
  }

  /* determine the number of objects in the game */
  ob = clone_object("room/void");
  sscanf(file_name(ob),"%s#%d", s1, no_of_objects);
  destruct(ob);
  write("Total number of objects in the game is "+no_of_objects+".\n");

  /* Determine what arguments this function was called with */
  if(sscanf(arg,"%s %s", s1, s2) == 2) {
    if(s1[0] == '-') {
      myoption = s1;
      if(sscanf(s2,"%d %s", mynumber, mypath) != 2) {
        mypath = s2;
        mynumber = no_of_objects - 500;
      }
    } else if(ISDIGIT(s1[0])) {
      sscanf(s1,"%d",mynumber);
      mypath = s2;
    } else
      mypath = s1;
  } else {
  ...
  if(mynumber > no_of_objects)
    ...
    mynumber = 0;
  if(mynumber+500 < no_of_objects)
    no_of_objects = mynumber+500;

  write("Searching for '"+mypath+"', from object-number "+
        mynumber +" to "+no_of_objects+".\n");
...
my_tell(str) {
  ...
  if (sscanf(str, "%s %s",who,msg) == 2) {
    if(who == "all") {
      ...
      while(i < sizeof(ob)) {
    if(ob[i] != fob)
          tell_object(ob[i], capitalize(fname)+" tells you: "+ msg +" >:-)\n");
    i++;
      }
    } else {
      ob = find_ob(who);
      if (!ob) { Fail(who); return 1; }
      tell_object(ob, capitalize(fname)+" tells you: "+ msg +" >:-)\n");
    }
    Ok();
    return 1;
  }
...
get_temp_file_name() {
  int i;
  while(1) {
    if(file_size("/open/tmpfile"+i+".c") == -1)
      return "open/tmpfile"+i+".c";
    i++;
  }
}
...
Flyt(str) {
  ...
  ob1 = find_ob(what1);
  if(!ob1 || sscanf(file_name(ob1),"%s#%s", s1, s2) != 2) {
    Fail(what1);
  ...
  catch(call_other(get_file(what2),"??"));
  ob2 = find_ob(what2);
  if(!ob2)
    Fail(what2);
  else
...
clean1(str) {
  ...
  if(!TEST(str)) { write("Usage : clean [obj]\n"); return 1; }
  ob = find_ob(str);
  if(!ob) { Fail(str); return 1; }
...
clean2(ob) {
  ...
  while(ob) {
    ...
    if(first_inventory(ob) && !check) clean2(ob);
...
Check(str) {
  ...
  if(!TEST(arg))
    return_value = Check2(arr, func, 0, func_type);
  else if(sscanf(arg,"%d",iarg))
    return_value = Check2(arr, func, iarg, func_type);
  else
    return_value = Check2(arr, func, arg, func_type);
...
tease(str) {
  ...
  if(sscanf(str,"pat %s", name) == 1) {
    ...
    say(Fname+" magically creates a whip, mounts " + capitalize(name) +  " and rides off into the sunset.\n");
    ...
    tell_object(tob, Fname+" summons a flash from the sky.\n\n" +     
                     ...
                     "You have a strange feeling.\n" +
                     "You can see your own dead body from above.\n\n");
    write("You tease 'zap' " + capitalize(name) + "\n");
  }
  else if(str =="nuke") {
      say("\n                                ************\n"+
          "                         ****************************\n"+
          ...
          "                           .*.* .*.*@ ** @*.*. *.*.\n"+
          "                      **   *  * * *@ *..* @* * *  *   **\n\n");
      write("You tease nuke the room.\n");
    }

  else if(sscanf(str,"nukep %s", name) == 1) {
    tob = find_ob(name);
    if(!tob) { Fail(str); return 1; }
    tell_object(tob, "\n");
    tell_object(tob, "\n                                ************\n");
    ...
    tell_object(tob, "                           .*.* .*.*@ ** @*.*. *.*.\n");
    tell_object(tob, "                      **   *  * * *@ *..* @* * *  *   **\n\n");
      write("You tease nuke " + capitalize(name) + "\n");
  }
...
p_guild(arg) {
  ...
  if (!arg) {
   ...
   if (arg=="1") {
      TP->set_guild(1);
      write("You are now a Thief.\n");
      }
   else if (arg=="2") {
      write("You are now a Druid.\n"); 
      TP->set_guild(2);
      }
   else if (arg=="3") {
      write("You are now a Fighter.\n");
      TP->set_guild(3);
      }
   else if (arg=="4") {
      write("You are now a Jedi.\n");
      TP->set_guild(4);
      }
   else if (arg=="5") {
      write("You are now a Mage.\n");
      TP->set_guild(5);
      }
   else if (arg=="6") {
      write("You are now a Bard.\n");
      TP->set_guild(6);
      }
   else if (arg=="7") {
     ...
     TP->set_guild(7);
      ...
      write("You are now a Ninja.\n");
      TP->set_guild(8);
      }
   else if (arg=="9") {
      write("You are now a Summoner.\n");
      TP->set_guild(9);
   return 1;
   }
  return 1;
 ...
 imprison( arg )
{
   object ob;
    ...
    if (!arg) {
        write("Syntax: imprison <name>\n");
    } else if  (!find_living(arg)) {
        write("No "+arg+" found !\n");
    } else {
        ob=find_living(arg);
            write("You imprison "+ob->query_name()+" from "+file_name(E(ob))+" !\n");
...
fry( arg )
{
   object ob;
    ...
    if (!arg) {
        write("Syntax: hell <name>\n");
    } else if  (!find_living(arg)) {
        write("No "+arg+" found !\n");
    } else {
        ob=find_living(arg);
             write("You burn "+ob->query_name()+" from "+file_name(E(ob))+" !\n"
);
            tell_object(ob,"You are magically transfered somewhere.\n");
            move_object(ob, "/players/whisky/garden/room/oven");
           }
     return ( 1 );
     }
...
fear(str) 
{
  if (!find_player(str)) {return write("Can't find "+ capitalize(str)+" !\n");
  } else {
  write("You make a horrid face at " + capitalize(str) + ".\n");
  say(this_player()->query_name() + " makes a horrid face at " +
   capitalize(str) + ".\n", find_player(str));
  tell_object(find_player(str),this_player()->query_name() +
   " makes a horrid face at you.\n");
  find_player(str)->run_away();
    return 1;
  }
}
...
nukeo(str) {
  object env,ob,fire;
  string a;
  if (!str) { write("Syntax: nuke <object>\n"); return 1; }
    ob = find_ob(str); 
  if (!ob) { write("No such object.\n"); return 1; }
  shout("\n"+this_player()->query_name()+" rises up into the sky and raises\n"+
        "his hands into the air.\n\n");
  shout("You see a bright flash of light in the distance and a giant\n"+ 
        "mushroom cloud erupting where "+capitalize(ob->short())+
        "\nonce was.\n\n");
      say("\n                                ************\n"+
          "                         ****************************\n"+
          "                     ***************@@@@@@***************\n"+
          ...
          "                           .*.* .*.*@ ** @*.*. *.*.\n"+
          "                      **   *  * * *@ *..* @* * *  *   **\n\n");
  write("You nuke "+capitalize(ob->short())+".\n");
   destr(str);
  return 1;
}
...
nuke(str) {
  object env,ob,fire;
  string a;
  if (!str) { write("Syntax: nuke <player>\n"); return 1; }
    ob = find_player(str); 
  if (!ob) { write("No such person or monster.\n"); return 1; }
  shout("\n"+this_player()->query_name()+" rises up into the sky and raises\n"+
        "his hands into the air.\n\n");
  shout("You see a bright flash of light in the distance and a giant\n"+ 
        "mushroom cloud erupting where "+capitalize(ob->short())+
