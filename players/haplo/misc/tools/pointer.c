...
{
  if (hide) return;
  if (lighted) return "pointer (lighted)";
  else
   return "pointer";
 }
...
void long() 
{
   write("This is the wiztool for wizes under level 33.\n"+
         "Just type 'help pointer' to get its features.\n");
   if (hide)
...
int help(string str) 
{
  ...
  write( 
  "COMMANDS APPRENTICE:\n\n"+
  "plight [num] : lights your tool, man or man [filename] : helpdir, \n"+
  "pinv [name] : shows the items of name \n"+
  "pitem [object] in [name] : gives you info about an item in a living,\n"+
  "pmail : clones you a mailbox and gives you all commands like in the post,\n"+
  "pmore or pmore [objectname] : filereading of room or present object,\n"+
  "ploc [name] : location of a living, pdebug : your errorlogfile,\n"+
  "plerr : reading the last lined in the lpmud.log, pwho : better than who,\n"+
  "pinto [num] : setting your intoxination, phide : hide your tool,\n"+
  "pshow : make your tool visible, pnet or pnet [player] : the ip number\n\n"+ 
  "COMMANDS NEWWIZ:\n\n"+
...
int pinto(string arg) 
{
    ...
    if (!arg) 
    {
       notify_fail("Syntax: pintro <num>     Current intoxication="+INTO+"\n");
     return 0;
     }
     if (sscanf(arg, "%d", i) == 1) 
     {
