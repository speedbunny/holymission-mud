...
#define MAGE_BIN	"/players/saffrin/guild/bin/"
#define MAGE_HELP	"/players/saffrin/guild/helpdir/"
...
varargs int reveal_me(int time) {
    object mage,env;
    ...
    if(!mage)
       ...
       call_out("reveal_me",time,0);
       return 0;
    }
    else {
// Sauron: Make sure that multiple call_outs don't exist.
       while(remove_call_out("reveal_me")!=-1);
       mage->set_vis();
       tell_object(mage,"You feel exposed!\n");
       if(E(mage))
           tell_room(E(mage),mage->query_name()+" appears.\n",mage);
       return 1;
    }
}
...
int _vis() {
// Sauron: Make sure that multiple call_outs don't exist.
    while(remove_call_out("_vis")!=-1);
    TP->set_vis();
    write("You feel exposed!\n");
    tell_room(environment(TP),TP->query_name() + " appears.\n",TP);
    return 1;
}

int _invis() {
    return (MAGE_BIN+"invisible")->main();
    write("You turn yourself invisible.\n");
    tell_room(environment(TP),TP->query_name() + " vanishes!\n",TP);
    TP->set_invis();
    call_out("_vis",TP->query_level() * 20,0);
    return 1;
}
...
// convert the mage_practice_sessions
    ...
    add_action("mage_line","mage/");
    add_action("_invis","invisibility");
/*
    add_action("_vis","visible");
*/
...
int _filter(string str) {
    ...
    if(file_size(MAGE_BIN+verb+".c")>0)
        if((MAGE_BIN+verb)->main(str))
            return 1;
...
_cast(str){
  ...
  if(sscanf(str, "%s %s", what, where)==2){
    if(what=="'teleport'") {
      write("SPELL DOWN FOR RECODE, SORRY\n"); return 1;}
    else return 0;
  }
  return 0;
}
...
help(arg) {
    ...
    if(arg == "levels") {
      GM->list_levels(5,TP->query_gender());
      return 1;
    }
    if(arg == "legendlevels") {
      GM->list_legend_levels(5,TP->query_gender());
      return 1;
    }
    if(strstr(arg," ")==-1 && cat(MAGE_HELP+arg))
        return 1;
    else
...
recognize_fight() {
    if(E() && E()->query_invis() && !E()->query_immortal())
        reveal_me();
}
