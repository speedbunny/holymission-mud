...
{
  if (petname && friend)
     write("You see "+CAP(petname)+", "+friend->query_real_name()+"'s dragon.\n");

  else 
 write("You see a powerful golden dragon who serves Tatsuo the samurai SHIHAN!\n");
...
short()
{
   if (objectp(friend) && environment() == friend)
      return "Dragon ["+friend->query_name()+"'s pet]";
   return "Dragon";
}
...
int get()
{
  ...
  else if (!objectp(friend))
  {
     ...
     tell_object(tp,"Your dragon shoots a plume of flame into the air.\n");
     set_heart_beat(0);
     return 1;
  } else 
...
heart_beat()
{
  ...
  if (objectp(friend) && env()!=friend && !stay)
  {
      ...
      if (env()!=env(friend))
      {
          tell_room(env(friend),"Dragon flies in.\n");
          SAY("Dragon flies out after his master.\n");
...
init()
{
  ...
  add_action("fcn_command","C");  /* commanding the dragon */
...
int fcn_task(string arg)
{
  string where, what;
    ...
    {
        SAY(tp->query_name()+" gives his dragon an order\n");
       SAY("Dragon looks bewildered.\n");
        return 1;
    }
    ...
    {
       SAY("Dragon flies after Tatsuo.\n");
       stay = 1;
     tell_room(places[where],"Dragon flies in.\n");
   ...
   else if (objectp(friend->query_attack()) && vic!=this_object() && !stay)
   {
     if (env(friend) == env())
     {
       vic = friend->query_attack();
        write("Your dragon flies screaming into the fight to rescue you !\n");
       say(friend->query_name()+"' dragon flies screaming into the fight !\n");
       attack_object(vic);
...
int fcn_eat()
{
  ...
  {
   SAY("Dragon incinerates the corpse.\n");
   destruct(corpse);
...
int fcn_stay()
{
   ...
   if (!stay)
   {
     SAY("Dragon lies down and rests.\n");
     stay = 1;
	 return 1;
   }
   SAY("Dragon stands up.\n");
   stay = 0;
   return 1;
}
...
int fcn_kill(string whom)
{
  ...
  mob = present(whom,env());
    ...
    write("Your dragon looks confused.\n");
    return 1;
  }
 ...
 if (mob && living(mob) && this_player()==friend)
 {
	...
	{
           friend->catch_tell("Your dragon dislikes your attempt.\n");
	   return 1; 
    }

        SAY("A dragon flames "+mob->query_name()+".\n");
	attack_object(mob);
	change_attacker(mob);
 }
return 1;
...
{
 string h;
 if (sscanf(arg,"dragon %s",h)!=1)
     return 0;
 else if (tp!=friend) 
 {
    notify_fail("Only Tatsuo can control this dragon.\n");
   return 0;
 }  
   petname = h;
   set_alt_name(petname);
  write("Your dragon is now called "+petname+".\n");
  return 1;
}
...
int no_follower(object player)
{
  ...
  if (ofriend = present("dragon",player) && ofriend->query_follower()==tp)
      return 0;
...
{
   if ((arg!="dragon" && arg!=petname) || tp!=friend)
