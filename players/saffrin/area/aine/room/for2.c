...
init() {
  if(this_player()->query_player() && this_player()->query_level() < 8) {
    this_player()->move_player("west#players/saffrin/area/aine/room/for1");
    tell_object(this_player(), "\nYOU ARE WEAK!! RETURN WHEN YOU ARE STRONGER!!\n\n");
    this_player()->look();
    return;
  }
  ::init();
