...
reset(arg){
  ...
  long_desc = 
      ...
      " graves.  There is a iron gate leading out of the graveyard to the southeast.  "+
      "  In the distance, the wind howls loudly at the darkness around you.  "+
      "The path leads deeper into the graveyard towards the northwest.\n";
  dest_dir=({
      HAM+"room/g9", "northwest",
      HAM+"room/garden1", "out",
  });
  items=({
      "cobblestones", "Small gray stones making up a path",
      "tombstones", "Various shapes and sizes",
      "flowers", "Daisies",
      "gate", "A large iron gate that leads out of the graveyard",
      "stairs", "Small marble stairs",
      "darkness", "You cannot see the darkness silly",
      "path", "A path made up of cobblestone",
  });
  clone_list=({      
      1, 1, "ghost", HAM+"npc/hamghost", 0,
  });
  ::reset();
