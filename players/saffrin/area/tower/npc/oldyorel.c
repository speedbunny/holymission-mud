// 281097: Sauron: Removed invisibility, teleport and portal from the list
//                 of spells you can practice.
...
int advance_spell( object who, string sn, string fn, int max_sk )
{
    ...
    if ( who->query_guild() != 5 ) {
      ltell("says:","You have to enroll in the College of Mages,\n"+
        "            to be able to improve your knowledge here.\n",
        this_object());
      lsay( "says:","You have to enroll in the College of Mages,\n"+
        "            to be able to improve your knowledge here.\n",
...
reset(arg) {
    ...
    if(!arg) {
      set_alt_name("master");
      set_name("Mage-Master Yorel");
      set_alias("yorel");
      set_short("Mage-Master Yorel the guild advisor");
      set_living_name("yorel");
...
catch_tell(arg) {
    ...
    if(sscanf(arg,"%s tells you: %s\n",who,what)!=2) {
      if(sscanf(arg,"%s says: %s\n",who,what)!=2) 
          ...
          if(!who) return;
          dummy=explode(what," ");
          if(member_array("yorel",dummy)!=-1 || member_array("master",dummy)!=-1) {
