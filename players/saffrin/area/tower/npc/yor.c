...
#define ROOM "players/saffrin/guild/quest/"
...
int advance_spell( object who, string sn, string fn, int max_sk )
{
    ...
    case -2:
	ltell( who, "says:",
"You have to go out and earn experience before coming here again!\n",
	  this_object() );
	break;
    default:
...
reset(arg) {
    ...
    {
	p = clone_object("players/saffrin/guild/obj/mportal");
	move_object(p,"room/church");
	p->set_portal("players/saffrin/guild/rooms/oldstart");
    }
    /*
	if (sizeof(explode(file_name(this_object()),"#"))==2 &&
	  !present("portal",find_object("room/church")))
...
{
    move_player("X#players/saffrin/guild/rooms/oldstart");
}
...
catch_tell(arg) {
	...
	in=1;
	who->move_player("X#players/saffrin/guild/quest/c1");
	break;
    default:
	dummy=explode(what+" "," ");
	if(member_array("quest",dummy)!=-1 ||member_array("help",dummy)!=-1) {
...
_notify_destruct(ob)
{
    object ad,en;
    query_ghost();
    ad=clone_object("/players/saffrin/guild/monsters/advisor");
