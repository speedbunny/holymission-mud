...
int cd14() {
  TELLR(ENV(TP),"The Tiny Demon frowns, then folds itself into nothingness.\n");
  if(demon) DE(demon);
}
...
int cd12() {
  ...
  say(lw("Tiny Demon hugs "+CRNAME(TP)+", in a way only a tiny paper Tiny Demon"
        +" could.\n"));
  call_out("cd13",2);
}
...
int card_open(string str) {
  ...
  say(lw("A cute little baby demon explodes from "+CRNAME(TP)+"'s card as "
     +HE(TP)+" opens it!\n"));
