...
reset(arg){
	...
	items= ({ "water", "Sparkling blue water",
		  ...
		  "fountain", "A small marble fountain spews fourth "+
		  "clean, blue water, ready for drinking. There is "+
		  "something written on the side of the fountain."
...
int _read(string arg){
	...
	if(arg=="foutain"){
		write("It reads 'wounds'. You wonder what that means!\n");
		return 1;
		}
	}
_drink(string arg){
	...
	if(arg=="water"){
		if(done== 20){
		write("You go to take a drink, but there is no more water!"+
		" Someone must have finished it all!\n");
		return 1;
		}
		write("You take a slurp of refreshing cool water from the "+
		"fountain! AHHHHHH!\n");
		call_other(TP(), "heal_self", 5);
		done= done + 1;
		return 1;
		}
	}
