...
/* THIS WAS CODED BY PATIENCE AND UPDATED FOR USE BY THE NEW NINJA */
/* GUILD BY AKIRA. SO GIVE THE CREDIT TO PATIENCE!                 */
...
poison_next_room()
{
	...
	if ( !cloud ) {
            cloud = clone_object( "/players/akira/mobs/poison_cloud" );
...
attack()
{
	...
	break;
        case 16..25: return(  poison_cloud() );
...
critical_hit()
{
	...
	/*        }  */
        toWhere = "/players/akira/area/temp";
...
block( arg )
{
    ...
    else if (query_verb() == "cast")
    {
	this_player()->restore_spell_points(-(this_player()->query_sp()/2));
	write("You try to cast a spell at the great Ryu but he sucks yoru energy away.\n");
	say(this_player()->query_name()+" tries to cast a spell but is countered by the great Ryu.\n",this_player());
	return 1;
    }
}
...
move_object(clone_object("/players/akira/weapons/ninja"), this_player());
