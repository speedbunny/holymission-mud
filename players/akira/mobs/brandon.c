...
set_long("This is Sifu Brandon, Master of Preying Mantis Kung-Fu. His "+
	  ...
	  "attack. He is here now to train with the ninjas and learn "+
	  "there ways. You don't think it would be smart to attack him.\n");
	set_al(100);
	set_ac(40);
	set_hp(2000);
	set_level(50);
...
block (arg) {
    if(query_verb()== "feet") {
	if(arg== "Brandon");
	write("He uses a mantic Block to parry your kick!\n");
    ...
    if(query_verb()== "punch"){
	if(arg=="brandon");
	write("Your punch is caught in a Mantic Grab. Brandon Laughs at you!\n");
