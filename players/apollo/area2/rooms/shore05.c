...
reset(arg){        
  ...
  dest_dir = ({
        "players/apollo/area2/rooms/shore01", "southeast",
        "players/apollo/area2/rooms/shore10", "northwest"
           }); 
  ...
  items = ({
	...
	"lake", "You see a lake covered in a dense fog that prevents you \n" +
		"from seeing very far to your north, east, and northeast. \n" +
  ...
  swim(str) {
    if (!str || str=="south" || str=="west" || str=="southeast" ||
	...
	this_player()->move_player("north#/players/apollo/area2/rooms/lake11");
      }	 
    if (str=="east") {
	write("You start to swim eastward out into the lake. \n");
	write("You can see much clearer now that you are in the lake. \n");
	this_player()->move_player("east#/players/apollo/area2/rooms/lake06");
      }
    if (str=="northeast") {
	write("You start to swim northeastward out into the lake. \n");
	write("You can see much clearer now that you are in the lake. \n");
	this_player()->move_player("northeast#/players/apollo/area2/rooms/lake12");
      }
    return 1;
  }
