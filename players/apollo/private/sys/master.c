...
int i;
  ...
  gd_info = ({
    ...
    ({ "thief", 	"kasgaroth", 	30,	0,	40,	10,	2,"T"}),
    ({ "druid", 	"sourcer",	30,	0,	42,	8,	2,"D"}),
    ...
    ({ "ninja",		"dice",		30,	0,	40,	10,	2,"N"}),
    ({ "summoner",	"matt",		30,	0,	42,	8,	2,"S"}),
    ({ "vagabond", 	"colossus",	30,	1,	42,	8,	2,"V"}),
    ({ "barbarian",     "tatsuo",       30,     1,      42,     8,      2,"*"}),
    ({ "samurai",       "haplo",        30,     1,      42,     8,      2,"X"}),
...
query_defines(gd) {
  ...
  case 0 : /* Adventurer Guild, Gender is supported */
     /* 011195 Tatsuo: Changed adventuers to 30 levels */
 ...
 ({ "Volunteer",   "Simple Gardner"		})		 /* 1 */
  ...
  case 8:		/* Ninja guild, Dice, gender support */
...
({ ({0,0,0}),
  ...
  case 11:              /* Barbarian guild, Tatsuo, gender support */
...
case 12:    /* Samurai Guild, Coded by Exos, Tatsuo, Haplo, Gender supported */

    return ({

       ({
        ({"Apprentice Wizard-Samurai",
	  "Apprentice Wizard-Samurai",
	  "Apprentice Wizard-Samurai"}),
	  ({0, 0, 0}) }), /*30*/
	({
