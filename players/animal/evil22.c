...
/* Done by Herp: instead of obj/monster */
     lionman = clone_object("players/animal/lionman");
if (!chat_str) {
  chat_str = allocate(4);
  ...
  a_chat_str[3] = "The prince growls: DIE SCUM!\n";
     ...
     lionman->set_whimpy(-1); /* Sort wimpy bug - Moonchild */
     lionman->load_chat(35,chat_str);
     lionman->load_a_chat(40,a_chat_str); /* Bugged */
...
object pl;
 ...
 say(pl->query_name()+".\n\n",pl);
 tell_object(pl,"you.\n\n");
 write_file("/players/animal/quest",pl->query_name()+" threw head on "+ctime(time())+".\n");
 destruct(ob); /* Herp: Don't worry ... it's only the King's head */
 g =g + 1;
 call_out("attack",4);
 return 1;
...
attack() {
         object ob;
         ob = find_player(ishere);
         lionman->attack_object(ob);
}
