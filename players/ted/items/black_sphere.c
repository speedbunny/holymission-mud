...
free_monster() {
 ...
 if(captured_monster == this_player()) {
	write("You are imprisoned inside a sphere. You can not escape!\n");
	return 1;
  }
 move_object(captured_monster, environment(this_player()));
 ...
 if(call_other(captured_monster,"query_npc"))
	call_other(captured_monster, "attack_object", this_player());
 empty = 1;
 monster_name = "";
 captured_monster = 0;
 return 1;
