...
read(str) {
   ...
   store = find_object("players/ted/party/party_store");
...
register_party(str) {
   ...
   party = clone_object("players/ted/party/party");
   if (str && str != "") party->set_title(str);
   move_object(party, "players/ted/party/party_store");
   while (ob) {
      if (living(ob) && ob->query_level() < 30) {
         party->set_player(ob);
         symbol = clone_object("players/ted/party/party_symbol");
