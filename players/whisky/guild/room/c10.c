...
status drink(string arg) {
  ...
  write("You try to drink from the barrel, but it is empty.\n");
  return 1;
}
...
***/
  ...
  if(!present("barrel",this_object())) {
    ...
    barrel->set_long(lw("A wooden barrel, normally used to be filled \
with fresh mountainwater for thirsty adventurers.\n"));
    barrel->set_weight(60); // can be taken but wouldn't be too clever
