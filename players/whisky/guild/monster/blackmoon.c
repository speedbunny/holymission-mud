...
void reset(int flag)
{
     ...
     if (flag == 0)
     {
       ...
       set_spell_mess1("Nun Blackmoon makes a combat move\n");
       set_spell_mess2("You are hit by Blackmoon's feet in your face !\n");
       set_chance(80);
...
int do_practice(string arg)
{
   int cost, sklev;
    ...
    else if (_tp->query_level() < 20)
    {
        write("Blackmoon tells you:\n"+
        "I only teach Monks above level 20.\n");
        return 1;
    }
    ...
    else if (!(sklev=PARTNER->query_skill(_tp,"monk_"+arg)) )
    {
     ...
     else if (_tp->query_money() < (cost=PARTNER->query_cost(_tp,"monk_"+arg)))
     {
      ...
      else if (sklev > (MAXPR-10) && !_tp->query_legend_level())
      {
          write("Blackmoon tells you:\n"+
                      "Sorry, but you have to be a Legend, that I teach you "+
                      "further.\n");
          return 1;
      }
      else if (sklev > MAXPR)
      ...
      else 
      {
         if (chk && sklev > random(MAXPR + _tp->query_int() + _tp->query_dex()))
         {
             ...
             say("Nun Blackmoon shows something to "+_tp->query_name()+"\n",_tp);
             _tp->add_money(-cost);
         ...
         if (PARTNER->raise_skill(_tp,"monk_"+arg,1))
         {
             write("You practice your "+arg+" skill with Blackmoon for "+
                   to_string(cost)+" gold coins.\n");
...
varargs int hold(object ob,int time)
{
   ...
   /*  To many kills use shouts! distracting and useless for non quest mobs!
