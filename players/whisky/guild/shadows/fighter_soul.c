...
init() {
  object gem;
  object shad;  /* here the shadow */

  if (environment()!=this_player())
      return;

  /* here the player is shadowed */
  if (this_player()->fighter_shad()!=1)
  {
      shad = clone_object("/players/whisky/guild/shadows/fighter_shad");
      shad->start_shadow(this_player(),0,"fighter_shad");
  }
  ...
  /* here to change the combat_style */ 
  add_action("fcn_mood","mood");
  add_action("emote","fighter/");
  add_action("guild_command","",1);
}

/* here setting the fighting style */
fcn_mood(arg)
{
    /* aggressive, moderate of defensive */
