...
int main(string str) {
  string size,stat,victim,ret,cn,mode_str,loc;
  string religion,prot_msg,tmp;
  int ac,got,lg,xp,sw,imm;
  object att_ob,tp;
  ...
  money: %d\n  experience: %d\n\
  combat: %d%%",
capitalize(GM->query_name(tp->query_guild())),religion,
...
sw/2,"  health:",
sw/2,"  "+tp->query_hp()+"["+tp->query_max_hp()+"]",
sw/2,"  power:",
sw/2,"  "+tp->query_sp()+"["+tp->query_max_sp()+"]",
sw/2,"  vitals:",sw/2,"  "+stat,
sw/2,"  protection:",sw/2,"  "+prot_msg,
sw/2,"  mood:",sw/2,"  "+mode_str,
sw/2,"  wimpy:",sw/2,"  "+tp->query_wimpy()+"% ("+tp->query_wimpydir()+")",
