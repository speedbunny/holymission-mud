...
static object pl;
reset(arg) { counter=0; i=0; }

id(str) { return str=="tree"; }
short() { return "An olive tree"; }
long() { write("A big olive tree.\n"); }
...
get_olive(str) {
  ...
  if (str=="olive") {
  if (present("olive",TP)) {
   write("You try to pick another olive, but there is no one reachable.\n");
   say(TPN+" tries to pick an olive but fails !\n");
  return 1;
  }
   ob=clone_object("players/whisky/meadow/obj/olive");
...
int i;
  ...
  if (str=="tree") {
   ...
   return 1;
    ...
    for (i=0;i<2;i++)
       move_object(clone_object("players/whisky/meadow/obj/olive"),
