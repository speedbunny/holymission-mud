...
reset(arg) {
  if (!golem) {
     golem=clone_object(MONST+"golem");
  ...
  long() {
  write("You are standing in a ravine. There are big rocks to\n"+
...
query_rope() { return rope; }
  ...
  jump() {
   if (present("mring",TP) && (TP->query_weight()< 2+TPST/4)) {
      ...
      TP->move_player("into the pit#players/whisky/quest/room/ravine8");
      return 1;
    }
  ...
  north() {
   if (present("golem")) {
       write("The stone golem blocks you the way !\n");
   return 1;
   }
   TP->move_player("north#players/whisky/quest/room/ravine10");
   return 1;
  }
