/*
  This should be a nw kind of illuminator
 */
...
reset(arg) {
    if (arg)
	return;
  amount_of_fuel = 6000; name = "lantern"; is_lighted = 0; weight = 1;
}
...
short() {
    if (is_lighted)
return "A little Lantern (lighted)";
return "A little Lantern";
}
...
light(str) {
	...
	return 0;
    if (is_lighted) {
write("You have your Lantern already lighted.\n");
	return 1;
    }
    is_lighted = 1;
...
extinguish(str) {
    ...
    is_lighted = 0;
    if (set_light(-1) == 0) {
write("Your Lantern suddenly turns dark.\n");
	say(call_other(this_player(), "query_name") +
