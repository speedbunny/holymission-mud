...
reset(arg) {
  ...
  if (!arg) {
   ...
   "is only waiting to rape you in her hot witchbed !\n");
    ...
    set_aggressive(1);
   move_object(clone_object("/players/whisky/garden/obj/whip"),this_object());
    command("wield whip");
    command("wear straps");
  }
}
...
hold(ob, time)
{
 ...
 monster_died(ob) {
   ...
   broom=clone_object("players/whisky/garden/obj/broom");
   write("As you have killed the witch you feel suddenly\n"+
         "the strong force to pick up all her items as fast\n"+
         "as you can\n\n");
    move_object(clone_object("players/whisky/garden/obj/key2"),this_player());
/* Matt 7-30-03: Removed crystal ball */
    move_object(clone_object("players/whisky/garden/obj/robe2"),this_player());
