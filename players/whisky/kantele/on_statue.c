...
ONE_EXIT("players/kantele/behind_altar","down",
         ...
         "You are on the statue, you can climb down.\n",1)

get_diamond(arg) {
    if (eyes > 0 && arg =="of eye" || eyes > 0 && arg =="of eyes") {
       eyes = eyes-1;
       write("You pry of a diamond eye.\n");
say(call_other(this_player(), "query_name") + " pries of a diamond eye.\n");
...
move_object(shagath, "players/kantele/behind_altar");
}
write("You are shaken down from the statue !\n");
call_other(this_player(), "move_player", "X#players/kantele/behind_altar");
tell_room(find_object("players/kantele/behind_altar"),
	"The Whole Temple shakes You can feel the Wreath of Kali !\n");
tell_room(find_object("players/kantele/behind_altar"),
