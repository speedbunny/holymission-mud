...
/* 15-DEC-1993 Galadriel: restored original Moonchild values, 1 location */
/* 18-JAN-1994 Galadriel: guild dir secured, help moved to misc/guild */
/* 05.30.94 Airborne: Changed to new directories. */
...
void init() {
  ...
  if(!EQE("TATTOOPOS"))
    ESE("TATTOOPOS", "brow");
  if(!shadow) {
    shadow=clone_object("players/airborne/guild/shadow");
...
int prepare_combat(string str) {
  ...
  de_equip();
  move_object(comb_soul=clone_object("players/airborne/guild/combat_soul"), E);
  return 1;
}
...
*/
  E->heal_self((int)c->query_weight());
...
int send_chatline(string str) {
  ...
  if(!str) {
    ...
    for(i=0;i<sizeof(u);i++) {
      int lev;
      lev=(int)u[i]->QL;
      write(((string)u[i]->QRN=="airborne") ? "[GM] " : lev>29 ? "[Wz] "
	: lev>9 ? "[" + lev + "] " : "[" + lev + "]  ");
...
int guild_help(string str) {
  ...
  if(str=="guild") {
/* 18-JAN-1994 Galadriel: help file now in misc/guild, guild secured */
