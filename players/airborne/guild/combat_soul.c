...
#define TPD 10+(int)this_player()->query_dex()*5
#define TPS 10+(int)this_player()->query_str()*5
*/
#define TPD (int)this_player()->query_dex()
#define TPS (int)this_player()->query_str()
  ...
  if(cmds_done[att_no] >= data[att_no][0])
	...
	switch((int)ob->query_size()) {
	  case 1:
	    ...
	    break;
	  case 5:
	    dam=random((TPD+TPS)/2 - 10);
	    break;
          default:
	    dam=random((TPD+TPS)/2);
	    break;
	}
	break;
      case 2:
	...
	switch((int)ob->query_size()) {
	  case 1:
	    ...
	    break;
	  case 2:
	    dam=TPS+random(TPS*3);
	    break;
	  case 3:
	    dam=TPS+random(TPS*2);
	    break;
	  case 4:
	    dam=TPS+random(TPS);
	    break;
