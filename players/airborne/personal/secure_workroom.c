...
reset(arg){
 ...
 ::reset(arg);
  tell_room("/players/airborne/workroom", "There has been a reset.\n");
...
init(){
 ...
 stop_hacker();
 check();
 if(this_player()->query_invis() && RN !="airborne"){
...
int stop_that(string str){
 if((RN == "tuppence") && (str == "airborne")){
  write("Now Ms. Tuppence, you know better than to tickle Willie.\n"+
	"Please don't make him have to spank you... even if you would"+
	"enjoy that.\n");
  say("Airborne rubs his hands together as he warns Tuppence what a spanking\n"
      +"might feel like.\n");
  return 1;
 }
 if(str == "airborne"){
...
stop_hacker(){
  ...
  hacker = find_living("airborne");
  if(RN=="airborne"){
    if((query_ip_number()=="199.181.141.58") ||
      (query_ip_number()=="24.92.35.5")){
      write("You are the one.\n");
      return 1;
  }

    else {
    write("You are not the real Airborne.\nLogged!\nGoodbye!\n");
    log_file("HACKER", "Hacker: "+ctime()+" from: "+
	     query_ip_number( )+"\n");
// changed by airborne 3.5.99:  login in from my new location
//  destruct( hacker );
    return 1;
    }
