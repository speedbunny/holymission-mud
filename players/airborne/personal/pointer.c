...
/* Auto_load changed by Ethereal Cashimor.
   There are illegal functions in this item.
   240293 */
#define FILE     "/players/airborne/personal/pointer:"
...
/* Ac/Dc  160893 */
int var_shout(string str) {
  shout((string)str+"\n");
  write((string)capitalize(this_player()->query_real_name())+" echoed: "+ str+"\n");
  return 1;
}
pshow() {
...
help(str) {
  ...
  "var_shout <msg>  (vsh)  WIZ       # A variable shout. \n"+
 "\n");
  }
       ...
       ob = users();
        while (i < sizeof(ob))
          { name = (string)ob[i]->query_real_name();
            ...
            if (snooper) {
              if (snooper==TP || snooper->query_level()<=TPL) {
