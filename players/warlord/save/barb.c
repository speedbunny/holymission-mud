...
void init() {
	...
	" howls at the world as he returns to battle!\n");
...
devour_corpse() { return ( BIN + "devour_corpse.c" ) -> devour_corpse(); }
...
int send_chatline(string str) {
    ...
    u=users();
     ...
     if( query_verb() == "barb/" ) {
        CHAT( 11, "<= Barbarian => " + CAP( TP->RNAME ) + " " + str + ".\n");
	return 1; 
    }
	...
	if( !str ) {
		...
		int lev;
		int leg_lv;
		leg_lv=(int)u[i]->query_legend_level();
		lev = (int)u[i]->query_level();

	        write(
		((string)u[i]->QRN=="apollo") ? "[Guild Master]      " :
		((string)u[i]->QRN=="pretzel") ? "[Assistant GM]      " :
		lev==120 ? "[New Wizard]        " :
		lev==140 ? "[Wizard]            " :
		lev==160 ? "[Sage Wizard]       " :
		lev==180 ? "[Retired]           " :
	    ...
	    write("\n");
            write("=-=-=-=-=-=-=-=-=-= There are "+sizeof(u)+ " Barbarians " + 
		  "online =-=-=-=-=-=-=-=-=-=-=-=-=\n");
	    return 1;
	}
