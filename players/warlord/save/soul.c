...
void init() {
	...
	ESE("TATTOOPOS", "brow");
    if(!shadow) {
	shadow= clone_object("players/tatsuo/guild/shadow");
	shadow->start_the_shadow(E);
    }
    if(TP!=E)
	return;
    // here the function for the Legendlevels is called: whisky
...
int prepare_combat(string str) {
    ...
    de_equip();
    move_object(comb_soul=clone_object("players/tatsuo/guild/combat_soul"), E);
    return 1;
}
...
int devour_corpse() {
    ...
    /* 15-DEC-1993 Galadriel: Too strong...
      E->heal_self((int)c->query_weight()*6);
    */
    TP->heal_self((int)c->query_weight()*4);
...
// recoded whisky a) for legends 2.) bad old code : speedintensive
int send_chatline(string str) 
{
    ...
    if (query_verb() == "barb/")
    {
	apply(call,"/sys/chatd","do_chat",
	  apply(call,this_player(),"query_guild"),"Barbarian =>"+
	  capitalize(apply(call,this_player(),"query_real_name"))+
	  " "+str+"\n");
	return 1; 
    }
    else if (str!="legends")
	...
	if (!str)
	{
	    ...
	    u=sort_array(filter_array(users(), "gd_filter", this_object(), 0),
	      "level_sort", this_object());

	    for(i=0; i<sizeof(u); i++){
		int lev;
		int leg_lv;
		leg_lv= apply(call, u[i], "query_property", "barbarian_legend");
		lev=(int)u[i]->QL;
		write(((string)u[i]->QRN=="tatsuo" || (string)u[i]->QRN=="gambit") ? "[GM] " :
		  (string)u[i]->QRN=="khan" ? "[Cheiftan] " :
		  u[i]->query_property("barbarian_legend")>100 ? "[Titan ["+lev+"/"+leg_lv+"] " :
		  u[i]->query_property("barbarian_legend")>0 ? "[Warlord ["+lev+"/"+leg_lv+"] " :
		  lev>29 ? "[Immortal] " :
		  lev>9 ? "["+lev+"] " :
		  "["+lev+"] " );
	    ...
	    write("\n");
	    write("-=-=-=-=-=-=-=-=-=-= There are "+sizeof(u)+ " Barbarians online =-=-=-=-=-=-=-=-=-=-\n");
	    return 1;
	}
	"sys/chatd.c"->do_chat(11, "|=| Barbarian "+capitalize(TP->QRN)+" |=| "+str+"\n");
	return 1;
    }
    num = 0;
    msg = "\n";
    ...
    {
	if (apply(call,u[i],"query_guild")==11 &&
	  (lv = apply(call,u[i],"query_property","barbarian_legend")) > 0)
	{
    ...
    if (num == 1)
	write("\n                1 Barbarian Legend online\n\n");
    else
    {
	write("\n               "+to_string(num)+" Barbarian Legends online.\n"+
	  "\n");
    }
    write(msg);
    return 1;
...
/*==============================================
  ...
  if(!str) {
    ...
    for(i=0;i<sizeof(u);i++) {
      int lev;
      lev=(int)u[i]->QL;
    write(((string)u[i]->QRN=="tatsuo" || (string)u[i]->QRN=="gambit") ? "[GM] "    : (string)u[i]->QRN=="axlrose" ? "[Assistant GM] " : lev>29 ? "[Wizard] "
	: lev>9 ? "["+lev+"] " : "["+lev+"] ");
      write(u[i]->short() ? (string)u[i]->short() : "["
	+ (string)capitalize(u[i]->QRN) + "]");
...
int levelsort(object p1, object p2) {
    if((string)p1->QRN=="airborne") return 0;
    if((string)p2->QRN=="airborne") return 1;
...
int guild_help(string str) {
    ...
    if(str=="guild") {
	this_player()->more_string(read_file("/players/tatsuo/guild/help"));
	return 1;
    }
}
...
_beep(arg) {
	object obj;
	if ( !this_player()->query_immortal() && arg != "gambit" && arg != "tatsuo") {
	    write( "Sorry, but you only can beep Gambit or Tatsuo.\n" +
	      "And for a good reason please ! (* A bug or something like that. *)\n" );
	    return( 1 );
	}
