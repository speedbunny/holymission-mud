/* 920228 mud@mud.edvz.uni-linz.ac.at Herbert Rosmanith	 */
...
/* hide		...  hide in the shadows
   ...
   poison	...  apply poison on weapons
   exits	...  shows the exits of a room
...
#define DODGE	dir+"/dodge_shadow"
#define	HIDE	dir+"/hide_shadow"
...
int rv_lock;					/* used to lock reveal */
...
guild_changes() {		/* We have to destruct the shadow here */
object sh;

  sh=shadow(TP,0);
  while(sh) {
    if (explode(file_name(sh),"#")[0]==DODGE)
	destruct(sh);
    else sh=shadow(sh,0);
  }
}
...
guild_line(str) { return this_player()->guild_line(str); }
...
object ob,en,sh;
  ...
  if (str) {			/* hide an object */
     ...
     if (!(ob=present(str,TP)) && !(ob=present(str,env(TP)))) {
	notify_fail("Hide what ?\n");
	return 0;
     }
     if (living(ob)) {
	notify_fail(ob->short()+" is too fat to be hidden.\n");
	return 0;
     }

     /* hardcode the castle ... */
     if (ob->id("castle") || ob->query_property("no_hide")) {
	notify_fail(ob->short()+" is too big to be hidden.\n");
	return 0;
     }
     if (env(ob)==TP && (en=env(TP))) transfer(ob,en); /* drop it */

     sh=clone_object(HIDE);
     sh->start_shadow(ob,str);
     say(TPN+" hides something.\n");
     write("Ok.\n");
     return 1;
  }
  ...
  if (env(TP)->query_property("no_hide")) {
     notify_fail("You can't find a place to hide here.\n");
     return 0;
  }
 
  if (TP->query_invis()) {
     notify_fail("You are already hidden in the shadows.\n");
     return 0;
  }
...
reveal(str) {
    ...
    call_out("reveal_item",5,str);
    return 1;

}

/* with full dexterity, the probabilty to find an item is 50 %, but
   there's a check against charisma (used as luck) also.
*/
...
int i,dx,ch;
    ...
    ob=present(str+"__hidden__",env(TP));
    if (!ob || env(TP)!=env(ob)) {
	write("You found really nothing.\n");
	return;
    }
    dx=TP->query_dex();
    ch=TP->query_chr();
    ...
    if (i) {
	if (sh=shadow(ob,0)) destruct(sh);
	write("You found "+ob->short()+".\n");
	say(TPN+" found something.\n");
	return;
    }
...
object ob;
...
#if 0	/* HERP: but Moonchild, that's ridiculous !!! */
  ...
  /* We now have two calls to query_attack.
     a) with an argument, to check ourselves,
     b) without an argument to check if someone else is attacking
  */

  if (ob->query_attack(TP)) {
     write(ob->query_name()+" is ready for you.\n");
     return 1;
  }
  if (ob->query_attack()) {
     write(ob->query_name()+" is much too alert, and recognises you.\n");
     ob->attack_object(TP);
     return 1;
  }
  ...
  ob->attack_object(TP);
  TP->add_exp(ob->hit_player(200*TPL*3*(TPDX/10+1)/360));
  return 1;
}
...
int i;
  ...
  if (!(owp=present(wp))) {
     write("You don't have a "+wp+".\n");
     return 1;
  }
  if (!(ops=present(p))) {
     write("You don't have a "+p+".\n");
     return 1;
  }
  if (!ops->query_poison()) {
     write("The "+ops->query_name()+" is no poison.\n");
     return 1;
  }

  if ((sh=shadow(owp,0)) && explode(file_name(sh),"#")[0]==POISON) {
     write("You fumble, and pour the "+ops->query_name()+" on the ground.\n");
     return 1;
  }
  write("You pour "+ops->query_name()+" over "+owp->short()+".\n");
  say(name+" pours "+ops->query_name()+" over "+owp->short()+".\n");

  ops->set_empty();  /* Matt 4-12-94: Moved this to make more sense */
...
int i;
  ...
  if (!str) {
    if (dest=env(TP)->query_room("dest_dir")) {
