#define DOOR "/players/whisky/dracdun1"
...
open(str) {
 DOOR->open(str);  /* we now just call the functions of the other room */
 return 1;
}
...
unlock(str) {
 ...
 id(str) {
  if (str=="grate") {
  write("You see a "+DOOR->query_grate()+"\n");
  return 1;
  }
  }
...
move() {
   if (DOOR->query_open()==1) {
      this_player()->move_player("west#players/whisky/dracdun1");
      }
   else {
...
long(str) {
 if (str=="at grate" || str=="grate") return;
