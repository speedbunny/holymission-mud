...
query_auto_load() {
return "/players/kit/closed/Fruitpie Wand:"; }
...
reset(arg) {
  ...
  desc_array[DSHORT] = "A Beautiful Looking Fruitpit Wand (wielded)";
...
Copy(str) {
  string to, from, buff;
  int start, end, i;
  if(sscanf(str, "%s %s %d %d", from, to, start, end) != 4) {
    write("Error in copy command. Use 'Copy <from-file> <to-file> <start-line> <end-line>'.\n");
    return 1;
   }
  for(i = start, i <= end, i++) {
    buff = read_file(from, i);
    write_file(to, buff);
   }
  write("Copied.\n");
  return 1;
