...
#define FORMULA2_N_AC   (6+TPW/4)
#define  SP_AIKIDO       (8+TPW/4)  
#define DUR_AIKIDO      (100+TPW*3) 
#define FORMULA_WC       (random(15+TPW/2+TPD/2)/2)
#define FORMULA_DEATH      ((60+random((TPW+TPD)*2)))
#define HP_DEATH          (TPC/3*2)

#define SP_DEATH          (25+(TPW+TPD))
#define ALLOW_DEATH     30
#define FORMULA_SMASH   (random(65)+TPD/3)
#define SP_SMASH        (24+TPD/3)
#define MAX_H_SMASH     (random(4+TPD/6))
#define ALLOW_SMASH      15
#define SP_CHOKE        ((TPD/3)+6)
#define FORMULA_CHOKE   ((TPD/3)+random(20))
#define ALLOW_CHOKE      5
#define SP_CHARM        (8+TPW)
...
reset(arg) {
  ...
  set_id("monk robe");        /* 920916 Herp */
set_type("guild");
  set_alias(id);        /* 920916 Herp */
...
int i,j;
    ...
    if ((i=TPCH*2)<=(j=ob->C) || random(i) <= random(j*3)) {
     ...
     if (ob->query_wielded()) { 
       ...
       /* TP->restore_spell_points(SP_CHARM); */ /* HERP :-((( */
       TP->restore_spell_points(-SP_CHARM);
      return 1;
    }
...
/* ########################## pentagram ############################### */
  ...
  pentagram(arg) {
   object pent; /* Moonchild - for transfer */
