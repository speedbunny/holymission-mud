...
light_activate() 
{
    ...
    if (call_other(TP, "query_level") < 31) 
     {
        set_light(1);
...
info_spellbook(arg) 
{
   if ((arg == "book") || (arg == "spell book")) 
   {
      ...
      say(TPN+" flicks through the old pages of his spell book.\n");
      long();
      return 1;
    }
   return 0;
...
rezrov_door(str) {
    ...
    if (call_other(TP, "query_level") < 31) {
       ...
       say(TPN+" tries to look very concentrated.\n");
       return 1;
       }
       ...
       if (obj) {
          ...
          if (((verify[0]==65) && (verify[2]==100)) && (verify[7]=116)) {
            ...
            say("Suddenly a ray of intense enery emits from "+TPN+"'s hand\n"+
	    ...
	    return 1;
          ...
          write("Somehow the "+CAP(str)+" doesn't appear to be here.\n");
          return 1;
          }
   }
...
padroz_it(str) {
    ...
    if (!str) {
	obj = environment(TP);
        say("For a brief moment the room glows brightly",TP);
    ...
    obj = present(str, TP);
    if (obj) {
	write("The Padroz spell resets " + call_other(obj, "short") + ".\n");
	call_other(obj, "reset", 1);
	return 1;
    }
    obj = present(str, environment(TP));
...
vaxum_it(str) {
     ...
     if (who) {
        ...
        TOBJ(who,"Suddenly you fail to see the point in this fight.\n");
        call_other(TP,"stop_fight");
...
cleesh_it(str) 
{
     object who;
     string name;
     if (call_other(this_player(),"query_level") <31) {
        write("Nothing happens.\n");
        say(TPN+" tries to look very concentrated.\n");
        return 1;
        }
     ...
     if (who) {
        ...
        write("The Cleesh spell was unable to transform "+CAP(str)+".\n");
        return 1;
        }
     }
...
zifmia_it(str) {
    ...
    if (who) {
        ...
        TOBJ(who,"Suddenly the world around you fades away into darkness.\n");
        MO(who,environment(TP));
        TOBJ(who,"You reappear somewhere else.\n");
        write("In a bright flash of magic "+CAP(str)+" appears.\n");
       return 1;
       } else {
