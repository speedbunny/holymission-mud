...
void long() 
{
  writelw("This is the corpse of the Lord of Air ");
  if (arms)
...
{  
  if (counter == 4)
    return "The corpse of the Lord of Air";
  if (counter == 3)
    return "The decayed corpse of the Lord of Air";
  if (counter == 2)
    return "The heavily decayed remains of the Lord of Air";
  if (counter == 1)
    return "The skeleton of the Lord of air";
}
  ...
  if (present("sacrificial knife", TP)) 
  {
    if (counter > 1)
    {
	  ...
	  say(TPN+" stabs deeply into the corpse and pulls something out.\n");
          ...
          ob->set_name("Head of the Lord of Air");
	  ob->set_part("head");
	  ob->set_code("ah1");
	  ob->set_owner("Lord of Air");
	  ob->set_short("A head");
	  ob->set_long("The hacked off head from the Lord of Air.\n");
	  ob->set_weight(1);
	  ob->set_value(1);
	  transfer(ob, TP);
	  head = 1;
	  ...
	  write("You hack off the arms from the corpse.\n");
  	  say(TPN+" stabs deeply into the corpse and pulls something out.\n");
          ob = clone_object(PLANES + "part");
          ob->set_name("Arms of the Lord of Air");
	  ob->set_part("arms");
	  ob->set_code("aa2");
	  ob->set_owner("Lord of Air");
	  ob->set_short("Pair of Arms");
	  ob->set_long("The hacked off arms from the Lord of Air.\n");
	  ob->set_weight(1);
	  ob->set_value(1);
  	  transfer(ob, TP);
	  arms = 1;
	  ...
	  say(TPN+" stabs deeply into the corpse and pulls something out.\n");
          ob = clone_object(PLANES + "part");
          ob->set_name("Legs of the Lord of Air");
	  ob->set_part("legs");
	  ob->set_code("al3");
	  ob->set_owner("Lord of Air");
	  ob->set_short("Pair of Legs");
	  ob->set_long("The hacked off legs from the Lord of Air.\n");
	  ob->set_weight(1);
	  ob->set_value(1);
	  transfer(ob, TP);
	  legs = 1;
	  ...
	  say(TPN+" stabs deeply into the corpse and pulls something out.\n");
          ob = clone_object(PLANES + "part");
          ob->set_name("Torso of the Lord of Air");
	  ob->set_part("torso");
	  ob->set_code("at4");
	  ob->set_owner("Lord of Air");
	  ob->set_short("A Torso");
	  ob->set_long("The hacked off torso from the Lord of Air.\n");
	  ob->set_weight(2);
	  ob->set_value(1);
	  transfer(ob, TP);
	  torso = 1;
...
int do_decay() 
{
  if (counter == 1)
  {
    write("The remains of the Lord of Air vanish.\n");
    destruct(TO);
    return 1;
  }
  counter--;
