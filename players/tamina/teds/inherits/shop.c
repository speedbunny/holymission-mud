...
public int get_list(string arg) 
{
  ...
  {
    write(keeper+" tells you: Sorry, but the store is empty.\n");
    return 1;
  }
  list = unique_list(all_inventory(query_store()),"short");
  if (sizeof(list) > maxlist)
    max = maxlist;
  else 
    max = sizeof(list);
  printf("%-8s%-10s%-40s%-6s\n","Num:","Store:","Name:","Value:");
  for(i = 0;i < max;i++)
  {
    printf("#%-7i%-10i%-40s%-6i\n",
...
public int player_buy(object player, object ob)
{
  if (player->query_money() < ob->query_value()*2)
    ...
    write(keeper+" tells you: Sorry you can't carry that much weight.\n"); 
  else 
  {
