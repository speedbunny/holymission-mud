...
int do_get(string str) 
{
   ...
   || str == "metal from pedestal" || str == "metal lump") 
  {
    ...
    if (set_light(0) <= 0) 
    {
      ...
      write("However, you manage to find the pedestal and feel around\n");
      write("on the top of the pedestal and find a lump of metal!\n");
      if (!TP->add_weight(6)) 
      {
...
int press(string str) 
{
  ...
  if (test == 6) 
  {
    ...
    call_other(TP,"reduce_hit_point", 30 );
    return 1;
  }
  if (test == 3) 
  {
  ...
  if ((test == 2) || (test == 5)) 
  {
    ...
    scroll = clone_object("/players/tamina/teds/items/adamite_hint_scroll");
