...
#define ACC ({"whisky","patience","llort","muzmuz","herp"})
...
int set_skills(object mob,string mskill,int lv)
{
   ...
   int sz;
    ...
    if (!interactive(this_player()) || 
         member_array(apply(call,this_player(),"query_real_name"),ACC)==-1)
         return 0;
    ...
    if (mskill=="all") 
    {
        ...
        {
           sz--;
           funcall(call,_gm,"change_skill",mob,skills[sz],lv,
                   SKILLS[skills[sz]][2],7);
        }
        return 2;
     }
     funcall(call,_gm,"change_skill",mob,mskill,lv,SKILLS[mskill][2],7);
     return 1;
}
