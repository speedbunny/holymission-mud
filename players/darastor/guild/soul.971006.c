...
void init() {
	...
	ESE("TATTOOPOS", "brow");
    if(!shadow) {
	shadow= clone_object("/players/darastor/guild/shadow_archive/shadow");
...
int prepare_combat(string str) {
    ...
    de_equip();
    move_object(comb_soul=clone_object("players/darastor/guild/soul_archive/combat_soul"), E);
    return 1;
}
...
int levelsort(object p1, object p2) {
    if((string)p1->QRN=="darastor") return 0;
    if((string)p2->QRN=="darastor") return 1;
...
/* Added help for legend levels. -Kryll */
    ...
    if(str=="guild") {
        this_player()->more_string(read_file("/players/darastor/guild/TEXT/help.txt"));
	return 1;
    }
    return 0;
}
...
_beep( arg ) {
   ...
   if ( !this_player()->query_immortal() && arg != "darastor") {
	write( "Sorry, but you only can beep Darastor.\n" );
	return 1;
   }
   ...
   if ( !obj ) {
	write( "Sorry, but Darastor must either be invis, busy, or not on.\n" );
	return 1;
   }
