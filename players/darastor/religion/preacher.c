...
#define tr(str)\
  tell_room(environment(this_object()),"Preacher Man says: " + str + "\n")
...
reset(arg) {
  ::reset(arg);
  if(arg) return;
  set_name("preacher man");
  set_alias("preacher");
  set_alt_name("priest");
  set_race("man");
  set_short("Preacher Man");
  set_long("This is the Preacher Man, who will marry players if asked.\n"
...
weddinghelp() {
  cat("/players/moonchild/wedding/helpfile");
  return 1;
}
...
int belief;
  ...
  if(money_needed>0) {
    write("Preacher Man still needs paid " + money_needed + " coins.\n");
    return 1;
  }
  if(marrying) {
    write("Preacher Man is already marrying " + capitalize(one) + " and " + capitalize(two) + ".\n");
    return 1;
  }
...
blah9() {
  ...
  tell_object(pone,"Preacher Man tells you: A simple yes or no will suffice.\n");
  marrying=3;
  call_out("blah10",1,0);
}
...
blah11() {
  ...
  tell_object(ptwo,"Preacher Man tells you: A simple yes or no will suffice.\n");
  call_out("blah12",1,0);
}
...
blah13() {
  ...
  say("Preacher Man gives ring to " + capitalize(one) + ".\n",pone);
  tell_object(pone,"Preacher Man gives ring to you.\n");
  move_object(ring=clone_object("players/moonchild/wedding/ring"),pone);
  ring->name1(capitalize(two));
  ring->name2(capitalize(one));
  ring->title_set();
  say("Preacher Man gives ring to " + capitalize(two) + ".\n",ptwo);
  tell_object(ptwo,"Preacher Man gives ring to you.\n");
...
add_money(m) {
  if(this_player()->query_immortal()) {
    write("Preacher Man tells you: I do not accept wizards' money!\n");
    return 1;
  }
  money_needed-=m;
  if(money_needed>0) write("Preacher Man tells you: I still need paid "
	+ money_needed + " coins.\n");
  else write("Preacher Man tells you: The wedding may now go ahead.\n");
}
