...
Ps: Most functions are explained now, sorry but with the amount 
...
void random_move() {
  ...
  if(environment() && dirs = environment()->query_dest_dirs()) {
    if(sizeof(dirs)/2) {
...
object looking_for_someone_to_attack(object my_env) {
  ...
  while (vic) {
    if(vic != this_object() && !chk_invis(me,vic)
                            && chk_attack(vic) 
                            && vic != tamer) {
      return vic;
    }
...
void set_level(int l) {
  ...
  level = l;
  stats = ({ l, l, l, l, l, l });
  natural_ac = 1 + level/8;
#if 0
  if(l < 20)
    stats = ({ l, l, l, l, l, l });
  else
    stats = ({ l+(l/2), l+(l/2), l+(l/2), l+(l/2), l+(l/2), l+(l/2) });
  natural_ac = 2 + level/4;
#endif
  recalc_ac();
