...
#define NINJA_POISON "/players/patience/guild/nother/ninja_poison"
...
throw_powder( arg )
{
   ...
   if ( random(100) < (nsoul->query_throw_skill()*2)/3) {            /* successful ? */                             /* miss              */
      if ( transfer( this_object(), who ) )
         move_object( this_object(), who );
      nsoul->advance_skill( this_player(), THROW_SKILL, 1, 100 );
     ...
     if (who && living(who))
      ...
      write( "You are not skilled enough to throw the dart.\n" );
