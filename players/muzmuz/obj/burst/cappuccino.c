#define COLOURS ({"excellent","marvellous","tasty","tasty","excellent"})
...
void reset(int arg) {
  if(arg)
    return;
  set_id(COLOURS[random(5)] + " Cappuccino");
  set_short("A " + name);
  set_alias("Cappuccino");
  set_value(10);
  set_weight(0);
...
int throw(string str) {
  ...
  write("You throw the Cappuccino to " + capitalize(str) + ".\n");
  transfer(this_object(), environment(ob));
  shout((string)this_player()->query_name()
	+ " sends a Cappuccino across the sky to "
	+ (string)ob->query_name() + "!\n");
  tell_object(ob, "You leap up and catch the Cappuccino with your hands.\n");
  say((string)ob->query_name() + " leaps up and catches the Cappuccino with "
