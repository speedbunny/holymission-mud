...
int throw(string str) {
  ...
  write("You throw the Pizza to " + capitalize(str) + ".\n");
  transfer(this_object(), environment(ob));
  shout((string)this_player()->query_name()
	+ " sends a Pizza across the sky to "
	+ (string)ob->query_name() + "!\n");
  tell_object(ob, "You leap up and catch the Pizza with your hands.\n");
  say((string)ob->query_name() + " leaps up and catches the Pizza with "
