/* 310793: Galadriel: now uses obj/keg. There were too many 
                      different and buggy and powerful kegs
                      in HM. */
...
reset(arg) {
  store = 80;
   ...
   if (!zeela) {
      zeela=clone_object("/players/meecham/tower/zeela");
      move_object(zeela,this_object());
      tell_room(environment(zeela),"Zeela arrives.\n");
      }
  ...
  long_desc=
    ...
    dest_dir=({ 
      "players/meecham/tower/bazaar","east",
    });
...
write(
     ...
     "We offer:\n\n"+
     "Nr: Sort:                                    Cost:       \n"+
     "1  A big Keg of Brandy                       10000 coins \n"+
     "2  A small Keg of Brandy                      6000 coins \n"+
     "3  A big Keg of Whiskey                       5000 coins \n"+
     "4  A small Keg of Whiskey                     3000 coins \n"+
     "5  A big Keg of Rum                           2500 coins \n"+
     "6  A small Keg of Rum                         1500 coins \n"+
...
order(str)
{
 ...
 if (!present("zeela",this_object())) {
    write("Sorry, Zeela is having a break, try later.\n");
    return 1;
    }
 ...
 if (!str) {
  ...
  weight = 2;
    ...
    write("Zeela says: Sorry but we don't sell "+str+" here.\n");
    return 1;
  }
 
  if (!store) {
     write("Zeela says: Sorry but we are out of kegs at the moment, please try later.\n");
     return 1;
    }
  else if (this_player()->query_money() < (value*2)) {
    write("Zeela says: That would cost you "+(value*2)+" coins, which you don't have.\n");
    return 1;
  }
  ...
  if (transfer(keg,this_player())) {
     write("Zeela says: You can't carry that much.\n");
     return 1;
     }
  store --;
  write("Zeela gives you a "+alt_name+" for "+(value*2)+" coins.\n");
