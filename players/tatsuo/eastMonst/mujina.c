...
reset(arg)
{
   ...
   set_long( "A mujina appears at first to be a normal person. It has a natural\n" +
    ...
    amulet = clone_object("/players/tatsuo/guild/nother/onyxRing" );
    transfer(amulet,this_object());
    amulet =clone_object("/players/tatsuo/guild/narmour/amulet");
    transfer(amulet,this_object());
    command("wear amulet");
...
unmask( victim )
{
   ...
   if (objectp(victim) && living(victim) && interactive(victim) ) 
   {
      if ( environment(this_object()) == environment(victim) )
      {
         ...
         madness = clone_object( "/players/tatsuo/eastMonst/insane" );
