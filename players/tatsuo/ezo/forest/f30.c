...
reset( arg )
{
   ...
   items = ({ "forest", long_desc,
              ...
              "rock-ledge", "The rock-ledge is about 5.5 meters above the ground",
               "cave","A dark hole in the cliff face",
              });
              
   dest_dir = ({ PPATH + "f25", "north",
                 PPATH + "f26", "northeast",
                 PPATH + "f31", "east",
                 PPATH + "f29", "west",
                 PPATH + "f24", "northwest",
...
enter(str){
if(str=="cave" || str=="cliff"){
this_player()->move_player("into the cave#players/haplo/quest/c1");
write("You slip into the darkness of the cave.\n");
return 1;
}
else return 0;
}
