...
reset(arg) {
   ...
   items=({
	"walls","They are shimmering with a faint yellow light",
	"wall","It is shimmering with a faint yellow light",
...
do_wash(arg) {
 ...
 say(TP->query_name()+" washes "+TP->query_possessive()+" "+arg+".\n");
   return( 1 );
}
...
do_rinse(arg) {
   ...
   say(TP->query_name()+" rinses "+TP->query_possessive()+" "+arg+".\n");
   return( 1 );
}
...
if(TP->query_religion() == "Amaterasu") {
  write("Amaterasu breathes life into your broken form.\n");
  TP->remove_ghost();
  }
return 1;
}
