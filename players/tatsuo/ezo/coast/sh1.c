...
reset( arg )
{
   ...
   set_rf_dirs( "west and northwest" );
   set_shore_dirs( "southwest" );
...
swim_it( arg )
{
   ...
   else if ( arg == "north" || arg == "to coast" || arg == "to northern coast" ) {
      TP->move_player( "into the sea#players/tatsuo/ezo/coast/waterLink" );
      return( 1 );
   }
...
build_long_desc( )
{
   ...
   s = "You are standing at the shore of the island of Ezo. The ground here is sandy\n" +
       ...
       "In some distance to the north can you see the coast of the northern part of Ezo.\n";
   
   s += "The shore continues " + query_shore_dirs();
   
   if ( query_rf_dirs() && strlen( query_rf_dirs() ) )
      s += " and to your " + query_rf_dirs() + "\nis the rock-face of the vulcano";
...
l_vulcano( )
{
   if ( query_rf_dirs( ) )
      ...
      return( "You cannot see the vulcano from here" );
}

l_rockface( )
{
   if ( query_rf_dirs( ) )
      return( "You think, that the rock-face almost zoom up to eternity" );
   else
