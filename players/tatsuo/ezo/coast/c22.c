...
reset( arg )
{
   ...
   items = ({ "slope", long_desc,
              "ground", "The ground consits of lava and mist crawls out of it",
              "lava", "It is dark black lava, already colded",
              ...
              "rock-face", "You think, that the rock-face almost zoom up to eternity",
              ...
              "vulcano", "The vulcano is very high and looks extremely steep",
              "sea", "@@l_sea@@",
              "nihonese sea", "@@l_sea@@",
              "water", "@@l_water@@",
              "wave", "@@l_wave@@",
...
get_it( arg )
{
   ...
   if ( arg == "plant" || arg == "plants" || arg == "vegetations" ) {
      write( "As you try to get one of the plants, it suddenly rottens in your hand.\n" );
      return( 1 );
   }
...
climb_it( arg )
{
   ...
   if ( arg == "rock-face" || arg == "vulcano" ) {
      ...
      tekagi = present( "tekagi", TP );
      if ( tekagi && tekagi->query_worn() )
         this_player()->move_player( "up the tree#players/tatsuo/ezo/coast/inTree" );
      else {
         write( "You can't find a halt at the tree and so you fail.\n" );
...
build_long_desc( )
{
   ...
   s = "You are standing on a slope of the island of Ezo. The ground consits of\n" +
       "lava and there is very spare vegetation here. Mist crawls from the ground.\n" +
...
{
   if ( query_coast_dirs() )
      return( "Water, as far as your eye can see" );
   else
