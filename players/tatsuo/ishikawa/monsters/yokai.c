...
reset(arg) {
    ...
    set_long(BS("The Hengeyokai is a person who is cursed with "+
    ...
    set_ac(6);
    set_ep(85000);
    weapon = clone_object("/players/patience/guild/nweap/kama");
...
heart_beat(){
	...
	if(!random(5)) {
	    tell_object(AO,BS("The Hengeyokai hisses very loud and transforms into a cat, "+
		"scratching you into two pieces!"));
	    say(BS("The Hengeyokai hisses very loud at "+AO->query_name()+" and transforms into a cat, scratching the hell out of "+
		this_player()->query_objective()+"!"),AO);
	    AO->hit_player(10);
	}
	else if (!random(6)) {
	    tell_object(AO,BS("The Hengeyokai moves with lightning speed as a cat, and slams into you!"));
	    say(BS("The Hengeyokai runs past "+AO->query_name()+" and slams into "+this_player()->query_objective()+"!"),AO);
	    AO->hit_player(20);
	}
    }
}
