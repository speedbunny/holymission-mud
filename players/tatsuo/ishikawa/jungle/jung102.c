...
dest_dir=({
	IPATH+"jung113","northwest",
	IPATH+"jung114","north",
	IPATH+"jung103","east",
	IPATH+"jung92","southeast",
	});
}
init(){
::init();
...
AA("_climb","climb");
if(present(slash,TP)){
MOVE("hacking "+TPOSS+" way east#players/tatsuo/ishikawa/jungle/jung103");
return 1;
}
else
...
_east(){
if(present(slash,TP)){
MOVE("hacking "+TPOSS+" way east#players/tatsuo/ishikawa/jungle/jung59");
return 1;
}
else{
write("You don't have anything that could cut through the foliage.\n");
