...
reset(arg){
    ...
    dest_dir=({
      IPATH+"jung110","north",
      IPATH+"jung111","northeast",
      IPATH+"jung100","west",
      IPATH+"jung91","southwest",
    });
}
void init(){
    ::init();
...
_north(){
    if(present(slash,TP)){
	MOVE("hacking "+TPOSS+" way north#players/tatsuo/ishikawa/jungle/jung110");
	return 1;
    }
    else
...
_west(){
    if(present(slash,TP)){
	MOVE("hacking "+TPOSS+" way west#players/tatsuo/ishikawa/jungle/jung100");
	return 1;
    }
    else{
	write("You don't have anything that could cut through the foliage.\n");
