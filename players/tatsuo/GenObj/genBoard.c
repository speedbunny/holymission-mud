...
long() {
   int ind;
   if ( this_player() &&
       (this_player()->query_wizard() || this_player()->query_real_name() == "patience") )
      write("This is "+short_name+".\n" +
            "Usage : note <headline>, read/remove <message number>\n" +
            "        store <message number> <file name>\n");
   else if ( del_rights_func && 
             call_other(del_rights_func,"delete_rights",TP) ) {
      write("This is "+short_name+".\n" +
            "Usage : note <headline>, read/remove <message number>\n" );
   }
   else
      write( "His is " + short_name + ".\n" +
             "Usage: note <headline>\n " +
             "       read <message number>\n" );
   ...
   write("The "+name+" contains " + msg_num);
   if (msg_num == 1)
      write(" note :\n\n");
   else 
      write(" notes :\n\n");
...
short() { return capitalize(short_name) + " ["+msg_num+" notes]"; }
...
remove_msg(what_msg) {
   ...
   if ( !( this_player()->query_name() == player || 
        (this_player()->query_real_name() == "patience") || 
        this_player()->query_archwiz() ||
        (del_rights_func && call_other(del_rights_func,"delete_rights",TP) ) ||
