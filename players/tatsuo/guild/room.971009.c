...
do_join() {
    ...
    write( "NOT POSSIBLE AT THE MOMENT !\n" );
    return( 1 );
...
query_maxsp(my_int,ll) {
  return( my_int*4 + my_int/3 + 120 + TP->query_legend_level()/3 );
}

query_maxhp(my_con,ll) {
  return( my_con*16+50+TP->query_legend_level()/2);
}

query_maxfp(my_dex,ll) {
   return( my_dex*14+30+TP->query_legend_level()/2 );
}
