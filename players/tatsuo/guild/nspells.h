...
_vanish( arg )
{
   ...
   switch(arg)    /* whisky it's faster */
   {
     ...
     case "church": 
      ...
      return( 1 );
      case "guild": 
         TP->move_player( "X", "/players/tatsuo/guild/room" );
         if ( !(TP->query_immortal()) )
            TP->restore_spell_points(vcost);
      return( 1 );
     case "tree":
        TP->move_player( "X","/players/tatsuo/ezo/inAlmond1");
        if ( !(TP->query_immortal()) )
           TP->restore_spell_points(vcost);
        return 1;
      case "jingu": 
         TP->move_player( "X", "/players/tatsuo/ezo/coast/jingu" );
         if ( !(TP->query_immortal()) )
             TP->restore_spell_points(vcost);
      return( 1 );
      case "temple": 
         TP->move_player( "X", "/players/tatsuo/ezo/forest/forestTemple" );
         if ( !(TP->query_immortal()) )
             TP->restore_spell_points(vcost);
       return( 1 );
       default:
