...
#define BLINDING_LIQUID "/players/tatsuo/guild/nother/blind_liquid"
...
throw_blood( arg )
{
   ...
   if ( random(100) >= THROWSKILL ) {                                              tell_object( this_player(), 
...
throw_blind( arg )
{
   ...
   if ( random(100) >= THROWSKILL ) {                                        
     write( "Too much chaos, you can't target it.\n" );
      return( 1 );
   }
...
throw_pepper( arg )
{
   ...
   if ( query_open() == 1 ) {
      ...
      shad = clone_object( "/players/tatsuo/guild/nother/sneeze_shad" );
      shad->start_shadow( TP, 20 );
      who->attack_object(TP);
   }
   else if ( random(100) >= THROWSKILL ) {
      ...
      write( "You throw a PEPPER-BOMB right into the face of your opponent,\n"+
             "making him start sneezing !!!\n" );
