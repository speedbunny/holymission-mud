...
reset(arg) {
  if(!arg) {
    short_desc="North in the Arena";
    long_desc="Northern part of the Arena.  This is where you have come to\n"
...
long(a) {
  ...
  if(tmp=get_inv_list(FPATH+"arena3")) {
    write("In the northeast corner you see: ");
    write(tmp);  fl=1;
  } 
  ...
  if(tmp=get_inv_list(FPATH+"arena7")) {
    write("In the southwest you see: ");
    write(tmp);  fl=1;
  }
  ...
  if(tmp=get_inv_list(FPATH+"arena9")) {
    write("In the southeast you see: ");
    write(tmp);  fl=1;
  }
  if(fl && get_inv_list(FPATH+"arena2")) write("And here is : ");
}  
