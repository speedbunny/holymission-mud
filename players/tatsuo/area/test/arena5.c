...
#define FPATH	"players/tatsuo/area/test/"

inherit "room/room";
#include "/players/tatsuo/ftp/summon.h"

reset(arg) {
  if(!arg) {
    short_desc="Center of the Arena";
  
    long_desc="Center of the arena.  This is where you have come to\n"
    ...
    set_light(1);
    dest_dir=({FPATH+"arena2","north"
    	      ,FPATH+"arena6","east"
	      ,FPATH+"arena4","west"
	      ,FPATH+"arena1","northwest"
	      ,FPATH+"arena3","northeast"
	      ,FPATH+"arena7","southwest"
	      ,FPATH+"arena9","southeast"
	      ,FPATH+"arena8","south"
	    });
       items=({
...
long(a) {
  ...
  if(tmp=get_inv_list(FPATH+"arena1")) {
    write("In the northwest you see: ");
    write(tmp); fl=1;
  } 
  ...
  if(tmp=get_inv_list(FPATH+"arena3")) {
    write("In the northeast you see: ");
    write(tmp);  fl=1;
  }
  ...
  if(tmp=get_inv_list(FPATH+"arena7")) {
    write("In the southwest corner you see: ");
    write(tmp);  fl=1;
  }
  ...
  if(tmp=get_inv_list(FPATH+"arena9")) {
    write("In the southeast corner you see: ");
    write(tmp);  fl=1;
  }
  if(fl && get_inv_list(FPATH+"arena5")) write("And here is : ");
}  
