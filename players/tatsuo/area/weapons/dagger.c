...
weapon_hit(ob) {
  ...
  say(this_player()->query_name()+" sneaks past "+ob->query_name()+"'s defences and stabs him with the dagger!\n");
  ob->add_poison(5);
  power_hit=50+random(charges*15);
  tell_object(this_player(),"I hit monster for "+power_hit+"\n");
  return power_hit;
}
