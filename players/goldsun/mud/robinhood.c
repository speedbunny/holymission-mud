...
void go_to_win_it(){
 ...
 transfer(this_object(),"/players/goldsun/workroom");
...
enter_inventory(ob) {
  ...
  if (ob->id("goldsun_silver_tooth")) {
    str += "tooth.\n";
  } else if (ob->id("goldsun_feathers")) {
    str += "feathers.\n";
  }
  
  if ( present("feathers") && present("tooth")
    && !this_player()->query_quests("help hern")) {
