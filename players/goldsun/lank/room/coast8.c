...
if (arg) return;
 ...
 long_desc=
  ...
  +"you can see the tall, snow-topped peak of Langbourg mountains. The ground\n"
  +"consists of sand and rocks. Huge waves are rolling towards rocks.\n"
                 );

 smell="You smell fresh, damp and salt air.";
 dest_dir=({"/players/goldsun/lank/room/coast4","northwest",
           "/players/goldsun/lank/room/coast7","southwest",
	   "/players/goldsun/lank/room/lank3","west"});

 items=({"sea","The sea is blue",
 	 "water","The water looks clear",
	 "rock","The rock is dark",
	 "rocks","The rock are from the Lankpeak. They are dark",
    	 "coast",long_desc,
	 "island",long_desc,
...
climb(arg){
 if (!arg) return;
 if (arg=="up" || arg=="mountains" || arg=="up mountains" ||
      arg=="Lankbourg mountains" || arg=="up Lankbourg mountains"||
      arg=="lankbourg mountains" || arg=="up lankbourg mountains"){
    write("You start to climb up the mountains.\n\n");
    write("Uffff\n\n");
    write("Uffff\n\n");
    TP->move_player("up#/players/goldsun/lank/room/peak1");
    return 1;
  }
 }
