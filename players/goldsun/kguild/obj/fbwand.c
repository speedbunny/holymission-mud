...
if (arg) return;
 ...
 account=10;   /* useable 5 times */ 
 set_name("ebony wand");
 set_alias("wand");
 set_short("An ebony wand");
 set_long("An ebony wand with inscription 'fire balls' on it.\n");
 set_value(1000);
 set_weight(1);
 }
...
int do_it(object ob){
 ...
 write("You cast a fire ball at "+ob->query_name()+".\n");
 say(TP->query_name()+" casts a fire ball at "+ob->query_name()+".\n");
 ob->hit_player(50,5);
 account--;
 if (account==0){
