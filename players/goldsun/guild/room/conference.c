#define PATH "/players/goldsun/guild/"
#define FILE  "/players/goldsun/guild/accept_banish"
#define TP  this_player()
#define TPN TP->query_name()
#define TPOBJ  TP->query_objective()
#define TPPOS  TP->query_possessive()
#define GN "guild/master"->query_number("fighter")
...
reset( arg )
{
   ...
   long_desc = 
      "This is the conference-room in the "+CN+". "+
...
do_accept( arg )
{
   if ( TP->query_guild() != GN )
      write( "You have to be a fighter to do this !\n" );
   else if ( TP->query_property("fighter_legend")==-1 )
      write( "Only Legends can do it.\n"); 
   else if ( !arg )
      write( "You have to give an argument (playername).\n" );
   else {
      if ( FILE->do_accept( TP, lower_case( arg ) ) ) {   
         write("You just have allowed "+capitalize(arg)+" to join the fighters"+
                ".\n" );
...
query_sign()
{
   return( 
      ...
      "\"accept <player>\":\n"+
      "   If a player wants to join the fighter-guild, at least one of the\n"+
      "   Fighter Legend have to accept this player. You can do this\n"+
      "   here.\n" +
    ...
    + "   to us (this will kick him/her out then). You need three Fighter\n"+
      "   Legends who must banish the player.\n"+
