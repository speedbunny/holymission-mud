    ...
    ::reset(flag);
     ...
     if (flag == 0)
     {
	     ...
	     "/players/goldsun/guild/room/hall",      "south",
             "/players/goldsun/guild/room/conference","west"
         });
...
int g_west()
{
  if (TP->query_level()>24 || TP->query_property("fighter_legend")!=-1) 
     return ::move("west");
  write("You are not allowed to enter this room yet.\n");
  return 1;
}
...
int do_read(string arg)
{
    if (!arg || (arg!="sign" && arg!="wooden sign"))
        return 0;
    write("Welcome to Fighter guild !\n"+
...
if ( !(CONF->query_is_accepted( TP->query_real_name() )) ) {
   write( "You have to talk to a Fighter Legend to join the fighters.\n" );
   return( 1 );
}
