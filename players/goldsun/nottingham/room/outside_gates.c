...
reset(arg){
 ...
 long_desc="There are large gates in front of you.\n";


 smell="The air is fresh.";

 dest_dir=({ "/players/goldsun/nottingham/room/r1","east"});
...
string query_gates(){
 if (GATES->query_is_open()) return "The gates are open";
 return "The gates are closed";
}
...
int knock(string str){
 ...
 if (str=="gates"){
   if  (GATES->query_number_of_enemy()>0){
    ...
    this_player()->move_player("into the town#players/goldsun/nottingham/room/gates");
   }  else {
...
int open(string str){
 ...
 if (str=="gates"){
   ...
   tell_room(GATES,"Someone opens the gates.\n");
   GATES->set_open(1);
   return 1;
 }
 return 0;
