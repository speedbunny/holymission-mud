...
{
 if (sizeof(foot)==0) 
return "A white snow. There are no footprints in it";
...
if (!arg){
 ...
 long_desc=
"You are walking through the cold valley. A breeze blows "+
...
"The whole island is covered by white snow.\n";
 ...
 items=({
   ...
   "peak","Very high peak with several trees on it. It's covered by snow too"
 });
}
 ::reset(arg);
...
int study(string str)
{
 ...
 if (str=="footprint" || str=="footprints" || str=="snow"){
   say(OPN+" studies snow carefully.\n");
   if (sizeof(foot)==0)
   {
