...
int set_backstab(int arg){
  if(arg>=0)
    in_backstab=arg;
  return in_backstab;
}

int able_to_hide(int arg){
  return !dont_hide;
}
...
int _filter(string str) {
    ...
    if(file_size("/players/blade/guild/bin/" + theverb + ".c") > 0)
        if (("/players/blade/guild/bin/" + theverb)->main(str))
            return 1;
        else return 0;
    if(theverb == "thief/" || theverb == "t/") {
        ("/players/blade/guild/bin/"+"thief")->main(str);
        return 1;
    }
    return 0;
}
