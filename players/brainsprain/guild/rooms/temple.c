...
reset(arg) {
 ...
 "A black altar for sacrificing lies in the center of the room.\n";
    ...
    dest_dir=({
      "guild/master"->query_dir(1) + "/room", "north",
      ROOMDIR+"scroll_room","east"
    });
    items=({
...
stand_up(){
  if(present(this_player(),this_object())){
    ...
    write("You leave the altar knowing that Mask watches you.\n");
    if(heal_score){
...
remove_ghost(str) {
  ...
  /*  if(belief==0) {
    write("You know that Mask is the true God.\n");
    this_player()->set_religion(RELIGION);
    } */
  if(belief!="mask") {
    /*    church = RLM->query_church(TP->query_religion()); */
    /*    if(this_player()->leave_religion(belief)) { */
  ...
  if(this_player()->query_ghost()){
    ...
    write("A horrible pain overcomes you, and you fall to the ground.\n");
    write("Mask has returned you to life, but you know he has taken a price.\n");
    call_out("stand_up",5);
    return 1;
  } 
