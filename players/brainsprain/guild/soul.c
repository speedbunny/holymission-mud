...
#define ROOM           "/players/brainsprain/guild/room"
...
int _filter(string str) {
    ...
    if(file_size("/players/brainsprain/guild/bin/" + theverb + ".c") > 0)
        if (("/players/brainsprain/guild/bin/" + theverb)->main(str))
            return 1;
        else return 0;
    if(theverb == "thief/" || theverb == "t/") {
        ("/players/brainsprain/guild/bin/"+"thief")->main(str);
        return 1;
    }
    return 0;
}
...
void init() {
  ...
  if (!thief->thief_shadow()) {
    shad = clone_object("/players/brainsprain/guild/thief_shadow");
  ...
  if(query_righteous()&& !dag){
    dag = clone_object("/players/brainsprain/guild/obj/rtd");
...
// write("renegade= "+renegade+" righteous= "+righteous);
  rm("/players/brainsprain/guild/thieves/"+THRNAME);
  write_file("/players/brainsprain/guild/thieves/"+THRNAME,
...
int get_guild_params() {
  ...
  if(!read_file("/players/brainsprain/guild/thieves/"+THRNAME)){
    return 2;
  }
  sscanf(read_file("/players/brainsprain/guild/thieves/"+THRNAME),
         "%s",file_string);
