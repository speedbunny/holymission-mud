...
int contest;  /* BLADE var for contest */
...
time_guild_soul()
{
    if (is_hidden == 1 && environment()->query_invis())
	stay_hidden--;
    if (stay_hidden < 0)
    {
...
backstab(str)
{
	...
	write("You move carefully toward " + ob->query_name()
	  + " but " + ob->query_objective() + " notices your approach!\n");
	ob->attack_object(this_player());
	ob->attack();
	return 1;
    }
...
if(contest){
  ...
  vat_cont=present(this_player()->query_real_name()+" barrel",find_object("/players/blade/guild/contest/contest_room.c"));
