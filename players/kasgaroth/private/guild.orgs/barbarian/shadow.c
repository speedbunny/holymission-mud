...
public int attack() {
  int i;
  while((i=do_attack()) && speed_counter>=33)
    speed_counter-=33;
  if(i)
    speed_counter+=speed;
  return i;
...
private int fight_filter(object ob) {
...
public int wield(object w, int hands) {
  ...
  if(!w->id("axe") && !w->id("club") && !w->id("knife") && !w->id("spear")) {
    weap_shad=clone_object("players/moonchild/guild/weap_shad");
...
public int add_poison(int i) {
  return (int)player->add_poison(FORT(i));
}
