...
inherit "players/kasgaroth/other/room";
inherit "room/room";

void reset(int arg) {
    if(arg)
	return 0;
    move_object("players/moonchild/guild/board", this_object());
    set_light(1);
    set_guild(11);
    ...
    dest_dir=({ "players/moonchild/pot/orc4", "north",
      "players/moonchild/guild/alit_temple", "southwest",
      "players/moonchild/guild/tamara_temple", "southeast" });
}

void init() {
    "players/kasgaroth/other/room"::init();
    "room/room"::init();
...
int do_join() {
	...
	"players/moonchild/guild/soul"->send_mess("<= Barbarians => "
...
int query_maxhp(int lvl) {
    return lvl*10 + 1000 + 4 * (this_player()->get_skill("barbarian_fortitude") ?
