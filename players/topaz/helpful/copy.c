...
init() {
  ::init();
  stop = 1;
  log_file("dummy","Dummy cloned by: "+this_player()->query_real_name()+
           " on "+ctime(time())+".\n");
...
end_dummy(arg)
{
 if (arg == "dummy")
  {
   write("The dummy says: Bye cruel world!\n");
 ...
 if (::attack())
  {
   ...
   hpd=500000-this_object()->query_hp();
...
{ say("The dummy crumbles to dust.\n");
  write("The dummy crumbles to dust.\n");
