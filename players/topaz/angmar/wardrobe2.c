...
out() {
  ...
  this_player()->move_player(
        "out#players/topaz/angmar/fairyland/road1");
  return 1;
}
...
open_door(str) {
  ...
  door_is_open=1;
  write("Ok.\n");
  say(this_player()->query_name(0)+" opened the door.\n");
  return 1;
}
...
close_door(str) {
  ...
  door_is_open=0;
  write("Ok.\n");
  say(this_player()->query_name(0)+" closed the door.\n");
  move_to_other();
  return 1;
}
...
move_to_other() {
  ...
  while(ob) {
    ...
    move_object(ob, "players/topaz/angmar/wardrobe1");
