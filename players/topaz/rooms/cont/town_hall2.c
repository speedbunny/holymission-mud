...
reset(arg) {
  ...
  if(!arg) {
    ...
    long_desc="This is the Allport Library.\n" +
              "There are many books on the shelves.\n" +
              "This room seems to get used fairly often.\n" +
              "You wonder why.\n" +
              "There is a large tome on one of the shelves that\n" +
              "catches your attention.\n";

    dest_dir=({CONT + "town_hall","up",
    });
...
object ob, k, i;
  ...
  k=sizeof(ob);
  for(i=0;i<k;i++)
    move_object(ob[i],"/players/topaz/rooms/cont/to_stats");
  busy=0;
  return 1;
}
