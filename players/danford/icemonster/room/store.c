...
east() {
    call_other(this_player(), "move_player", "leaves#players/randyandy/beach/shop");
    return 1;
}
...
reset(arg) {
 ...
 move_object(torch, this_object());
    ...
    if (!present("amulet")) {
    	object amulet;
    	amulet = clone_object("/players/xio/obj/msamulet");
    	move_object(amulet, this_object());
    }
    if (!present("sack")) {
    	object sack;
    	sack = clone_object(inherit "room/room";sack");
