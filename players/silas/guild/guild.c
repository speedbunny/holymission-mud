...
#include "std.h"
#define GUILD_NAME      "druid"			/* thieves guild	*/ 
...
TWO_EXIT("players/wolfi/room/chamber1","south",
         "players/wolfi/room/wood13","out",
	 "Druid's guild", "You are in the Druid guild.\n" +
...
do_join() 
{
if (this_player()->query_real_name() != "zoron") {
  write("This guild is not open yet.\n");
  return 1;
}
  ...
  TP->set_guild(guild);
  GM->get_soul(guild);
  write("You're now a member of the Druids.\n");
  say(TPN+" has joined the druid guild.\n");
  return 1;
} 
