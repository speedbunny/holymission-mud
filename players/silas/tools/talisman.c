#define FILENAME "/players/silas/tools/talisman"
#define CHECKINGFORBUGS 0	/* MSC93 */
/* MSC93 checking for backdir, not very useful under UNIX however. */

/** This talisman is used to create monsters and other objects.  It creates a
    file for them, so they can be used in your rooms.  Do not use this tool
    to create unique items, create them directly into the room code.  This is
    only for items that you will be cloning multiples of, such that having a
    separate file for them will save disk space.  All files will be saved in
    your /players/NAME file.  So if you want to save 'claw' in your /obj
    directory, when it asks you for a file name type this:
    obj/claw   --  this will save the file as /players/NAME/obj/claw.c
    Yep, the .c is automatically inserted as well.  :-)  **/
object ob;

int i;
string type,file_text,weapon_fname,armour_fname,treasure_fname,filename;
string extra1,extra2,ex_race;
string longs,arm_type;
string old_long;
string chat_str,a_chat_str;
string short_desc,long_desc,alias,race,spell_mess1,spell_mess2,name;
string mess1;
int value,weight,old_num,abort_flag;
int line_num,coins,worn,wielded;
int chat_chance,in_edit;
int extra_wc,extra_chance;
int level,hp,al,wc,ac,aggressive,chance,dam,random_pick,num_chat,num_a_chat;
id(str) {return str == "talisman";}

drop() { return 1; } /* MSC93 */
query_auto_load() { return FILENAME + ":"; } /*MSC93*/

short() { return "a small black talisman"; }
long() {
	write("Object Maker v1.2\n"+
	  "This talisman will let you create various types of objects to use in your\n"+
	  "rooms.\n"+
	  "\nJust type 'create'.\n\n");
	write("Note: On all filename references, /players/"+this_player()->query_real_name()+"/ is automatically added.\n");
}
init() {
...
make() {
	...
	if(this_player()->query_level() < 40) {
		write("Like, yer totally too low in level to use this, k?\n");
		return 1; }
	wielded = 0;
	worn = 0;
	treasure_fname = "";
	...
	random_pick = 0;
	alias = "";
/* If the user chooses not to define
the defined variables of the last creation
are not used in its place.. */
	write("\n"+
	  "The talisman will let you make these types of objects:\n"+
...
weapon_fname(str) {
	if(str && str != "") {
		weapon_fname = "/players/"+this_player()->query_real_name()+"/"+str;
		write("Wielded?: ");
...
armour_fname(str) {
	if(str && str != "") {
		armour_fname = "/players/"+this_player()->query_real_name()+"/"+str;
		write("Worn?: ");
		input_to("get_worn");
		return 1; }
	worn = 0;
...
get_worn(str) {
	...
	return 1; }
treasure_fname(str) {
	if(str && str != "") treasure_fname = "/players/"+this_player()->query_real_name()+"/"+str;
	main_menu();
	return 1; }
get() {return 1;}
...
start(str) {
	if(!str) {
		write("Aborted.\n");
		return 1; }
	if(str == "monster" || str == "weapon" || str == "armour" || str == "treasure") {
		type = str;
		write("Name: ");
		input_to("get_name");
		return 1; }
...
write_monster(str) {
	...
	add_line("");
/*
	add_line("#include \"compat.h\"");
 comment out because there is no compat.h */
	add_line("inherit\"obj/monster\";");
	add_line("reset(arg) {");
	if(coins) add_line("object coins;");
	...
	if(spell_mess1 && spell_mess1 != "") add_line("set_spell_mess1(\""+spell_mess1+"\");");
	if(spell_mess2 && spell_mess2 != "") add_line("set_spell_mess2(\""+spell_mess2+"\");");
	if(num_chat && chat_str != "") add_line("load_chat("+chat_chance+",chat_str);");
	if(num_a_chat && a_chat_str != "") add_line("load_a_chat("+chat_chance+",a_chat_str);");
	if(random_pick) add_line("set_random_pick("+random_pick+");");
	if(coins) {
		add_line("coins = clone_object(\"obj/money\");");
		add_line("coins->set_money("+coins+");");
		add_line("transfer(coins,this_object());");
	}
	if(armour_fname != "") {
...
does_extra(str) {
	if(str != "y" && str != "yes") {
		main_menu();
		return 1; }
	write("This message is seen by others in the room. The next message will be the one\n"+
...
get_extra1(str) {
	...
	if(in_edit) {
		main_menu();
		return 1; }
	write("This message is the one printed to the wielder of the weapon during an extra\n"+
	  "hit.\n");
	print_macros();
	write("Extra string 2: ");
	input_to("extra2");
...
get_arm(str) {
	if(str != "") arm_type = str;
	if(str != "helmet" && str != "magic" && str != "armour" && str != "shield" && str != "ring" && str != "glove" && str != "leg" && str != "cape" && str != "boot") {
		write("Valid armour types are:\n\nhelmet, magic, armour, shield, ring, leg, glove, cape, and boot.\n\n");
...
*/
	...
	if(extra1 != "" && extra2 != "" && extra_wc && extra_wc) add_line("set_hit_func(this_object());");
	add_line("}");
...
ex_race(str) {
	...
	return 1; }
write_it() {
	filename = "/players/"+this_player()->query_real_name()+"/"+filename+".c";
...
check_name(str) {
	...
	if(sscanf(str,"%s//%s",t1,t2)>0) {
		write("// is not allowed in filenames.\n\n");
		abort_flag=1; }
	if(sscanf(str,"%s.%s",t1,t2)>0) {
...
print_macros() {
	write("The following symbols can be placed in your message. The wand will convert\n"+
	...
	write("\nNote: No spaces are needed to seperate these commands.\n");
}
find_last() {
	int k;
	k = i;
