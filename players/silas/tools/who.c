/* Zak's who version 0.1   release: 5/11/92                            */
/* Use it as you see fit just don't claim it as your own, or I'll hunt */
/* you down. :)                                                        */
/* New stuff will be added eventually, so keep your eyes open, and if  */
/* you have any trouble with it, mail me. biswick@ms.uky.edu           */
/* Or come talk to me on Terrapin mud at bessel.umd.edu (128.8.251.131)*/
/* port 2000.                                                          */
/* Suggestions/comments also welcome.                                  */
...
long(str) {
  write("This is Zaknafein's who. To see what it can do, type \"zakhelp\".\n");
}

id(str) {
  return str == "who" || str == "zak's who";
}
...
query_auto_load() {
  return "players/zaknafein/obj/who:"; 
}

init() {
  add_action("help_me","zakhelp");
  add_action("who","zw");
  add_action("idle","idle");
  add_action("where", "where");
  add_action("zaknafein_people", "zpe");
...
help_me() {
  write("The limited abilities of the who currently include:\n"+
	"       zw -- Will show players currently in your realm.\n"+
	"       zw <wizard> -- Will show the players in <wizard>'s realm.\n"+
        "       zw mud -- Will show the players in /room rooms.\n"+
	...
	"       where <player> -- Will show the player's current location.\n"+
        "       zpe -- Will show a modified `people'.\n"+
...
who(str) {
  ...
  list = users();
  drokk_it = 0;      /* Gosh, I just wonder what song was playing here... */
