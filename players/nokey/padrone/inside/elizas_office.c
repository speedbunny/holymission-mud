...
void reset(int arg)
  {
  if (!eliza) start_eliza();
  if(arg) return;
  set_light(1);
  dest_dir = ({"players/padrone/inside/courtyard", "southeast"});
  short_desc = "Eliza's office";
  long_decs = "This is Eliza's office. Eliza is a psychiatrist,\n" +
...
int word_in_phrase(string word, string phrase)
{
    string a, b;
