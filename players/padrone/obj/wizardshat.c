...
reset(arg) {
	...
	dest_room = "players/padrone/tower/hall"; /*no workroom for players*/
    }
    else {
	/* Periodical reset */
	if (!light_is_on) {
...
remove(str) {
    ...
    {  
	this_player()->set_vis();
	write("You become visible again !\n");
    }
    return ::remove(str);
} /* remove */
	...
	if (!dest_room) {
	    ...
	    */
		...
		/*
				...
				this_player()->look(0);
		*/
		this_player()->move_player("in a flash#"+dest_room);
	    } else {
		...
		worn_by = 0;
		worn = 0;
		transfer(this_object(),dest_room);
...
explode_hat() {
    ...
    worn_by = 0;
    worn = 0;
    transfer(this_object(),"players/padrone/store");
    return 0;
} /* explode_hat */
