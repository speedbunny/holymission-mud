 ...
 *
 */
/* Changed by Ethereal Cashimor, 230293 */
#define VERSION "3.5.1 (Feb 23, 1993)"
#define COST 1000
...
reset(arg) {
	...
	move_object(clone_object("obj/soul"), me);
	init_done = 0;
    }
} /* reset */
...
initialize_bodyguard() {
    ...
    set_long(cap_name + " is a bodyguard, currently" +
	     ...
	     cap_name +
	     " is dressed in a dark suit and is wearing a black cape.\n");
...
check_guarded() {
    ...
    else if (present(guarded, environment(me))) {
	if (lost) {
...
random_attack() {
	...
	if (   living(this_ob) && this_ob != me && this_ob != guarded
...
check_fighting() {
    ...
    else if (enemy && present(enemy, environment(me))) {
...
handle_giving(str) {
	...
	if (id(lower_case(whom)) && the_obj) {
		...
		move_object(the_obj, environment(me));
	    }
	...
	if (employed && who == guarded_cap_name)
	    ...
	    object ng;
	    ng = present(lower_case(who), environment(me));
	    if (ng) {
		notify(cap_name + " says: Ok, boss! I'll work for you!\n");
		if (employed && guarded && present(guarded, environment(me)))
...
handle_command(cmdline) {
	...
	obj = present(lower_case(arg), environment(me));
	if (!obj) {
	    notify(cap_name + " says: But there is no " + arg + " here, " +
		   get_honorary() + "!\n");
	}
	else if (!living(obj) || obj == me || obj == guarded) {
...
try_wield(wpn) {
	...
	if (!guarded->add_weight(wpn_weight)) {
	    move_object(wpn, environment(me));
