...
mapping mFingerInfo;	// A mapping of personal finger info.
...
int group(string arg) {
  ...
  if(lel > level + 10 || lel < level - 10) {
    write("You can only group with players within 10 of your level.\n");
    return 1;
  }
...
// 111297: Sauron: Added to stop buggy arena when someone quits.
  if(environment()->query_arena()) {
    if(ARENA->contestant_left(TO))
      ARENA->arena_shout(query_real_name(1)+" quit the bloodbath!\n");
  }
...
#define MAX_PLAY_IDLE  1800
      ...
      if(query_idle(this_object()) >
         ((wiz_level || this_object()->query_testchar()) ?
               MAX_IMM_IDLE : MAX_PLAY_IDLE)) {
/* Sauron: why just save wizards?
        if(is_wizard) {
*/
          save_me(0);
/*
...
// only thiefes and wizards see the full inventory of a player
    ...
    if ( !living(ob) || ob->query_npc() 
                     ...
                     || this_object()->query_real_guild() == 1) {
      max = MAX_LIST;
      msg = "";
...
*/
  ...
  add_action( "nprocess_spell", "cast" );
    ...
    // ----- the rest ...------------------------------
/* Sauron: Don't need this add_action in anymore...
...
// If there is a TP, they have to be an arch.
  else if(TI && environment(TI) && !(int)TI->query_archwiz())
    return illegal_patch(sprintf("set_wiz_level (PO == %O TP == %O)",po,TI));
...
void setFingerInfo(mapping mInfo) {
    mFingerInfo=mInfo;
}

mapping queryFingerInfo() {
    if(!mFingerInfo) {
        mFingerInfo="/sys/bin/player/chfn"->defFingerInfo();
        save_me(0);
    }
    return mFingerInfo;
}
