/* =============================================================== 
   ...
   ---------------------------------------------------------------
   Art: The guildsoul of the monks included in ~sauron/guild/soul.c
...
string bin;        // New cmd bin added 050997 by Sauron.
...
int cmd_filter(string str) {
    ...
    if(verb=="monk/" || verb=="m/" || verb=="monk:" || verb=="m:")
        return (bin+"player/monk")->main(str);

    if(file_size(bin+"player/"+verb+".c")>0)
        if((bin+"player/"+verb)->main(str))
            return 1;
...
// this is important so skill north don't work anymore
            if (arg && query_verb()!="teach" && query_verb()!="locate")
           {
