...
void reset( int arg ) {
  ...
  long_desc = "You are standing at a narrow corridor that leads to the "
    + "cell rooms of the Shire sheriff station. Lit torchs line "
    + "the walls of the corridor and the flickering flames cast "
    + "dancing shadows on the walls. A celldoor is in the south wall.\n";
  dest_dir = ({
    "/players/colossus/mearth/shire/quest/hall13", "east",
    "/players/colossus/mearth/shire/quest/hall11", "west",
  });
  hidden_dir = ({
    "/players/colossus/mearth/shire/quest/cell0", "south",
  });
  items = ({
    ...
    "door", "Try celldoor instead",
    "celldoor", "dummy door descruption",
  });
  ::reset( arg );
