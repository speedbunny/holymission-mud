...
reset(arg){
    ...
    dest_dir = ({
      "/players/colossus/mearth/shire/hall", "southeast",
    });
...
scan_string(scan,str){
    ...
    */
    if(member_array(scan+",",temp)>-1) fin=1;
    if(fin) tell_room(this_object(),"Member_array found.\n");
    return fin;
}
...
say(str){
    ...
    if(!str) return;
    witness=present("MURDER_WITNESS", this_object());
    if(!witness) return 0;
    if(scan_string("job",str))
	witness->state_job(this_player());
    else if(scan_string("name", str))
	witness->state_name(this_player());
    else if(scan_string("harbor",str) && !done[3])

	witness->state_murder_room("harbor", this_player());
    else if(scan_string("square", str) && !done[3])
	witness->state_murder_room("square", this_player());
    ...
    else if(scan_string("inn", str) && !done[3])
	witness->state_murder_room("inn", this_player());
