...
#undef  EXTRA_INIT
#define EXTRA_INIT	extra_init(); ::init();
#undef  EXTRA_MOVE1
#define EXTRA_MOVE1	if (!chk_leave())
#undef  EXTRA_MOVE2
#define EXTRA_MOVE2     if (!chk_leave())  
/*
#define EXTRA_MOVE2\
 if ((TP->query_guild()!=guild)&&(!TP->query_immortal())) {\
    write("Only mages are allowed in the mage tower.\n");\
    return 1;\
 } else if (!chk_leave())  
*/
...
TWO_EXIT("room/mount_top2","south",
   ...
   if (lev == oldlev+1)
   {
       ...
       if(lev > plevs[0])
       {
           GM->change_skill(TP,"mage_practice",
                            pracs[0]+8+(this_player()->query_int()-oldlev)/2,
                            500,5);
           GM->change_skill(TP,"mage_prac_lev",lev,100,5);
       }
   }
   return 1;
}
...
do_join() {
  ...
  if(!TP->get_skill("mage_practice"))
  {
      ...
      GM->change_skill(TP,"mage_prac_lev",this_player()->query_level(),100,5);
  }
  return 1;
}
