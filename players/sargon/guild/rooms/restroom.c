...
reset(arg) {
  if(!arg) {
    ...
    dest_dir = ({
               "/players/sargon/guild/rooms/room", "north",
               "/players/sargon/guild/rooms/hall", "west",
	       "/players/sargon/guild/rooms/magicshop", "up",
                });
    property=({"no_fight","no_steal"});
    smell="It stinks like sweat!! whew!";

     lvlbook = TR(CO("/players/sargon/guild/obj/book.c"), TO);
...
sword() {
  ...
  sword = CO("/players/sargon/guild/obj/bas_sword.c");
  sword->set_weight(0);
  if(TR(sword,TP)) {
...
mantle() {
  ...
  ob2 = CO("/players/sargon/guild/obj/mantle.c");
  if(TR(ob2,TP)) {
    write("You are carrying too much!\n");
    destruct(ob2);
    return 1;
