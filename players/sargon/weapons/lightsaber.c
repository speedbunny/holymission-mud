...
int press(string str) {
    ...
    if(ENV(TO)!=TP || !wielded || TP->query_guild()!=GM->query_number("jedi")) {
...
int weapon_hit(object attacker) {
    ...
    if(attacker->query_wielded()) {
        if (!random(100-(2*TP->LEVEL))) {
            weap = attacker->query_wielded();
            if (weap) {
                weap->drop(0);
...
void fade_away() {
    if ((!living(ENV(TO))) || (ENV(TO)->query_guild() != 4)) {
