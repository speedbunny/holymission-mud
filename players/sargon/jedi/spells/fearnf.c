...
int main(string str) {
    ...
    if(!str) {
       notify_fail("Whom do you want to fear?\n");
        return 0;
    }
    ...
    if(targ == TP) {
        notify_fail("You don't really want to do that.\n");
        return 0;
    }
    if(!living(targ)) {
    ...
    if(targ->query_guild() == 4 && targ->LEVEL > TP->LEVEL - 2) {
        notify_fail(targ->NAME + " is a Jedi, too. You cannot fake " + targ->OBJ + ".\
n");
        TELL(targ,TP->NAME + " tries to fear you.\n");
        return 0;
    }
    if(targ->WIS - 20 > (TP->WIS + TP->CHA)) {
        notify_fail(targ->NAME + " is to wise to fall for that.\n");
        return 0;
    }
...
if(TP->query_legend_level() &&!targ->query_immortal()
                                && random(2)) {
    ...
    targ->run_away();
    if(ENV(targ) == ENV(TP)) {
        notify_fail("The monster does not flee.\n");
    }
    return 0;
}
