...
int main(string str) {
   ...
   notify_fail("Your wounds are not serious enough at this time to heal.\n");
            return 0;
        }
        else {
            a = TP->MAXHP/5;
            if (TP->SP < a/2) {
                notify_fail("You do not have the energy to heal yourself.\n");
                return 0;
            }
            if (TP->SP < a) a = TP->SP;
            TP->ADDHP(a);
...
//            TP->ADDSP(-a);
    ...
    if (str == "poison" || str == "alcohol") {
        ...
        if(!TP->query_poisoned() && !TP->query_intoxination()) {
            notify_fail("You are not poisoned or intoxicated.\n");
            return 0;
        }
        if(TP->SP < a) {
            notify_fail("You lack the power to do that.\n");
            return 0;
        }
