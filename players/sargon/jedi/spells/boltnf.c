...
int _main(string str) {       /*** BOLT Used to send a quick
    ...
    if(env->query_property("no_fight")) {
      notify_fail("You cannot harm anyone in this place.\n");
        return 0;
    }
    dam=5+TP->LEVEL/3+TP->WIS;
...
//    cost=30-TP->INT/3+TP->LEVEL;
    ...
    if(!whom) {
        if(!str) notify_fail("Bolt whom?!\n");
        else notify_fail("No " + CAP(str) + " here.\n");
            return 0;
    }
    if(whom==TP) {
        notify_fail("You don't want to do that.\n");
        return 0;
    }
    if(!living(whom)) {
        notify_fail(CAP(str)+" is not a living thing.\n");
        return 0;
    }

    write("You hold your hands towards " + whom->NAME + ".\n");
    if(TP->SP < cost) {
       notify_fail("You lack the Force to finish the spell.\n");
        return 0;
    }
