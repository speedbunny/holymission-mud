...
#include "/players/mangla/guild/patchdefs.h"
...
void reset( int tick )
{
   ...
   set_long(
            "Yoda has travelled very far, and collected alot of knowledge\n"+
            "in his search for excellence. He ca teach you some of this\n"+
   ...
   /*
   transfer(clone_object(PTAH+"guild/ligthsaber"),TO);
   */
...
int advance_spell( object who, string sn, string fn, int max_sk )
{
   ...
   if ( who->query_guild() != 4 ) {
      ltell( who, "says:", "You are not one with the force!\n", TO );
      return( 1 );
   }
...
int learn_spell( object who, string sn, string fn, int max_sk )
{
   ...
   if ( res == -4 ) {
     ...
     tell_object(who,"Yoda teaches you some the spell.\n");
