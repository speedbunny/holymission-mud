...
throw(str) {
 string player;
  ...
  if(plyr->query_immortal()) {write("No wiz egging please.\n"); return 1;}
  say(capitalize(TP()->query_name())+" hurls a giant egg into the sky!\n"+
  capitalize(player)+" is hit dead center in the face by the egg.\n");

  tell_object(plyr,"You are hit in the face with a giant egg.\n"+
  "You hear "+capitalize(TP()->query_name())+" laughing at you!\n");
