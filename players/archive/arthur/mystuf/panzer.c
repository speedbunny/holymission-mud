...
reset(arg) {
  if (arg) return 1;
   set_id("panzer");
   /* set_light(1); - this goes in a room set light not needed. */
   ...
   if (where == environment(this_object())) {
      move_object(this_object(), "players/arthur/workroom");
      unlock();
   }
...
board(arg) {
   ...
   if (locked == 0) {
      ...
      tell_room("players/arthur/mystuf/inpanzer", this_player()->query_name() +"blah\n");
      move_object(this_player(), "players/arthur/mystuf/inpanzer");
...
jimi() {
   ...
   enfi ="players/arthur/mystuf/inpanzer";
   tell_room("players/arthur/mystuf/inpanzer", "aslf\n");
   ...
   if (!occ) {
      call_other("players/arthur/mystuf/inpanzer", "unlock");
      return 1;
   }
   if (random(5) == 3) {
      call_other("players/arthur/mustuf/inpanzer", "unlock");
       return 1; }
   }
*/
short() {
