...
reset(arg) {
    ...
    set_long("A dual edged single or two-handed bastard sword with dwarven \n"
      +"runes inscribed along its hilt.  Acid burns are prominent along the \n"
      +"otherwise polished blade.\n");
    set_class(15);
    set_value(2500);
    set_weight(1);
...
weapon_hit(attacker) {
	...
	write ("Suddenly the bastard sword shatters into various acid \n"
	  +"burnt pieces, ruining it forever.\n");
	...
	call_out("dest_me",0);
	treasure = clone_object("/players/axlrose/objects/fragments");
