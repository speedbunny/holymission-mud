...
read(str) {
    if (str = "dog tag") {
	write("The tag reads: Belongs to Airborne.\n");
	return 1;
    }
}

heart_beat() {
    who = find_player("airborne");
    ::heart_beat();
    if (who && ENV(who)) {
	if (ENV(who) != ENV(this_object())) {
	    tell_room(ENV(this_object()), "Puppy leaves after Airborne.\n");
	    move_object(this_object(),ENV(who));
	    tell_room(ENV(this_object()), "Puppy arrives following Airborne.\n");
	}
    }
}
