...
init_speeder() {    
    ...
    if(!(l = this_player()->query_alias_list())) {
        l = allocate(2);
        l[0] = allocate(MAX_ALIAS + 1);
        l[1] = allocate(MAX_ALIAS + 1);
        this_player()->set_alias_list(l);
    }
    if(!(l = this_player()->query_history_list())) {
        l = allocate(MAX_HISTORY + 1);
        this_player()->set_history_list(l);
    }
    if(!(l = this_player()->query_forbid_list())) {
        l = allocate(MAX_FORBID + 1);
        this_player()->set_forbid_list(l);
    }
    if(!(l = this_player()->query_nick_list())) {
        l = allocate(2);
        l[0] = allocate(MAX_NICK_NAMES + 1);
        l[1] = allocate(MAX_NICK_NAMES + 1);
        this_player()->set_nick_list(l);
    }
