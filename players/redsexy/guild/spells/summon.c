summon(arg) {
  int i, pos_id, crnum, light_strength, emptyslot;
  ...
  switch(arg) {
	...
	raven = clone_object(CRTS + "raven");
	write("TEST: You chant the words of Summoning.\n");
  ...
  case "wisp":
      ...
      if(!wisp) {
	CHECK_SP(25);
	wisp = clone_object(CRTS + "wisp");
	write("You chant the words of Summoning.\n");
	write("A wisp starts to hover in front of you.\n");
	say(TP->NAME + " traces burning symbols in the air and " +
	    "calls out:\n");
	say("'Te arcesso ut illumines noctem!'\n");
	say("A Living Globe of light appears, hovering before " + TP->SNAME +
	    ".\n");
	TRANS(wisp, TP);
	...
	return 1;
      ...
      write("You have already summoned a Wisp.\n");
      return 1;
      break;
	...
	creatures[emptyslot] = clone_object(CRTS + arg);
