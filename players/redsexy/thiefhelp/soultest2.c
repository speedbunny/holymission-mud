...
#include "/players/brainsprain/guild/headers2.h"
#define CHK_ATT(); \
...
void init() {
  ...
  if (!thief->thief_shadow()) {
    shad = clone_object(dir + "thief_shadow");
...
int get_guild_params() {
  ...
  if(!read_file(TDIR+THRNAME)){
    return 2;
  }
  sscanf(read_file(TDIR+THRNAME),
         "%s",file_string);
