...
int _buy(string str) {
	...
	obh=clone_object("/players/bobo/kobj/glowworm");
        MOVE(obj,TP);
        TP->add_money(-150);
	...
	obj=clone_object("/players/bobo/kobj/salve");
        MOVE(obj,TP);
        TP->add_money(-200);
...
void reset(int arg) {
    ...
    long_desc="You are in the Faun-druid's magic shop.\n"+
              ...
              "counter.\n";
    dest_dir=({
		"players/bobo/krooms/mythwood/faun_mark2","north",
		"players/bobo/krooms/mythwood/faun_drpr","east",
             });
    items=({
