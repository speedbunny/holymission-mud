...
ONE_EXIT("players/bobo/rooms/mythwood/faun_smith","east",
	 "The faun-shop",
...
do_sell(ob) {
    ...
    call_other("players/bobo/krooms/mythwood/store","store,ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if(call_other("players/bobo/krooms/mythwood/store","value",item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/bobo/krooms/mythwood/store","but",item);
    return 1;
}
...
west() {
    ...
    this_player()->move_player("west#players/bobo/krooms/mythwood/store");
    return 1;
}

list(obj) {
    call_other("players/bobo/krooms/mythwood/store","inventorey",obj);
    return 1;
}
