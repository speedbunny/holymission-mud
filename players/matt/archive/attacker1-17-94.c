...
#include "/players/matt/guild/crdefs"
#include "/players/matt/guild/formulae"
#include "/players/matt/guild/functions"

status calmed, berzerk, ter;
int bansp, reqlevel;
string motion, msg, sname;
object att, otherattack, master, cloak;
...
heart_beat() {
  ...
  if(att && (ENV(att) == ENV(TO)) && !berzerk && !calmed &&
    ...
    TO->attack_object(att);
    if(att && att->query_attack() != TO)
      if(TO->HIT(att->query_n_wc() / 3)) {
	TELL(master, att->NAME + " hits your " + TO->NAME + ".\n");
	TELL(att, "You hit " + TO->NAME + ".\n");
	TELLRS(ENV(TO), att->NAME + " hits " + TO->NAME + ".\n",
	       att, TO, master);
      }
    }
