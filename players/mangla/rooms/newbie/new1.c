...
int _open(string str) {
    ...
    if(str == "gate") {
        printf("You open the gate, exit, and close it behind you.\n");
        TP->MOVEP("through the gate#/players/mangla/rooms/newbie/new0");
    }
    return 1;
}
...
reset(arg) {
    ...
    long_desc = "A rather large and peaceful forest where the birds are singing\n" +
                ...
                "There is a large gate to the south. Just open it to leave.\n";
...
//                 NEW + "new0","south",
    ...
    if (!ob1 && !ob2 && !ob3 && !ob4) {
        ...
        MOVE(ob1,TO);
        ob2 = clone_object(MNST + "rat");
        MOVE(ob2,TO);
        ob3 = clone_object(MNST + "rat");
        MOVE(ob3,TO);
        ob4 = clone_object(MNST + "rat");
        MOVE(ob4,TO);
    }
}
