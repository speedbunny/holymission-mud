...
/* 04.19.97 Airborne:  Removed Ansi from guild upon request. */
...
int _do_join() {
    ...
    TP->set_guild(4);
    log_file(LGF, (string)this_player()->RNAME +
...
int query_maxsp(int level,int leg_lev) {
    ...
    if( retval > 570) retval = 630 + (leg_lev * 12);
    return retval;
}
...
int query_maxhp(int level, int leg_lev) {
    ...
    if( retval > 700) retval = 800 + (leg_lev * (12 + leg_lev));
    return retval;
}
...
void reset(int arg) {
    ...
    dest_dir = ({
                 "/players/mangla/guild/rooms/hall","north",
                 "players/redsexy/jedi/areas/island/other/t2", "south",
               });
