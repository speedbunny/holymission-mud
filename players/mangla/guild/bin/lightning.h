...
int do_free(string str) {
	...
	write("You free the energy.\n");
        say(TP->NAME+" fills the area with lightning effect.\n" +
    ...
    if(!preparing) {
         ...
         say(TP->NAME+" throws non-harmful lightning at "+att->query_name()+".\n"+
...
int lightD() {
    ...
    if(TP->query_legend_level())
        return 30 + (TP->query_legend_level() * (random(10) + 1));
    if(TPL<10) return 10;
    if(TPL<15) return 14;
    if(TPL<20) return 18;
    if(TPL<25) return 23;
