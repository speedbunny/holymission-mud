...
/* 281092: Moonchild: fix_weight() implemented */
/* 100393: Ethereal Cashimor: Included read */
/* 040593: Ethereal Cashimor: query_arm_light() implemented */
/* 010693: Ethereal Cashimro: Prepared for documentationscroll */
// 010793: Moonchild, allow no message for non-wearing (wear in this_player()
//         returning -1) for use in my guild
// 110893: Moonchild, strict_types + show size in short() if size!=0
//	   also removed process_string in long() - what about things players
//	   can affect the long() of? there may well be some on here.
// 290893: Q174, Rearranged order of some statements in wear() to optimize cpu.
...
void long(string str) { write(long_desc); }

void fix_weight() { 
  if(!ac || type[0]=="guild") return;
  switch(ac) {
    case 4: if(weight<5) weight=5; break;
    case 3: if(weight<3) weight=3; break;
    case 2: if(weight<2) weight=2; break;
    case 1: 
	if((type[0]=="shield" || type[0]=="boot") && weight<2) weight=2;
	else if(weight<1) weight=1; break;
    default: if(weight<(ac*2-3)) weight=ac*2-3; break;
  }
}
...
//   say(this_player()->query_name()+" failed to wear "+short_desc+".\n");
  ...
  if (size && size!=(int)this_player()->query_size()) {
    write(short_desc+" doesn't fit.\n");
    return 1;
  }
  wea=(object)this_player()->query_wielded();
  if (wea && type[0]=="shield" && wea->query_two_handed()) {
...
in armour.h.
*/
void set_type(string t) { t=="armor"?(t="armour"):(t=t); type=explode(t,"/"); }
