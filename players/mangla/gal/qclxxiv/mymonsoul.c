/* had to add my own copy since including monsoul doesnt seem to be stable */
/* since a lot of my monsters need the commands  Ill add them via mymonster */
...
static string mygTellstring;

filter_tell(ob) {
    if (ob == this_player())
	return 0;
    return ob->catch_shout(mygTellstring);
}
