...
/* next line added Qclxxiv : if invisible then short() returns "" */
int is_invisible;  /* use f.i if items or room-long should describe door */

/* next func added Qclxxiv: call per door to set that door 'invis' */
set_invisible(i) {is_invisible=i;}
/* next aded by Qclxxiv to be able to check open-state */
query_closed() { return is_closed; }

query_dir() { return direction;}
query_locked() { return is_locked; }
query_partner_door() { return partner_door; }
...
init()
{
  ...
  add_action( "go", "go");
  add_action( "go", "enter");  /* added Qclxxiv */
...
which_key()
{
  ...
  for (i=0;i<sizeof(ob);i++) {
      if (ob[i]->id("key") && (str=ob[i]->query_type())) {
		t+=1;
	 	if (t==num_key) write(" or the "+str+" key?\n");
	 	else if (t==1) write("The "+str+" key,\n");
