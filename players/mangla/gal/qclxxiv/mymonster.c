...
*/
inherit "obj/monster";
#include "/players/qclxxiv/living.defs"
#include "/players/qclxxiv/mymonsoul.c"
...
reset(arg) {
	...
	sreset(arg);
	if (arg) return;
	set_name("An unfinished monster of Qclxxiv's");
	set_race("monster");
}

id(str) {
	if (::id(str)) return 1;
	else return str=="Qclxxiv monster";
}

its_a_qclxxiv_monster() { return 1; }
