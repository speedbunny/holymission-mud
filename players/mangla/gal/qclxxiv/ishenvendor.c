...
reset(arg) {
  object spliff, suitcase, coat;
	...
	load_chat(18, ({
		...
		"Ishen vendor whispers: Sensimilla spliff for 300! Four for 1000!\n",
		"Ishen vendor says: 4 or more spliffs are only 250 the piece\n",
	...
	if (sscanf(str,"%s gives you %s.",who,what)==2 
		...
		if (sscanf(what, "%d gold coins", coins)==1) {
			if (coins>=300) {
				...
				if (coins>=1000) num = coins / 250;
				else num = coins / 300;
				dest =  find_player(lower_case(who));
				while (!dest) {
				...
				for (t=1; t<=num; t++) {
					spliff=clone_object("/players/qclxxiv/obj/spliff_s");
					say("Vendor says: thank you "+who+".\n");
					if (transfer(spliff, dest )!=0) {
						failed++;
					}
				}
/*				write("The ishen vendor gives you "+num+" spliffs.\n"); */
				if (failed) {
					num-=failed;
					if (num<=0) {
						notify("couldn't give you any spliffs, maybe too heavy?\n");
					}
					else notify("Couldn't give you all spliffs, maybe too heavy.\n");
					init_command("drop "+300*failed+" coins");
				}
				if (num>1)
					say("The ishen vendor gives "+who+" "+num+" spliffs.\n");
				else
					say("The ishen vendor gives "+who+" one good spliff.\n");
				if ((failed+num)*300 < coins) 
