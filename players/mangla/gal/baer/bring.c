#define FILENAME "/players/baer/bring:"
...
init () {
    add_action ("f",        "find");  add_action ("ring_widge",        "r");
}
get ()   { return 1; }
drop ()  { return 1; }
...
int ring_widge(string arg)
{
   object mob;
    ...
    if (this_player()->query_real_name()=="baer")
        mob = find_living("widge");

    if (!mob)
    {
       notify_fail("Widge isn't here :(\n");
       return 0;
    }
