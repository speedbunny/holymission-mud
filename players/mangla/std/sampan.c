...
reset(arg){
    if(arg) return;
    if(!captain){
	move_object(clone_object("/players/haplo/ishikawa/monsters/fisher"),
	  this_object());
    }
	...
	"/players/haplo/ishikawa/jungle/beach1"}));
    move_object(this_object(),"/players/haplo/ishikawa/jungle/beach1");
    ::reset(arg);
}
