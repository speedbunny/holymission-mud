...
#define TPARCH	(TP->query_level()>=50)		/* damn ... a constant again */
...
int i,j,k,f;
  ...
  msg="";
  if (!silent) {
    msg="You've enterd another weird game from Herp the Hacker.\n\n";
    if (!arr) {
...
register() {
  ...
  if (!op1) {
     ...
     rn1=TPRN;
     op1=TP;
     write("You have registerd to the game.\n" +
...
int slot;
  ...
  switch (chk_win(slot)) {
  case -1 : who_moves=-1; 
	    SAY("Herp shouts: "+n1+" and "+n2+" played remi against "+
		  "each other in 4wins.\n"); back_player(); return 1;
  case 1  : if (who_moves) { w=n2;l=n1; }
	    else { w=n1;l=n2; };
	    who_moves=255; who_won=w;
	    SAY("Herp shouts: "+w+" won against "+l+" playing \"Connect "+num+"\".\n");
	    back_player(); return 1;
  }
