...
a_chat_str[7] = "The Rastafarian says: see de hypocrite. Dem a galong dey.\n";
   ...
   ras = clone_object("obj/monster");

transfer(ras,"room/church");
	/* Reuse the same arrays. */
call_other(ras,"load_chat",2,chat_str);
call_other(ras,"load_a_chat",20,a_chat_str);
call_other(ras,"set_match",this_object(),function,type,match);
call_other(ras,"set_name","rastaman");
call_other(ras,"set_alias","rastafarian");
call_other(ras,"set_short","A Rastaman");
call_other(ras,"set_long","He is smoking a giant spliff of ganja.\n");
call_other(ras,"set_ac",14);
call_other(ras,"set_level",18);
call_other(ras,"set_al",1000);
call_other(ras,"set_hp",220);
call_other(ras,"set_ep",100000);
call_other(ras,"set_wc",17);
call_other(ras,"set_aggressive",0);
	
call_other(ras,"set_random_pick",20);
call_other(ras,"set_move_at_reset",0);
    }
}
...
notify("The Rastaman says: Maybe " + who + "is on thje way to Zion.\n");
    }
