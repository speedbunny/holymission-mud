...
void reset(int flag) 
{
	...
	claw=clone_object("players/haplo/ishikawa/obj/serpentclaw");
	mail=clone_object("players/haplo/ishikawa/obj/serpentmail");
       crown=clone_object("players/haplo/ishikawa/obj/crown");
...
// we check the filename too, because there can me other things called star
    ...
    if(_star && 
      explode(file_name(_star),"#")[0] == "players/haplo/ishikawa/obj/star") 
      return 0;
    else 
    {
       _star = clone_object("players/haplo/ishikawa/obj/star");
       if (transfer(_star, _pl))  /* too much weight */
          move_object(_star, environment(_pl));
          transfer(clone_object("/players/haplo/ishikawa/obj/serpent_id"), this_player()),
   ...
   {      
       _id= clone_object("/players/haplo/ishikawa/obj/serpent_id");
       if (transfer(_id, _pl))
          move_object(_star, environment(_pl));
   }
  return 0;
...
hit_player(dam,kind)  /* poisons only when it got a hit */
{
	...
	tell_room(environment(),
	  "Serpent King claws "+this_player()->query_name()+".\n");
...
int attack() 
{
	...
	if(this_player() && living(this_player()))
	{
		...
		if (inv[i]->query_wielded())   /* a wielded weapon */
		{
		    ...
		    this_player()->stop_wielding(inv[i]); /* don't destruct :*) */
		    move_object(inv[i],this_object());
		    say("The serpent grabs "+this_player()->query_name()+
...
anti_magic(){
 write("Kyoshi radiates a field of anti_magic!\n");
 return 1;
  }

// Mangla: added to stop it from being summoned in mages tower...
no_summon() {
    return 1;
}
