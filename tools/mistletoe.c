/* mistletoe 
   ...
   This a a combination between the dynion (markertool) and the pointer.
   It uses both advantages and is level secured.  

Mangla: changed it to be more level restricted so all levels can use.
	all commands of pointer and dynion should work if you have
	problems with this item then please let me know. This include
	possible security leaks!!

Nylakoorub: add the shout curse command from the dynion..
	    also made unshoutcurse, which you can remove a shoutcurse
	    made shoutcurse/unshoutcurse... logged, can't shoutcurse
	    someone at or above your level..
	    changes approved by Silas 3-27-95

Mangla: I checked this thing and therefore I WANT IT HOW WAS
	the shout curse changes are NOT needed and are illegal.
	Also they are bugged. Code does not work.

Turbo: Changed layout of 'help marker'.  11 March 1999
If you want changes made to the tools then contact Mangla or Sauron.
*/
...
void long() 
{
  write("This is just a combination between the markertool\n"+
   "and the utility tool, pointer. To get the marker commands\n"+
   "type: help marker. To get the utility commands,\n"+
   "type: help utility.\n");
  if (hide)
...
void init() 
{
  if ( !environment() || !(environment()->query_immortal()) )
...
int do_unprison(string arg)
{
  ...
  write("You unprisoned "+ob->query_real_name()+
...
#if 0
shout_curse(string arg)
{
  object ob,curse;
  if (!arg || !interactive(this_player()))
  {
    write("Shout curse whom?\n");
    return 1;
  }
  ob = find_living(arg);
  if(!ob || !ob->query_player())
  ...
  {
    write("You can only shout curse a lower level!\n");
    return 1;
  }
  else if(present("shcurse",ob))
  {
    write(capitalize(arg)+" already is shout cursed.\n");
    return 1;
  }
  curse = clone_object("obj/shout_curse");
  curse->start(ob);
  write("Ok, "+capitalize(arg)+" is shout cursed.\n");
  log_file("SHOUT_CURSE",this_player()->query_real_name()+
   " shout cursed "+ob->query_real_name()+" at "+
   ctime(time())+"\n");
  return 1;
}

int do_unshout(string arg)
{
  object ob,curse;
  if (!arg || !interactive(this_player()))
  {
  ...
  {
    write(capitalize(arg)+" is not shoutcursed!\n");
    return 1;
  }

  curse = present("shcurse",ob);
  write("You unshout "+ob->query_real_name()+" and it's logged.\n");
  if(curse)
    destruct(curse);
  tell_object(ob,"You start to feel that you can shout again, so you\n"+
   "let out a big shout!\n");
  command("shout I can shout again, praise the lord!",ob);
  log_file("SHOUTCURSE",this_player()->query_real_name()+
   " unshout cursed "+ob->query_real_name()+
   " at "+ctime(time())+"\n");
  return 1;
}
#endif
...
int help(string str) 
{
  ...
  {
    write("The mistletoe is only for the use of immortals.\n");
    destruct(this_object());
    return 1;
  }
  ...
  if (str=="utility") 
  {
    ...
    if(WIZ)
      write(
       "COMMNDS WIZ:\n"+
       "noshout [name] placing a shout curse on someone, must be below your level\n"+
       "unshout [name] remove a shout curse from someone, any level\n"+
  ...
  {
    if(!WIZ) {
      write("You are not allowed access to these abilities.\n");
      return 1;
    }
    write("WIZARD COMMANDS:\n"+
     "align [string] -- changing the alignment\n"+
     "clear [file] -- all items in room destructed\n"+
     "scan -- a 'look' that shows all objects and filenames\n"+
     "I -- improved i that shows all objects and filenames\n"+
     "il -- 'mark' a living\n"+
     "idl -- list inventory of marked living\n"+
     "im [object] -- 'mark' an object (use the filename or the number from idl)\n"+
     "ic [function] [mixed] -- calling a function within an object\n"+
     "ic efun [efun] -- calling an efun within an object\n"+
     "ic code [code] -- writing a code into a marked object\n"+
     "ime -- mark yourself\n"+
     "i_clone [file] -- clone an object into the marked object\n"+
     "im last -- marking the last marked object (variables mark && last)\n"+
     "isw -- swaping the last with the marked object\n"+
     "ic init - calling the init of a marked object\n"+
     "im object [file] -- makring the maser object\n"+
     "i_who or i_who [num] -- getting the userlist as marked\n"+
     "imo [where] -- moving a marked object\n"+
     "ih -- marking your environment\n"+
     "ish - advanced look of a marked object (doesn't work)\n"+
     "ir or ir [num] -- reseting a marked object\n"+
     "icl -- clearing a marked object\n"+
     "id -- destruct marked object\n"+
     "i_more -- read file of marked object\n"+
     "where [item] or [player] -- filename\n"+
     "EXAMPLES:\n"+
     "1.) ime, idl, im1, i_more\n2.) i_who, i_who 2, ic query_real_name\n"+
     "3.) il demolian, ic code mark->run_away() \n"+
     "4.) i_clone /obj/armour, ic set_ac 4, ic set_name mail, \n"+
     "    ic code move_object(mark,last)  [last == demolian]\n"+
     "5.) im object /players/mangla/workroom, icl\n");
    return 1;
  }
  return 1;
}
...
int mmore(string arg)
{
  string *ploder;
  string file;
  object mob;

  if (arg) mob = find_player(arg);
  else mob = environment(this_player());
  if (mob && mob->query_player()) {
  ...
  {
    write("Usage: mmore [arg]\n"+
     "mmore to read your environment code\n"+ 
     "mmore [object] to read code of a present item.\n");
    return 1;
  }
  return 1;
}
...
int uphere()     
{
  ...
  for (sz = sizeof(pl), i = 0; i < sz; i++)
    move_object(pl[i],"/room/temproom");
...
/* Mangla: What is this function?  I see no use.
...
int minto(string arg) 
{
  ...
  if (!arg) 
  {
    notify_fail("Syntax: minto <num>     Current intoxication="+INTO+"\n");
    return 0;
  }
  if (sscanf(arg, "%d", i) == 1) 
  {
...
int mmail() 
{
  ...
  {
    write("You already possess an invisible mailreader.\n");
    return 1;
  }
  transfer(clone_object("obj/mail_reader"),TP);
...
int mitem(string arg) 
{
  ...
  if (sscanf(arg,"%s in %s",arg1,arg2) == 2) 
  {
    if (!find_living(arg2)) 
    {
      notify_fail("Could not find "+arg2+".\n");
      return 0;
    }
...
int orc(string arg) 
{
  ...
  if (sscanf(arg , "%d" , i) == 1) 
  {
    ...
    {
      write("You shall give an argument > 0 \n");
    }
    if(!WIZ) {
      file = "/" + file_name(E(TP));
      files = explode(file,"/");
      if( files[1] != TP->query_real_name() ) {
	write("You cannot place an object outside your area!\n");
	illegal = 1;
	return 1;
      }
      illegal = 0;
...
int weap(string arg) 
{
  ...
  {
    if (i>20) {
      write("The class of the weapon was > 20  Try again.\n");
    }
    else if (i<=0) 
    {
      write("You shall give an argument > 0 \n");
...
int arm(string arg) 
{
  ...
  if (sscanf(arg, "%d " ,i) == 1) 
  {
    if (i>4) 
    {
      write("The class of the armour was too high. Try again.\n");
    }
    else if (i<=0) 
    {
      write("You shall give an argument > 0 \n");
...
int mguild(string str) 
{
  ...
  if(this_player()->query_real_guild() == gnr)
  {
...
/* Mangla: reducing the number of shadows in game
		   so certainly do not need this any more.
