...
string tmp;
  ...
  if ((int)ob->is_invis_for(TP)) return 0;
#if 0
...
{
  if(objectp(dest) && interactive(dest) && item->query_weight())
  {
      log_file("MOVE_OBJ","Illegal 'move_object' "+
               (stringp(item) ? item : file_name(item))+" to "+
               file_name(dest)+". \nPRV_OBJ: "+
