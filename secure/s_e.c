 ...
 *   3: use the list of non-interactive livings (ie npcs only) in the
 *      environment of this_player().
 *   4: use the list passed via the oaArr argument.
 */
...
// Player's names are always unique - no need to use unique_array().
    oaArr = filter_array(ALL(ENV(TP)), #'interactive);
  else if(iFlag == 2) {
// Get a list of unique names in case there are lots of monsters with the
// same name in the player's environment.
    xaTmp = unique_array(filter_array(ALL(ENV(TP)), #'living),
                         "query_real_name");
// Since we are matching unique names, we just want the first object from
// each group of objects.
    oaArr = ({});
    for(i = 0; i < sizeof(xaTmp); i++)
      oaArr += ({xaTmp[i][0]});
  }
  else if(iFlag == 3) {
// Get a list of unique names in case there are lots of monsters with the
// same name in the player's environment.
    oaArr = filter_array(ALL(ENV(TP)), #'interactive);
    xaTmp = unique_array(filter_array(ALL(ENV(TP)), #'living) - oaArr,
                         "query_real_name");
// Since we are matching unique names, we just want the first object from
// each group of objects.
    oaArr = ({});
...
// If we have an exact match, return the string.
    if(sTmp == sName)
      return sName;
// If not, check for a partial match.
...
precise.\n",
    ...
    printf("No-one found wth a name to match '%s'.\n", sName);
}
