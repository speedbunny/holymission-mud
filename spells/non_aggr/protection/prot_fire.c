...
void reset( int tick )
{
   ...
   set_name( "protection from fire" );
   set_kind( NON_AGGRESSIVE );
   set_syntax( "cast 'protection from fire' [target]" );
   set_save_vs( NO_SAVE );
   set_cost( 70  );
	set_guild_allowed(2,14);
   set_speed( 1 );
   set_player_help(
	"You become nearly invulnerable to damage from fire."
   );
}
...
void release_spell( string arg, int act_skill, int max_skill )
{
   ...
   if ( check_success( act_skill, max_skill ) ) {
    ...
    shad = clone_object("/spells/non_aggr/protection/fire");
    shad->start_shadow(oppo,time);
    if(TP!=oppo) write(oppo_name+" is protected from fire.\n");
    say(oppo_name+" shimmers red for a second.\n",oppo);
    tell_object(oppo,"You are protected from fire.\n");
   }
}
