...
void reset( int tick )
{
   ...
   set_name( "cure serious" );
   set_kind( NON_AGGRESSIVE );
   set_syntax( "cast 'cure serious' [target]" );
   set_save_vs( NO_SAVE );
   set_cost( 50 );
	set_guild_allowed(2,8);
...
void release_spell( string arg, int act_skill, int max_skill )
{
   ...
   if ( check_success( act_skill, max_skill ) ) {
      ...
      if ( do_it )
    {
        cure = roll_dice(4,40,TP->query_level());
        ...
        {
          tell_object(oppo,"You feel somewhat better.\n");
          if(TP != oppo) write(capitalize(oppo_name)+" feels better.\n") ;
          say(oppo_name+" feels better.\n",oppo);
        }
