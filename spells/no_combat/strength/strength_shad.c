...
start_shadow(who, time, num) {
    if(!num)
    {
        newstr = who->query_r_stats(0) + this_player()->query_level()/5;
/********* Removed since level 1 players gets 39 STR.
        newstr = (50-apply(#'call_other,who,"query_r_stats",0))/4 +
                 apply(#'call_other,this_player(),"query_level")/5 + 1;
***************/
    }
    else {
        if(this_player()->query_level() + 5 > num)
            num = this_player()->query_level() + 5;
    }
