...
void long(string str) {
    ...
    if (!str) {
      ...
      if(!no_obvious_msg) query_obvious_exits();
...
static void query_obvious_exits() {
  string premsg,exits;
  int i,j;
  if(!dest_dir || !(j=sizeof(dest_dir))) {
    no_obvious_msg="    No obvious exits.\n";
    return;
  }
  i=1;
  if(j==2) 
    ...
    premsg=sprintf("    There are %s obvious exits: ",
	((string *)SHARE->share_room_numbers())[j/2]);
  exits=dest_dir[i];
  for(i=3;i<j;i+=2) {
    if(i==j-1) 
  ...
  no_obvious_msg=sprintf("%s%-=*s\n", premsg, SCREEN_WIDTH-strlen(premsg),
	exits);
}
