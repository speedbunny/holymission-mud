...
long(str) {
    ...
    if (!str) {
      ...
      if (!dest_dir) 
        write((no_obvious_msg ? no_obvious_msg : "    No obvious exits.\n"));
      else {
        i = 1;
        if (!pointerp(numbers)) numbers=SHARE->share_room_numbers();
        if (sizeof(dest_dir) == 2) premsg="    There is one obvious exit: ";
        else if (sizeof(dest_dir) > 18) 
            premsg="    There are many obvious exits: ";
        else premsg="    There are " + numbers[sizeof(dest_dir)/2] +
                   " obvious exits: ";
        for(i=1,exits="";i<sizeof(dest_dir);i+=2) {
            if (i!=1) {
                if (i == sizeof(dest_dir) - 1) exits+=" and ";
                else exits+=", ";
            }
            exits+=dest_dir[i];
        }
...
/* This command is used to print the local description of a room, when
   ...
   exactly, but somewhere in Ted's at least. Cashimor */

query_long() {
  return lw(process_string(long_desc));
}
