...
int check_had_account(object ob) {
int i,testyear,nowyear,tmcount,nmcount;
string *list,testmon,nowmon,*months;
mixed *line,junk,*nowtime;
  list = explode(read_file("/log/FICO.CLOSE"),"\n");
  testmon = 0;
  testyear = 0;
  for(i=0;i<sizeof(list)-1;i++) {
    line = explode(list[i]," ");
    if(line[8] == ob->query_real_name()) {
      testmon = line[1];
      testyear = line[4];
      if(line[2] == ".") testyear = line[5];
      }
    }
  if(testmon) {
    nowtime = explode(ctime(time())," ");
    nowmon = nowtime[1];
    nowyear = nowtime[4];
    if(nowtime[2] == ".") nowyear = nowtime[5];
    months = ({"Jan","Feb","Mar","Apr","May","Jun",
               "Jul","Aug","Sep","Oct","Nov","Dec"});
    nmcount = member_array(nowmon,months);
    tmcount = member_array(testmon,months);
    if(nmcount > tmcount + 1) return 0;
    else if(nmcount + 12 > tmcount + 1 && nowyear > testyear) return 0;
    else return 1;
    }
  return 0;
  }
...
mixed junk;
  ...
  /* check that, if the player had an account here before, it was at least
     one month ago. */
  if(check_had_account(this_player())) {
    write("The clerk opens a thick leather book. He checks, and checks, and checks again.\n"
        + "The clerk tells you: You had an account here recently. You are not yet\n"
        + "allowed to open a new one.\n");
    return 1;
    }
