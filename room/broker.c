/* Uglymouth 400130: Fixed bugs due to lost .o file. Changes made in
     do_account, do_statement, do_deposit and do_withdraw.
     Updated do_clean to new playerfile structure.
     Now for wizards only: listbank, cleanbank and reload. */
...
/* Uglymouth 940130: Should only be available to wizards. */
    if(!TPWIZARD)
        return 0;
...
/* Uglymouth 940130: changed to new structure in playerfiles.
	if  (file_size("/players/"+ctab[0][i]+".o")>=0) {
*/
        if (file_size("/p/"+ctab[0][i][0..0]+"/"+ctab[0][i]+".o")>=0) {
	    ct_new[0] += ({ ctab[0][i] });
	    ct_new[1] += ({ ctab[1][i] });
	}
    }
...
/* Uglymouth 940130: have to prevent double broker_ids in players
  move_object(clone_object(IDFILE),this_player());
*/
  if(!id_present) get_and_save_id
...
/* Uglymouth 940130: bug due to lost .o file when player has broker_id
     but no account. Fixed by inserting this check. */
  if(!chk_existing(TPRNAME)) {
    while(id_present) destruct(id_present);
    write("The clerk says: Due to a major re-organisation your account has been lost.\n"
      + "\tYou can open a new one, if you like by typing 'account'\n");
    return 1;
  }
...
/* Uglymouth 940130: bug due to lost .o file when player has broker_id
     but no account. Changed message, so players can fix themselves. */
   if(idx < 0)
   {
/*
...
int reload_bank() {
/* Uglymouth 940130: Only wizards should be able to do this.
...
*/
  if(!TPWIZARD)
    return 0;
  nr_of_accounts = sizeof(ctab[0]);
  for (i=0; i<nr_of_accounts; i++)
      write(ctab[0][i]+"\n");
  write(nr_of_accounts + " Players have an account here.\n");
  return 1;
}
...
void reset(int arg) {
  ...
  + "To the south you notice the entry to the old bank hall.\n"
  + "If you don't have an account, you should open one!\n\n"
  + "Type 'help bank' to get more information.\n"
  + "Muzmuz knows the bank is buggy. So do NOT bug him about it anymore.\n"
  + "He already tries to find and eliminate the bug.\n"
