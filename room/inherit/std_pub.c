...
Jun 28 1997  Kryll        Cleaned up the file, debugged it, and now usable
                          with USERDOCs.
Apr 03 1998  Mangla       Added in the ability to have a pure SP boosting
                          drink without healing HP
*/
...
status _buy(string str) {
  ...
  switch(item_list[what][3]) {
    ...
    case "spell":
      if(!TP->drink_soft(item_list[what][2]))
        return 1;
      if(TP->SP + item_list[what][2] <= TP->MAXSP)
        TP->ADDSP(item_list[what][2]);
      else
