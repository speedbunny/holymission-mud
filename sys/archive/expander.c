...
/* #sourcer# max-number of alias is 30 941009 */
...
static int afk;
static string check_fn;
static string check_funct;
...
reset_expander(arg) {
  ...
  reset_prompt();
  lock_time=0;  // just to be sure - Llort
  check_fn=0;
  lock_msg = "Input locked !";
  afk = 0;    // tschusd to mage schure - pat
...
init_expander() {
  ...
  add_action("set_afk", "afk" );
}

set_afk()
{
   write( "You quietly go afk ...\n" );
   say( this_player()->query_name() + " quietly goes afk ...\n" );
   afk = 1;
...
rem_afk()
{
   say( this_player()->query_name() + " returns to the keyboard.\n" );
   write( "Your return to your keyboard.\n" );
   afk = 0;
...
static add_alias(str) {
  ...
  if(!str) {
    ...
    write("You have "+sizeof(alias_list[0])+" alias, the Maximum is "+
	MAX_ALIAS+".\n");
    return 1;
  }
  cmds=explode(str," ");
  ...
  write("You have "+(sizeof(alias_list[0])+1)+" alias, the maximum is "+
	MAX_ALIAS+".\n");

  if(sizeof(alias_list[0]) > MAX_ALIAS)
    {
...
string _xx() {
  ...
  s=(int *)this_object()->conf_prompt();
  pr=(string)get_prompt_str();
  path=(string)this_object()->query_path();
  ...
  if(this_object()->query_immortal() ||
    ...
    ret+=((s[PR_CR2] || pr=="") ? "" : " - ");
    ret+=(s[PR_NUM] ? (hist_faktor*MAX_HIST+phist+1)+" - " : "");
    ret+=(s[PR_DAY] ? ctime()[0..2]+" - " : "");
    ret+=(s[PR_TIM] ? ctime()[11..<9]+" - " : "");
    ret+=(s[PR_CWD] ? "/"+path+" - " : "");
...
void reset_prompt() {
  int *s;
  string pr;
  set_prompt(symbol_function("_xx",this_object()));
}
...
string modify_command(string str) {
  ...
  if(check_fn)
  {
      if(apply(call,check_fn,check_funct,this_object()))
          return "";
  }
...
// time in heartbeats. -1 infinite, 0 next heartbeat, ...
lock_input(time, msg )
{
  if(time<0) lock_time=-1;
  else lock_time = time+1;
...
check_input( checker, funct)
{
  if(check_fn || !checker || !funct || !stringp(funct) 
//     || explode(checker,"/")[0]=="players"
    )
       return 0;
  check_fn=checker;
  check_funct=funct;
