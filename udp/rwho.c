...
rwho_message()
{
    ...
    */
    ...
    for (wiz = ({}), ply = ({}), il = 0; il < sizeof(u); il++)
    {
	if (u[il] && u[il]->query_wiz_level())
	    wiz = wiz + ({ capitalize(u[il]->query_real_name()) });
	else if (u[il])
	    ply = ply + ({ capitalize(u[il]->query_real_name()) });
    }

    r +=  capitalize(LANG_WNUM(sizeof(u) - sizeof(wiz))) + " players and " +
	LANG_WNUM(sizeof(wiz));

    if (sizeof(wiz) == 1)
	r += " wizard. Wizard is:\n";
    else if (sizeof(wiz))
	r += " wizards. Wizards are:\n";
    else
	r += " wizards.\n";
    ...
    if (sizeof(ply))
	r += break_string(COMPOSITE_WORDS(ply), 70, 3);
