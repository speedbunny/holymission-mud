...
int atoi(string args)
{
    int ret;
    if (sscanf(args,"%d",ret))
	return ret;
    else
	return 0;
}
...
finger_player(string player)
{
	...
	str = ob->query_name() + " is currently in the game, Location: " +
    ...
    str += pron + " is level " + ob->query_wiz_level() + " (" +
	capitalize(WIZ_LEV_NAME(ob->query_wiz_level())) + ").\n";
    str1 = lower_case((string)ob->query_real_name());

    str3 = (string)SECURITY->query_wiz_chl(str1);
    if (strlen(str3))
	str += pron + " was set to this level by " + capitalize(str3) + ".\n";
	...
	if (WIZ_TYPE(ob->query_wiz_level()) == WIZ_LORD)
	    str += pron + " is lord of the domain " + str2 + ".\n";
	else {
            if (strlen(str3))
                str += pron + " is a member of the domain " + str2 +
		      " with the lord " + capitalize(str3) + ".\n";
	    else
		str += pron + " is a member of the domain " + str2 +
		      " with no lord.\n";
	}
    ...
    str3 = (string)SECURITY->query_wiz_chd(str1);
    if (strlen(str3))
	str += pron + " was added to this domain by " + capitalize(str3) + ".\n";
