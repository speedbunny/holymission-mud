...
dump_max_stats(r,leg_lev)
{
   string rtc,glds;
   int i,j,k,sum,hp,sp,fp;
...
//   i=r;
   {
       ...
       {
           hp=GM->query_maxhp(j,race_info[i][3][2],leg_lev);
           sp=GM->query_maxsp(j,race_info[i][3][3],leg_lev);
           fp=GM->query_maxfp(j,race_info[i][3][1],leg_lev);
           sum = hp+sp+fp;
           rtc+="  "+GM->query_name(j)+":  \t"+hp+"/"+sp+"/"+fp+"\t= "+sum+"\n";
       }
   }
   return rtc;
}
