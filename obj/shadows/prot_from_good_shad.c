...
varargs int hit_player(int dam,int kind,mixed ele) 
{
   if (dam > 0)
   {
      if ( chk_al = (apply(call,this_player(),"query_alignment") > 0) )
          return apply(call,me,"hit_player",(dam*9/10),kind,ele);
   }
