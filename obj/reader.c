/* last edit 24.04.92 by calidor */
/* 24.04.92: added the possibility to have more "cc's" by seperating
	     them with a "," e.g. cc:herp,jug,kelly
	     added also the possibility to mail to ALL arches or lords
	     by typing "mail arches" or "mail lords" ... calidor */
...
static string arches = "jug,james,calidor";
static string lords  = arches+",kelly,wolfi";
...
mail_to_continue(str) {
    str = lower_case(str);
    if (!restore_object("players/" + str)&&str!="arches"&&str!="lords") {
...
/* HERP */
    if (!restore_object("players/"+dest)&&dest!="arches"&&str!="lords") {
...
help_msg() {
    ...
    write("\n<name>    You can mail to "arches" or "lords", too.\n");
    return 1;
}
