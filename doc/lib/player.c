...
void logon2() {
  ...
  if(lv=master_object()->get_wiz_level(name)) {
    ...
    if(level!=lv) {
      level_changed = "Your level has changed during your absence!\n" +
...
int query_exp_next_level() {
  ...
  return( (ne <= 0) ? funcall(call, GM, "query_exp", guild, L_PLAYER+1, 0)
                    : ne );
}
 ...
 */
  ...
  if (is_invis && level < apwz)
    set_vis();
  if (is_invis)
    write("YOU ARE INVISIBLE (level "+ is_invis+")!\n\n");
...
nomask int query_immortal()     { return level>L_PLAYER; }
nomask int query_newwiz()       { return level>L_APPR; }
nomask int query_wizard()       { return level>L_NEWWIZ; }
nomask int query_lord()         { return level>L_SAGE; }
nomask int query_archwiz()      { return level>L_ELDER; }
